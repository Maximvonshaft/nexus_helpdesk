...
| retired legacy session sync jobs are accumulating | Runtime health / jobs table | P2 | Confirm no old producer is enqueueing `openclaw.sync_session`; these jobs should be drained as retired compatibility work. |
...
5. Explicit statement that outbound is either `disabled/queued-only`, `enabled/native`, `enabled/email`, or another approved non-OpenClaw adapter.

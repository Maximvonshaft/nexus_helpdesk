...
def _knowledge_hits_from_rag_trace(rag_trace: dict[str, Any]) -> list[KnowledgeChunkHit]:
    ...
    for index, item in enumerate(top_hits[:10]):
        ...
        hits.append(KnowledgeChunkHit(
            ...
            direct_answer=item.get("direct_answer") if isinstance(item.get("direct_answer", None), str) else None,
            answer_mode=item.get("answer_mode") if isinstance(item.get("answer_mode", None), str) else None,

...
from app.services.webchat_debug_bundle_service import (
    build_ai_debug_bundle,
    create_test_finding,
)
...
def _ensure_schema_and_user() -> None:
    ...
    with engine.begin() as conn:
        ...
        if "runtime_trace_json" not in columns:
            conn.execute(
                text(
                    "ALTER TABLE webchat_ai_turns "
                    "ADD COLUMN runtime_trace_json TEXT"
                )
            )
        outbound_columns = {
            col["name"]
        ...
        for column_name, column_type in outbound_contract_columns.items():
            if column_name not in outbound_columns:
                conn.execute(
                    text(
                        "ALTER TABLE ticket_outbound_messages "
...
def _clear_webchat_ai_jobs() -> None:
    ...
    try:
        ...
        if job_ids:
            db.query(WebchatAITurn).filter(
                WebchatAITurn.job_id.in_(job_ids)
            ).update(
...
def test_ai_turn_completes_and_clears_pending_after_dispatch(monkeypatch):
    ...
    def fake_run_runtime(**_kwargs):
        return _runtime_reply(
            ...
            runtime_trace={
                ...
                "endpoint": (
                    "https://apis.speedaf.com/open-api/mcp/order/query"
                    "?appCode=SHOULD_NOT_PERSIST"
                ),
    ...
    try:
        ...
        finding = create_test_finding(
            ...
            finding_type="other",
            tester_note="Ticketless Agent turn remains inspectable.",
        )
        assert finding.ticket_id is None
        assert (
            db.query(WebchatAIDebugRun)
            .filter(WebchatAIDebugRun.ai_turn_id == turn.id)
            .count()
            == 1
        )
        assert (
            db.query(WebchatAITestFinding)
            .filter(WebchatAITestFinding.debug_run_id == debug_run.id)
            .count()
            == 1
        )
    finally:
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: _runtime_reply(
            ...
            recommended_agent_action=(
                "Human agent should review the customer request."
            ),
            elapsed_ms=34,
            runtime_trace={
                "agent_runtime": True,
...
def test_failed_ai_turn_persists_safe_runtime_trace():
    ...
    sent = _send(
        ...
        conversation_id,
        visitor_token,
        "Where is my parcel CH020000129135?",
        "turn-runtime-failed-trace-1",
    )
    ai_turn_id = sent["ai_turn_id"]

...
- QA / Training Loop: `/qa-training` now consumes `/api/lite/qa-training` for real WebCall/WebChat/Email/Ticket QA samples, scorecard, coaching tasks, knowledge gap loop and explicit template block closure status. Agent appeal now writes `qa_appeal` operator tasks through `POST /api/lite/qa-training/appeals` and records TicketEvent/AdminAuditLog evidence.
...
The next PR should continue template-block parity with Persona runtime evidence query or remaining Operations governance write endpoints as the next likely cut after the QA agent appeal workflow lands.

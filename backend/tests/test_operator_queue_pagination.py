...
def test_priority_id_cursor_paginates_without_duplicates_or_gaps(db_session):
    ...
    expected = [
        row.id
        for row in db_session.query(OperatorTask).order_by(OperatorTask.priority.asc(), OperatorTask.id.desc()).all()
    ]

    actual, cursors = _collect_pages(db_session)

    assert actual == expected
    assert len(actual) == 5
    assert len(set(actual)) == 5
    assert cursors[-1] is None
    assert any(cursor for cursor in cursors[:-1])
...
def test_filters_do_not_break_pagination(db_session):
    ...
    actual, _ = _collect_pages(db_session, status="pending", source_type="webchat", task_type="handoff")

...
test('canonical MUI surfaces meet bounded accessibility truth requirements', () => {
  ...
  assert.match(workspaceConversation, /role="log"/)
  assert.match(workspaceConversation, /aria-live="off"/)
  assert.match(workspaceConversation, /role="status" aria-live="polite"/)
  assert.match(workspaceCase, /<SpineSurface label="处理进度"/)
...
test('workspace queue and conversation freshness remain visible and bounded', () => {
  assert.match(workspacePage, /visibilityState === 'visible' \? 15_000 : 60_000/)
  assert.match(workspacePage, /refetchIntervalInBackground:\s*false/)
  assert.match(workspacePage, /useOperatorRealtime/)
  assert.match(operatorRealtime, /buildApiWebSocketUrl\('\/api\/webchat\/ws'\)/)
  assert.match(workspacePage, /realtimeStatus === 'live'/)

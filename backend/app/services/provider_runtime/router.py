...
from .traffic_selection import TRAFFIC_SELECTION_SCHEMA, ProviderTrafficPath, ProviderTrafficSelection, effective_kill_switch, persisted_traffic_configuration_errors, select_provider_traffic

logger = logging.getLogger(__name__)
_DIRECT_ENV_PROVIDERS = {"private_ai_runtime"}
_TRAFFIC_CONFIGURATION_ERRORS = {"provider_runtime_traffic_mode_invalid", "provider_runtime_canary_percent_invalid", "provider_runtime_kill_switch_invalid"}
...
class ProviderRuntimeRouter:
    ...
    def _write_audit(self, request: ProviderRequest, operation: str, status: str, provider: str, elapsed_ms: int, safe_summary: dict | None, error_code: str | None = None):
        try:
            ...
            """), {"id": str(uuid.uuid4()), "tenant_id": request.tenant_id, "provider": provider, "request_id": request.request_id, "channel_key": request.channel_key, "session_id": request.session_id, "operation": operation, "status": status, "safe_summary": json.dumps(safe_summary or {}), "error_code": error_code, "elapsed_ms": elapsed_ms, "now": datetime.now(timezone.utc)})
            self.db.commit()
    ...
    def _parse_webchat_runtime_output(request: ProviderRequest, result: ProviderResult) -> dict:
        ...
        try:
            parsed_reply = parse_runtime_reply_provider_output(result.structured_output, evidence_present=request.tracking_fact_evidence_present)
            ...
            if not isinstance(reply, str) or not reply.strip():
                raise
            OutputContracts.check_security_rules(raw_output=json.dumps(parsed, ensure_ascii=False, default=str), parsed=parsed, evidence_present=True, request_body=request.body, knowledge_context=(request.metadata or {}).get("knowledge_context"))
            parsed_reply = type("_ParsedRuntimeFallback", (), {"reply": reply.strip(), "intent": parsed.get("intent") or "tracking", "tracking_number": parsed.get("tracking_number"), "handoff_required": bool(parsed.get("handoff_required", False)), "handoff_reason": parsed.get("handoff_reason"), "recommended_agent_action": parsed.get("recommended_agent_action"), "ai_decision": None})()
        ...
        OutputContracts.check_security_rules(raw_output=json.dumps(parsed, ensure_ascii=False, default=str), parsed=parsed, evidence_present=request.tracking_fact_evidence_present, request_body=request.body, knowledge_context=(request.metadata or {}).get("knowledge_context"))
        return parsed
    ...
    async def route(self, request: ProviderRequest) -> ProviderResult:
        ...
        rule = self.db.execute(statement, {"tenant_id": request.tenant_id, "channel": request.channel_key, "scenario": request.scenario}).mappings().first()
        if not rule:
            primary_provider, fallbacks, output_contract, timeout_ms, kill_switch, canary_percent = "private_ai_runtime", [], "nexus_webchat_runtime_reply_v1", 10000, False, 0
        else:
        ...
        persisted_canary_percent, persisted_kill_switch = canary_percent, kill_switch
        try:
            primary_provider, fallbacks, output_contract, timeout_ms, kill_switch, canary_percent = _apply_env_overrides(primary_provider, fallbacks, output_contract, timeout_ms, kill_switch, canary_percent)
            fallbacks = [provider for provider in fallbacks if provider in _DIRECT_ENV_PROVIDERS]
            persisted_errors = persisted_traffic_configuration_errors(canary_percent=persisted_canary_percent, kill_switch=persisted_kill_switch)
            if persisted_errors and not kill_switch:
                raise ValueError(persisted_errors[0])
            traffic = select_provider_traffic(request, canary_percent=canary_percent, kill_switch=kill_switch)
            if persisted_errors:
                traffic = replace(traffic, configuration_errors=tuple(dict.fromkeys((*traffic.configuration_errors, *persisted_errors))))
        except (RuntimeError, TypeError, ValueError) as exc:
            error_code = _traffic_configuration_error_code(exc)
            summary = {"traffic_selection": {"schema_version": TRAFFIC_SELECTION_SCHEMA, "configured_mode": "invalid", "configuration_errors": [error_code], "path": "control", "canary_percent": None, "bucket": None, "execute_candidate": False, "authoritative": False, "reason": error_code}}
            self._write_audit(request, "traffic_select", "failed", primary_provider, 0, summary, error_code)
            return _unavailable_with_traffic(error_code, None, summary=summary)
        if traffic.path == ProviderTrafficPath.KILL_SWITCH:
            self._write_audit(request, "traffic_select", "skipped", primary_provider, 0, _summary_with_traffic({}, traffic), "kill_switch_active")
            return _unavailable_with_traffic("kill_switch_active", traffic)
        if traffic.path == ProviderTrafficPath.CONTROL:
            self._write_audit(request, "traffic_select", "skipped", primary_provider, 0, _summary_with_traffic({}, traffic), "provider_canary_control_path")
            return _unavailable_with_traffic("provider_canary_control_path", traffic)
        request.output_contract, request.timeout_ms = output_contract, timeout_ms
        seen: set[str] = set()
        providers_to_try = [name for name in [primary_provider, *fallbacks] if name and not (name in seen or seen.add(name))]
        ...
        for provider_name in providers_to_try:
            ...
            if health_decision.skip:
                self._write_audit(request, operation, "skipped", provider_name, 0, _summary_with_traffic({"provider_health": health_decision.safe_summary()}, traffic), health_decision.reason or "provider_health_skip")
                continue
            adapter = ProviderRegistry.get(provider_name, self.db)
            if not adapter:
                self._write_audit(request, operation, "failed", provider_name, 0, _summary_with_traffic({}, traffic), "adapter_not_registered")
                continue
            ...
            if not result.ok:
                ...
                self._write_audit(request, operation, "failed", provider_name, result.elapsed_ms, safe_summary, result.error_code)
            ...
            try:
                ...
                if request.scenario == "webchat_runtime_reply" and output_contract == "nexus_webchat_runtime_reply_v1":
                    ...
                    parsed = OutputContracts.validate_and_parse(output_contract, json.dumps(result.structured_output), request.tracking_fact_evidence_present, (request.metadata or {}).get("persona_context"), request.body, (request.metadata or {}).get("knowledge_context"))
                ...
                if not shadow_only:
                    ...
                    if health_event:
                        safe_summary["provider_health"] = health_event
                result.raw_payload_safe_summary = safe_summary
                self._write_audit(request, operation, "ok", provider_name, result.elapsed_ms, safe_summary)
                if shadow_only:
                    return _unavailable_with_traffic("provider_shadow_completed", traffic, elapsed_ms=result.elapsed_ms, summary={"shadow_provider": provider_name, "shadow_result": "valid"})
                return result
            except Exception as exc:
                ...
                if not shadow_only:
                    ...
                    if health_event:
                        safe_summary["provider_health"] = health_event
                self._write_audit(request, "shadow_parse_reject" if shadow_only else "parse_reject", "failed", provider_name, result.elapsed_ms, safe_summary, "parse_reject")
        error_code = "provider_shadow_failed" if shadow_only else "all_providers_failed"
        self._write_audit(request, operation, "failed", "router", last_elapsed_ms, _summary_with_traffic({}, traffic), error_code)
...
def _traffic_configuration_error_code(exc: BaseException) -> str:
    value = str(exc).strip()
    return value if value in _TRAFFIC_CONFIGURATION_ERRORS else "provider_runtime_traffic_configuration_invalid"
...
def _unavailable_with_traffic(error_code: str, traffic: ProviderTrafficSelection | None, *, elapsed_ms: int = 0, summary: dict | None = None) -> ProviderResult:
...
def _apply_env_overrides(primary_provider: str, fallbacks: list[str], output_contract: str, timeout_ms: int, kill_switch: bool, canary_percent):

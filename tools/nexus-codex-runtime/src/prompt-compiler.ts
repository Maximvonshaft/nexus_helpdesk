...
function formatPersonaContext(value: Record<string, unknown> | null | undefined): string {
  ...
  const key = typeof value.profile_key === "string" ? value.profile_key : "";
  const name = typeof value.name === "string" ? value.name : "";
  const summary = typeof value.summary === "string" ? value.summary : "";
  const content = isRecord(value.content_json) ? JSON.stringify(value.content_json) : "";
  return truncate([key || name, summary, content].filter(Boolean).join(" | "), 260);
}
...
function formatKnowledgeContext(value: Record<string, unknown> | null | undefined): string {
  ...
  return value.hits
    ...
    .map((hit, index) => {
      ...
      const title = typeof hit.title === "string" ? hit.title : `Knowledge ${index + 1}`;
      const text = typeof hit.text === "string" ? hit.text.replace(/\s+/g, " ").trim() : "";

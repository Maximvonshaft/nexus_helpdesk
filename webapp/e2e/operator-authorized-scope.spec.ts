...
test('normal operators enter the canonical shell through a server-authorized scope', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/admin/operator-queue/my-scopes') return json(route, { items: [{ tenant_key: 'tenant-authorized', tenant_hash: '123456789abc', country_code: 'CH', channel_key: 'webchat' }] })
...
test('switching among multiple authorized scopes remounts the workspace with the selected grant', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    const url = new URL(route.request().url())
    if (url.pathname === '/api/auth/me') return json(route, authUser)
    if (url.pathname === '/api/admin/operator-queue/my-scopes') return json(route, { items: [
      { tenant_key: 'tenant-ch', tenant_hash: 'aaaaaaaaaaaa', country_code: 'CH', channel_key: 'webchat' },
      { tenant_key: 'tenant-me', tenant_hash: 'bbbbbbbbbbbb', country_code: 'ME', channel_key: 'whatsapp' },
    ] })
    if (url.pathname === '/api/admin/operator-queue/unified') {

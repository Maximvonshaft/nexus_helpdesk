...
from app.models import AdminAuditLog, Market, Team, Tenant, Ticket, User
...
def _tenant(db, key: str = TENANT) -> Tenant:
    ...
    existing = db.query(Tenant).filter(Tenant.tenant_key == normalized).first()
...
def _ownership(tenant: Tenant, source: str = "fixture") -> dict[str, object]:
    return {
        "tenant_id": tenant.id,
        "tenant_assignment_source": source,
        "tenant_assignment_version": "unified-operator-queue-test-v1",
    }
...
def _team(
    ...
    *,
    ...
    sequence = db.query(Market).count() + 1
    market = Market(
        code=f"M-{country}-{sequence}",
        name=f"Market {country} {sequence}",
...
def _user(
    ...
    *,
    ...
    if team_id is not None:
        ...
        assert team is not None and team.tenant_id is not None
...
def _dispatch(
    ...
    *,
    ...
    if ticket is not None:
        ...
        tenant = _tenant(db, tenant_key or TENANT)
...
def test_response_is_bounded_and_contains_no_customer_or_provider_secrets(db_session):
    ...
    db_session.commit()
    serialized = UnifiedOperatorQueueResponse.model_validate(
    ...
    assert next(
        item
        for item in _list(db_session, agent)["items"]
        if item["source_type"] == "handoff"
...
def test_forged_tenant_scope_is_denied_even_with_other_grant(db_session):
    _, agent, *_ = _seed_all(db_session)
    _tenant(db_session, "tenant-forged")
    with pytest.raises(HTTPException) as exc:
...
def test_grant_does_not_expand_ticket_team_visibility(db_session):
    ...
    other_market = Market(
        code="M-OTHER",
        name="Other same-country market",
        country_code=COUNTRY,
        is_active=True,
        **_ownership(tenant),
    )
...
def test_cross_tenant_ticket_provenance_fails_closed(db_session):
    ...
    _tenant(db_session, "tenant-conflict")
    _conversation(
        db_session,
        ticket=ticket,
...
def test_orphan_dispatch_remains_visible_only_in_its_exact_scope(db_session):
    _admin, agent, *_ = _seed_all(db_session)
    orphan = _dispatch(
        db_session,
        ticket=None,
    ...
    assert next(
        item for item in result["items"] if item["source_id"] == orphan.id
    )["case_key"] is None
...
def test_filters_cover_state_source_owner_priority_sla_and_retry(db_session):
    ...
    assert {
        item["source_type"]
        for item in _list(db_session, agent, source_type="handoff")["items"]
    } == {"handoff"}
    assert all(
        item["state"] == "active"
        for item in _list(db_session, agent, state="active")["items"]
    )
    assert all(
        item["priority"] == "high"
        for item in _list(db_session, agent, priority="high")["items"]
    )
    assert {
        item["source_type"]
        for item in _list(db_session, agent, owner="mine")["items"]
    } == {"handoff", "ticket", "dispatch"}
    assert all(
        item["sla"]["state"] == "at_risk"
        for item in _list(db_session, agent, sla="at_risk")["items"]
    )
...
def test_stale_and_reopened_states_are_explicit(db_session):
    ...
    handoff = db_session.query(WebchatHandoffRequest).filter(
        WebchatHandoffRequest.ticket_id == ticket.id
    ).one()
    handoff.updated_at = NOW - timedelta(days=2)
    dispatch = db_session.query(OperationsDispatchOutboxRecord).filter(
        OperationsDispatchOutboxRecord.ticket_id == ticket.id
    ).one()
...
def test_stable_cursor_has_no_duplicates_and_is_bound_to_actor_filters_and_grant(db_session):
    ...
    assert {item["queue_id"] for item in page1["items"]}.isdisjoint(
        {item["queue_id"] for item in page2["items"]}
    )
    other = _user(
        db_session,
        username="other-cursor",
        role=UserRole.agent,
    ...
    new_team = Team(
        name="Cursor team",
        market_id=same_country_market.id,
        is_active=True,
        **_ownership(tenant),
    )
    ...
    grant = db_session.query(OperatorQueueScopeGrant).filter(
        OperatorQueueScopeGrant.user_id == agent.id
    ).one()
    grant.enabled = False
...
def test_cursor_continues_across_more_than_one_source_fetch_window(db_session, sort):
    ...
    admin = _user(
        db_session,
        username=f"paging-admin-{sort}",
        role=UserRole.admin,
    )
    agent = _user(
        db_session,
        username=f"paging-agent-{sort}",
        role=UserRole.agent,
        team_id=team.id,
    )
    _grant(db_session, admin=admin, user=agent)
...
def test_many_same_scope_conversations_cannot_monopolize_ticket_page(db_session):
    ...
    admin = _user(
        db_session,
        username="conversation-admin",
        role=UserRole.admin,
    )
    agent = _user(
        db_session,
        username="conversation-agent",
        role=UserRole.agent,
        team_id=team.id,
    )
    _grant(db_session, admin=admin, user=agent)
...
def test_ticket_with_exact_outbox_provenance_but_no_webchat_is_included(db_session):
    ...
    no_provenance = _ticket(
        db_session,
        suffix="NO-PROVENANCE",
        team_id=team.id,
        created_at=NOW + timedelta(seconds=1),
    )
    db_session.commit()
    ...
    assert no_provenance.id not in {
...
def test_post_snapshot_outbox_scope_change_cannot_admit_ticket_on_later_page(db_session):
    ...
    admin = _user(
        db_session,
        username="snapshot-outbox-admin",
        role=UserRole.admin,
    )
    agent = _user(
        db_session,
        username="snapshot-outbox-agent",
        role=UserRole.agent,
        team_id=team.id,
    )
    _grant(db_session, admin=admin, user=agent)
    ...
    other_tenant = _tenant(db_session, "tenant-before-snapshot")
    poisoned_provenance = _dispatch(
        db_session,
        ticket=hidden,
        created_at=NOW - timedelta(seconds=4),
    )
    poisoned_provenance.tenant_key = other_tenant.tenant_key
    last = _ticket(
        db_session,
...
def test_poisoned_legacy_ticket_status_is_bounded_to_unknown(db_session):
    ...
    admin = _user(
        db_session,
        username="poisoned-status-admin",
        role=UserRole.admin,
    )
    agent = _user(
        db_session,
        username="poisoned-status-agent",
        role=UserRole.agent,
        team_id=team.id,
    )
    _grant(db_session, admin=admin, user=agent)
...
def test_source_links_only_reference_existing_safe_routes(db_session):
    _, agent, *_ = _seed_all(db_session)
    by_source = {
        item["source_type"]: item for item in _list(db_session, agent)["items"]
    }
    assert by_source["ticket"]["source_links"]["ticket"].startswith(
        "/api/tickets/"
    )
    assert by_source["handoff"]["source_links"]["conversation"].startswith(
        "/api/webchat/admin/tickets/"
    )
    assert (
        by_source["handoff"]["source_links"]["handoff"]
...
def test_scope_grant_crud_is_normalized_hashed_and_audited(db_session):
    normalized_tenant = "tenant.example"
    _, team = _team(db_session, tenant_key=normalized_tenant)
    admin = _user(
        db_session,
    ...
    audit = db_session.query(AdminAuditLog).filter(
        AdminAuditLog.target_id == grant.id
    ).one()
    ...
    assert db_session.query(AdminAuditLog).filter(
        AdminAuditLog.action == "operator_queue.scope_grant.deleted"
    ).count() == 1
...
def test_grant_schema_forbids_unknown_fields_and_wildcards(db_session):
    ...
    _, team = _team(db_session)
    _user(db_session, username="wild-admin", role=UserRole.admin)
    agent = _user(
        db_session,
        username="wild-agent",
        role=UserRole.agent,
...
def test_legacy_projection_table_is_not_used_as_unified_source(db_session):
    ...
    assert SENSITIVE_SENTINEL not in UnifiedOperatorQueueResponse.model_validate(
        result
    ).model_dump_json()

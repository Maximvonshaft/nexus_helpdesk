...
_SAFE_EXACT_KEYS = {
    ...
    "fallback_group_id_present", "present", "redacted", "type", "category", "count", "size", "length",
    "item_count",
}
...
def sanitize_audit_payload(value: Any, *, limits: AuditSanitizerLimits = DEFAULT_LIMITS) -> Any:
    """Return deterministic, bounded, JSON-safe audit data without mutating input."""
...
def _sanitize_mapping(value: Mapping[Any, Any], *, depth: int, seen: MutableSet[int], limits: AuditSanitizerLimits) -> dict[str, Any]:
    ...
    try:
        ...
        for safe_key, source_key, raw_value in items:
            ...
            result[output_key] = (
                _marker(raw_value, category=_sensitive_category(source_key))
                if _is_sensitive_key(source_key)
                else _sanitize(raw_value, key=source_key, depth=depth + 1, seen=seen, limits=limits)
            )
...
def _sanitize_text(value: str, *, key: str, limits: AuditSanitizerLimits) -> str | dict[str, Any]:
    ...
    if len(text) > limits.max_string_length:
        ...
        text = f"{text[: max(0, limits.max_string_length - 30)]}...[truncated:{digest}]"
    return text

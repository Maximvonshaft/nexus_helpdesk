...
def assert_demo_has_no_retired_markers(base_url: str, out_dir: Path, errors: list[str]) -> dict[str, Any]:
    ...
    return {"status": demo.status, "elapsed_ms": demo.elapsed_ms, "ok": 200 <= demo.status < 300, "retired_markers": found}
...
def release_metadata_complete(
    ...
    *,
    ...
    ok = True
    if expected_git_sha:
        actual_git_sha = str(health_payload.get("git_sha") or health_payload.get("commit_sha") or "")
        if actual_git_sha != expected_git_sha:
            errors.append(f"release_git_sha_mismatch expected={expected_git_sha} actual={actual_git_sha or '[missing]'}")
            ok = False
    if expected_image_tag:
        actual_image_tag = str(health_payload.get("image_tag") or health_payload.get("release_image_tag") or "")
        if actual_image_tag != expected_image_tag:
            errors.append(f"release_image_tag_mismatch expected={expected_image_tag} actual={actual_image_tag or '[missing]'}")
            ok = False
    return ok
...
def main() -> int:
    ...
    parser.add_argument("--origin", default=os.getenv("WEBCHAT_SMOKE_ORIGIN", os.getenv("ORIGIN", "https://www.leakle.com")))
    ...
    try:
        health = request(urljoin(base_url, "/healthz"), timeout=20, allow_http_error=True)
        ...
        ready = request(urljoin(base_url, "/readyz"), timeout=20, allow_http_error=True)
        ...
        summary["webchat_demo"] = assert_demo_has_no_retired_markers(base_url, out_dir, errors)
        ...
        send_json = send.json()
        summary["reply_starts_json"] = isinstance(send_json, dict) and (send_json.get("ok") is True or bool(send_json.get("ai_pending")))
        ...
        if not reply:
            summary.update({"ok": args.allow_pending, "conversation_id": conversation_id, "pending": True, "last_poll": last_poll})
        ...
        if reply_source and not is_private_ai_runtime_source(reply_source):
            errors.append(f"webchat_reply_source={reply_source}")

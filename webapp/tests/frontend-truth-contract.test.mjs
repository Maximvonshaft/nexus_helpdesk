...
test('workspace queue and selected conversation freshness remain visible and bounded', () => {
  ...
  assert.match(workspace, /history_expanded/)
  assert.match(workspace, /EVENT_IDLE_POLL_MS/)
  assert.match(workspace, /EVENT_RETRY_MAX_MS/)
  assert.match(workspaceApi, /before_message_id/)
  assert.match(workspaceApi, /WebchatEventSummary/)
  assert.match(workspaceApi, /\/api\/webchat\/admin\/tickets\/\$\{ticketId\}\/events/)
  assert.match(workspaceApi, /wait_ms=0/)
  assert.doesNotMatch(workspaceApi, /wait_ms=10000|thread-v2|thread-page/)

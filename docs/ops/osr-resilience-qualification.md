...
| --- | --- | --- |
| Concurrent queue claim | 24 synthetic `BackgroundJob` rows, 4 independent PostgreSQL sessions, 6 claims per worker | 24/24 rows claimed once; no duplicate row ID; all 4 worker identities represented |
| Active dedupe enqueue | 8 concurrent transactions enqueue one active dedupe key | exactly 1 active durable row; all callers resolve the same row ID |
| Expired worker lease recovery | 1 synthetic `processing` row with a lock older than `JOB_LOCK_SECONDS` | the same row is reclaimed by the recovery worker; no duplicate durable row |

The tests execute the production `enqueue_background_job` and `claim_pending_jobs` boundaries against a disposable PostgreSQL 16 database with pgvector, after the repository's exact Alembic head is applied.
...
2. the three required scenario test names are present in JUnit evidence;
...
Raw JUnit output is deleted before upload. Test case names, payloads, customer identifiers and Provider material are not emitted in the aggregate report.

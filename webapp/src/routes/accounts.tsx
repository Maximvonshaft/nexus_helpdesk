...
function AccountsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (selectedId) return api.updateChannelAccount(selectedId, payload)
      return api.createChannelAccount(payload)
    },
  ...
  return (
    <AppShell>
      <PageHeader
        eyebrow="发送线路"
        title="渠道账号与备用线路"
        description="把不同市场、不同渠道、不同备用线路配置清楚，客服回复才能稳定发出去。"
      ...
      {!permitted ? (
        ...
        <>
          ...
          <div className="page-grid split-grid-wide">
                ...
                <div className="list">
                  {filteredAccounts.map((account) => (
                    <button key={account.id} className={`queue-card ${selectedId === account.id ? 'selected' : ''}`} onClick={() => setSelectedId(account.id)}>
                      ...
                      <div className="queue-card-meta">市场 ID：{account.market_id ?? '—'} · 优先级：{account.priority}</div>
                      <div className="queue-card-meta">备用账号：{sanitizeDisplayText(account.fallback_account_id)} · 更新时间：{formatDateTime(account.updated_at)}</div>
                    </button>
                  ))}
            ...
            <Card>
              <CardHeader title={selectedId ? '编辑发送线路' : '新建发送线路'} subtitle="给运营和客服配置清晰、稳定、可兜底的发送线路。" />
              <CardBody>
                <div className="stack">

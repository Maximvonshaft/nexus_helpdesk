...
- Control Tower / Governance: `/control-tower` now consumes `/api/lite/control-tower` for KPI/tower, manager action queue, team workload, channel health, bulletin impact, RBAC/governance lanes and template closure status. Remaining work is other visible Operations/Engineering handoff template screens and governance write endpoints.
...
The next PR should continue template-block parity with remaining Operations governance write endpoints, broader visual parity polish, or Persona runtime analytics after the QA knowledge-gap workflow lands.

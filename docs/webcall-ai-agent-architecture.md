...
PR-0/PR-1 does not make WebCall AI functional yet. PR-0/PR-2 does not make WebCall AI functional yet. PR-3 does not make WebCall AI functional yet. PR-4 does not implement functional AI voice. PR-5 does not implement functional AI voice. PR-6 does not implement functional AI voice. PR-7 does not implement functional AI voice. PR-8 does not implement functional AI voice. PR-9 does not implement functional AI voice. Acceleration Pack A does not implement functional AI voice. Acceleration Pack B does not implement full AI voice. Acceleration Pack C does not implement full AI voice. Acceleration Pack D does not implement full production AI voice. The final pilot closure does not enable full production AI voice by default. These PRs only add the guarded architecture, schema, config, tests, no-op worker claim lifecycle, deterministic mock turn persistence, deterministic mock STT/TTS boundaries, the real STT/TTS provider contract skeleton, the first Deepgram STT adapter behind feature flags, a controlled static HTTPS audio reference source for STT input, a fake LiveKit AI participant ownership skeleton, a server-side LiveKit AI participant token issuer wrapper, a backend no-media AI presence runtime, controlled audio ingress plus STT transcript persistence, deterministic text-only tracking orchestration with read-only tracking fact lookup, a governed TTS runtime plus voice egress boundary, and a backend-only pilot closure smoke path. No WebRTC audio subscription by default, real AI audio publish path, LLM/runtime integration, or Speedaf write automation is introduced here.
...
Acceleration Pack B adds controlled audio ingress plus STT runtime and transcript persistence only. It is disabled by default with `WEBCALL_AI_STT_RUNTIME_ENABLED=false`, supports `WEBCALL_AI_STT_RUNTIME_MODE=mock_text | audio_reference`, and writes `webchat_voice_transcript_segments` only when `WEBCALL_AI_STT_TRANSCRIPT_WRITE_ENABLED=true`. The worker can resolve a controlled static audio reference, run the configured STT provider, persist a redacted final transcript segment idempotently, use the redacted STT text in the existing mock AI turn, and release safely. Acceleration Pack B does not implement full AI voice. It does not query Speedaf, generate or publish AI audio, change frontend, call LLM/provider runtime/ExternalChannel/OpenAI/Codex, execute Speedaf writes, persist tokens, log tokens, or expose AI participant tokens/transcripts to browsers.

Acceleration Pack C adds deterministic WebCall AI voice orchestration and read-only tracking fact lookup only. It is disabled by default with `WEBCALL_AI_ORCHESTRATOR_ENABLED=false`, uses `WEBCALL_AI_ORCHESTRATOR_MODE=deterministic_tracking`, and calls tracking facts only when `WEBCALL_AI_TRACKING_LOOKUP_ENABLED=true`. It can classify a safe tracking intent, ask for a missing tracking number, request last-four suffix selection for multiple candidates, or explain a trusted redacted tracking fact. It must hand off cancel, address change, refund, compensation, driver/DSP, legal, privacy, and complaint cases. Acceleration Pack C does not implement full AI voice. It does not generate or publish AI audio, change frontend, call LLM/provider runtime/ExternalChannel/OpenAI/Codex, execute Speedaf writes, create work orders, cancel orders, update addresses, promise compensation/refunds/delivery times, contact drivers/DSPs, persist tokens, log tokens, or expose AI participant tokens/transcripts/tracking facts to browsers.

Acceleration Pack D adds a governed TTS runtime and voice egress boundary only. It is disabled by default with `WEBCALL_AI_TTS_RUNTIME_ENABLED=false` and `WEBCALL_AI_VOICE_EGRESS_ENABLED=false`, supports `WEBCALL_AI_TTS_RUNTIME_MODE=mock_audio_reference | provider_audio_reference`, and supports `WEBCALL_AI_VOICE_EGRESS_MODE=fake_audio_reference | livekit_audio_publish_stub`. It can turn an already-redacted AI reply into a governed audio reference and pass that reference to a fake egress client or an injected LiveKit publish stub. Acceleration Pack D does not implement full production AI voice. It does not enable real audio publish by default, change frontend, call LLM/provider runtime/ExternalChannel/OpenAI/Codex, execute Speedaf writes, create work orders, cancel orders, update addresses, persist tokens, log tokens, expose tokens to browsers, or persist raw audio bytes.

The final pilot closure adds a backend-only closure smoke path. It is disabled by default with `WEBCALL_AI_PILOT_CLOSURE_ENABLED=false`, blocked by default with `WEBCALL_AI_PILOT_KILL_SWITCH=true`, rejected in production, and requires explicit session/tenant allowlist or deterministic non-production canary. It can run a simulated full loop with fake tracking, a handoff safety smoke with no voice egress, and a sanitized evidence report. It does not change frontend, execute Speedaf writes, create work orders, cancel orders, update addresses, call LLM/provider runtime/ExternalChannel/OpenAI/Codex, persist tokens, log tokens, expose tokens to browsers, persist raw audio bytes, or enable production traffic by default.
...
Browser code must never receive AI provider tokens, ExternalChannel tokens, Speedaf appCode, Speedaf secretKey, signing material, full phone numbers, full addresses, or raw Speedaf payloads.
...
4. PR-4: deterministic mock STT/TTS boundaries only, with no audio, STT, TTS, LLM, external_channel, or Speedaf calls to real providers.
...
12. Acceleration Pack C: deterministic text-only tracking orchestration with read-only tracking fact lookup, disabled by default and rejected in production.
13. TTS provider integration behind feature flags and canary controls.
14. Human handoff workflows and operator evidence.
15. Summary, callback, and evidence hardening.

## Non-Goals

This foundation does not implement full AI voice. It does not implement real TTS. It also does not implement production LiveKit audio subscription by default, real ExternalChannel/LLM voice calls, frontend WebCall UI changes, AI handoff UI, or any Speedaf write action execution from AI.

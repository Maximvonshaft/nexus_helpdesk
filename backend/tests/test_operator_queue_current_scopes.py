...
os.environ.setdefault(
    "DATABASE_URL",
    "sqlite:////tmp/operator_queue_current_scopes.db",
)
...
from app.models import Market, Team, Tenant, User
...
def _ownership(tenant: Tenant) -> dict[str, object]:
    return {
        ...
        "tenant_assignment_version": "operator-current-scopes-test-v1",
    }
...
def test_current_user_receives_only_own_active_grant_scopes(db_session):
    tenant = _tenant(db_session, "tenant-me")
    me_team = _team(db_session, tenant=tenant, code="ME", country="ME")
    admin = _user(
        db_session,
        username="admin",
        role=UserRole.admin,
    ...
    assert [
        (item.tenant_key, item.country_code, item.channel_key)
        for item in validated.items
    ] == [
        ("tenant-me", "CH", "webchat"),
        ("tenant-me", "ME", "webchat"),
    ]
    serialized = validated.model_dump_json()
    assert '"country_code":"DE"' not in serialized
    assert '"country_code":"FR"' not in serialized
...
def test_manager_receives_each_active_scope_owned_by_that_manager(db_session):
    ...
    result = OperatorQueueCurrentScopesResponse.model_validate(
        get_current_operator_queue_scopes(
            db=db_session,
    ...
    assert [
        (item.country_code, item.channel_key, item.tenant_key)
        for item in result.items
    ] == [
        ("CH", "webchat", "tenant-a"),
        ("ME", "whatsapp", "tenant-a"),
    ]


def test_admin_without_explicit_scope_is_not_given_a_guessed_tenant(db_session):
    tenant = _tenant(db_session, "tenant-admin-empty")
    admin = _user(
        db_session,
        username="admin",
        role=UserRole.admin,
...
def test_admin_explicit_scope_can_drive_the_same_selector_without_broad_inventory(
    db_session,
):
    tenant = _tenant(db_session, "tenant-admin")
    grantor = _user(
        db_session,
        username="grantor",
        role=UserRole.admin,

...
from app.models import Tenant, Ticket, TicketEvent
...
TENANT_KEY = "snapshot-test-tenant"
...
def db_session():
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "CREATE UNIQUE INDEX ux_tickets_source_dedupe_key "
                "ON tickets(source_dedupe_key)"
            )
        )
        conn.execute(
            text(
                ...
                "ON webchat_messages(conversation_id, client_message_id) "
    ...
    db.add(
        Tenant(
            ...
            display_name="Snapshot Test Tenant",
            is_active=True,
        )
    )
    db.flush()
...
def test_create_ticket_from_webchat_snapshot_sets_source_dedupe_key(db_session):
    ...
    assert ticket.source_dedupe_key == svc.webchat_handoff_source_dedupe_key(
        snapshot
    )
    assert ticket.tenant_id == db_session.query(Tenant.id).scalar()
    assert db_session.query(Ticket).count() == 1
    assert db_session.query(TicketEvent).count() == 1


def test_create_ticket_from_webchat_snapshot_returns_existing_after_unique_conflict(
    db_session,
    monkeypatch,
):
    snapshot = _snapshot()
    existing = svc.create_ticket_from_webchat_snapshot(
        db_session,
        snapshot=snapshot,
    )
    ...
    returned = svc.create_ticket_from_webchat_snapshot(
        db_session,
        snapshot=snapshot,
    )
...
def test_existing_ticket_message_linkage_returns_existing_after_unique_conflict(
    db_session,
    monkeypatch,
):
    snapshot = _snapshot()
    ...
    messages = db_session.execute(
        select(WebchatMessage).order_by(WebchatMessage.id.asc())
    ).scalars().all()

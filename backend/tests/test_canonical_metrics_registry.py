...
CONTROLLED_RUNBOOK = (
    REPO_ROOT / "docs" / "releases" / "controlled-candidate-convergence-runbook.md"
)
...
def test_controlled_runtime_uses_one_shared_prometheus_registry() -> None:
    ...
    mounted = {
        ...
        if "prometheus-multiproc:/var/run/nexus-prometheus"
        in (service.get("volumes") or [])
    }
    ...
    metrics_enabled = {
        ...
        and "METRICS_ENABLED" in service["environment"]
    }
    assert {
        name for name, value in metrics_enabled.items() if value == "true"
    } == {"app-controlled"}
    ...
    assert services["app-controlled"]["environment"]["METRICS_TOKEN"] == (
        "${METRICS_TOKEN:?set dedicated metrics token}"
    )
    assert services["app-controlled"]["environment"][
        "PROMETHEUS_MULTIPROC_DIR"
    ] == "/var/run/nexus-prometheus"
    assert services["migrate-controlled"]["environment"][
        "PROMETHEUS_MULTIPROC_DIR"
    ] == ""
    assert services["livekit-agent-controlled"]["environment"][
        "PROMETHEUS_MULTIPROC_DIR"
    ] == ""
    ...
    assert "worker-background-controlled" in runbook
    assert "worker-metrics" not in services
    assert "pushgateway" not in CONTROLLED_COMPOSE.read_text(
        encoding="utf-8"
    ).lower()
...
def test_observability_owns_multiprocess_collection_and_container_safe_identity() -> None:
    ...
    assert (
        "values.MultiProcessValue(process_identifier=prometheus_process_identifier)"
        in app_init
    )
...
def test_app_and_worker_lifecycles_clean_live_metric_files() -> None:
    ...
    assert app_command[app_command.index("--config") + 1] == (
        "/app/backend/gunicorn.conf.py"
    )
...
def test_prometheus_multiprocess_scrape_combines_independent_container_namespaces(
    tmp_path: Path,
) -> None:
    script = """

...
def test_security_remediation_removes_sensitive_exception_and_log_flows() -> None:
    provider = (ROOT / "backend/app/services/provider_runtime_status.py").read_text(
        encoding="utf-8"
    )
    support = (ROOT / "backend/app/services/support_intelligence_service.py").read_text(
        encoding="utf-8"
    )
    audit = (
        ROOT / "backend/app/services/webchat_ai_decision_runtime/audit.py"
    ).read_text(encoding="utf-8")
    readiness = (ROOT / "backend/scripts/validate_production_readiness.py").read_text(
        encoding="utf-8"
    )
    logs = (ROOT / "backend/app/services/webchat_message_service.py").read_text(
        encoding="utf-8"
    )
...
def test_widget_avatar_is_same_origin_and_not_configurable() -> None:
    ...
    demo = (ROOT / "backend/app/static/webchat/demo/index.html").read_text(
        encoding="utf-8"
    )
...
def test_codeql_exception_policy_has_no_stale_exceptions() -> None:
    payload = json.loads(
        (ROOT / "config/security/codeql-exceptions.json").read_text(
            encoding="utf-8"
        )
    )

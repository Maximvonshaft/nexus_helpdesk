...
os.environ.setdefault(
    "DATABASE_URL",
    "sqlite:////tmp/webchat_handoff_control_tests.db",
)
...
from app.services.webchat_ai_turn_service import (  # noqa: E402
...
from app.services.webchat_runtime_ai_service import (  # noqa: E402
    WebchatRuntimeReplyResult,
)
from app.services.webchat_service import (  # noqa: E402
    add_visitor_message,
    admin_reply,
)
...
def make_webchat(
    ...
    customer = Customer(
        name="Handoff Visitor",
        external_ref="handoff-visitor",
    )
    db.add(customer)
    db.flush()
    ticket = Ticket(
...
def make_whatsapp_webchat(
    ...
    ticket = Ticket(
        ...
        description="canonical WhatsApp synthetic inbound",
        customer_id=customer.id,
        source=TicketSource.user_message,
    ...
    db.flush()
    conversation = WebchatConversation(
        public_id=f"wa_canonical_{ticket.id}",
        visitor_token_hash="token-hash",
        tenant_key="pytest",
...
def test_admin_reply_to_whatsapp_inbox_queues_canonical_outbound(db_session):
    ...
    agent_message = (
        ...
        .filter(WebchatMessage.id == reply["message"]["id"])
        .one()
    )
...
def test_worker_processes_whatsapp_admin_reply_with_canonical_adapter(
    db_session,
    monkeypatch,
):
    agent = make_user(
        db_session,
        "whatsapp_worker_agent",
        UserRole.manager,
    )
    ticket, conversation, message, _account = make_whatsapp_webchat(db_session)
    _accept_handoff(
    ...
    def fake_canonical(db, *, message, ticket, idempotency_key):
        ...
        return MessageStatus.sent, "whatsapp_test_sent", None, {
            "adapter": "whatsapp",
    ...
    monkeypatch.setattr(
        message_dispatch,
        "dispatch_whatsapp_outbound",
        fake_canonical,
    )

    processed = message_dispatch.process_outbound_message(
        db_session,
        outbound,
    )

    assert processed.status == MessageStatus.sent
    assert processed.provider_status == "whatsapp_test_sent"
...
def test_whatsapp_ai_reply_queues_canonical_outbound(db_session, monkeypatch):
    ticket, conversation, message, _account = make_whatsapp_webchat(db_session)
...
def test_decline_accept_release_and_resume_ai_state_machine(db_session):
    ...
    assert (
        ...
        .filter(
            ...
            WebchatHandoffDecision.actor_id == admin.id,
        )
        .count()
        == 1
    )
...
def test_stale_requested_handoff_auto_resumes_on_public_visitor_message(
    db_session,
):
    ticket, conversation, message = make_webchat(db_session)
    ...
    request.requested_at = stale_at
    request.created_at = stale_at
    request.updated_at = stale_at
    ...
    http_request = Request(
        {
            ...
            "method": "POST",
            "path": (
                f"/api/webchat/conversations/{conversation.public_id}/messages"
            ),
    ...
    result = add_visitor_message(
        ...
        "你好",
        http_request,
        client_message_id="stale-handoff-message",
    )
    db_session.flush()

    visitor_message_id = result["message"]["id"]
    assert request.status == "resumed_ai"
    assert conversation.current_handoff_request_id is None
    assert conversation.handoff_status == "none"
    assert conversation.ai_suspended is False
    assert ticket.conversation_state == ConversationState.ai_active
    assert (
        ...
        .filter(
            ...
            == f"webchat-ai-reply:{visitor_message_id}"
        )
        .count()
        == 0
    )
...
def test_force_takeover_blocks_new_ai_turns_and_agent_reply_is_audited(
    db_session,
):
    ...
    def fail_create_job(*args, **kwargs):
        raise AssertionError(
            "handoff-owned conversation must not create AI jobs"
        )
...
def test_supervisor_roles_can_force_takeover_ai_active_session(
    db_session,
    role,
):
...
def test_direct_reply_to_ai_active_session_requires_force_takeover_first(
    db_session,
):

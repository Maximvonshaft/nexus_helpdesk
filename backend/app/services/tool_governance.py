...
TEXT_KEY_RE = re.compile(
    r"(text|body|message|content|reply|description|summary|transcript)",
    re.IGNORECASE,
)
SAFE_STATUS_KEYS = {
    "ok",
...
_PLATFORM_EXTERNAL_SEND_TOOLS = {"messages_send"}
...
def _enforcement_mode() -> str:
    ...
    return mode if mode in VALID_ENFORCEMENT_MODES else "audit_only"
...
def classify_tool_type(tool_name: str) -> str:
    ...
    if normalized in _PLATFORM_EXTERNAL_SEND_TOOLS or lowered.endswith(".send") or lowered.endswith("_send"):
        return "external_send"
    if normalized in _PLATFORM_READ_TOOLS:
        return "read_only"
    return "read_only"
...
def evaluate_tool_call_policy(
    *,
    ...
    reason = (
        "write_or_external_send_allowed_with_capability"
        if capability_ok
        else "write_or_external_send_requires_capability"
    )
...
def _summarize_scalar(key: str | None, value: Any) -> Any:
    ...
    if len(cleaned) <= 24 and re.fullmatch(r"[A-Za-z0-9_.:@/-]+", cleaned):
        return {
            "value_preview": cleaned[:4] + "…" if len(cleaned) > 4 else cleaned,
            "length": len(cleaned),
...
def _summarize_payload(value: Any, *, key: str | None = None, depth: int = 0) -> Any:
    ...
    if isinstance(value, Mapping):
        result: dict[str, Any] = {"_type": "object", "_keys": sorted(str(item) for item in value.keys())[:30]}
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        ...
        return {
            "_type": "array",
            "_count_visible": len(items),
            "items": [_summarize_payload(item, key=key, depth=depth + 1) for item in items],
        }
    return _summarize_scalar(key, value)


def summarize_input_safe(payload: Any) -> str:
    return _stable_json(_summarize_payload(payload))[:MAX_SUMMARY_CHARS]
...
def _insert_tool_call_audit(
    ...
    *,
    ...
    get_or_create_tool_registry_entry(
        ...
        description=(
            json.dumps(policy_decision.as_payload(), ensure_ascii=False)
            if policy_decision
            else None
        ),
    )
    ...
    row = ToolCallLog(
        ...
        input_summary=(
            summarize_input_safe(
                {"payload": input_payload, "policy": policy_decision.as_payload() if policy_decision else None}
            )
            if input_payload is not None or policy_decision
            else None
        ),
        ...
        error_message=redact_sensitive_error_text(
            error_message,
            error_code=resolved_error_code,
...
def record_tool_call(
    *,
    ...
    """Record a bounded audit without breaking the primary Tool operation."""
    ...
    except Exception as exc:  # pragma: no cover - audit must not break runtime
        if owns_session:
            try:
                session.rollback()
            except Exception:
        ...
        LOGGER.warning(
            ...
            extra={
                "event_payload": {
                    ...
                    "provider": provider,
                    "status": safe_status,
                    "error": redact_sensitive_error_text(
                        str(exc),

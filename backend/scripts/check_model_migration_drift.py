#!/usr/bin/env python3
"""Check that registered SQLAlchemy metadata matches a migrated PostgreSQL schema.

Run only after ``alembic upgrade head`` against a disposable PostgreSQL
database.  The gate checks complete model registration plus high-signal schema
contracts: tables, columns, named unique constraints, and selected lifecycle
constraints/indexes whose absence would weaken OSR safety.
"""
...
def metadata_registration_drift() -> list[Drift]:
    """Prove imported module families registered representative tables."""
    ...
    for table_name in sorted(metadata_tables):
        ...
        if required_checks:
            ...
            for constraint_name in sorted(required_checks - metadata_checks):
                drift.append(Drift("missing_metadata_check", f"{table_name}.{constraint_name}", "required check constraint is not registered in ORM metadata"))
        ...
        if required_indexes:
            ...
            for index_name in sorted(required_indexes - metadata_indexes):
                drift.append(Drift("missing_metadata_index", f"{table_name}.{index_name}", "required index is not registered in ORM metadata"))
            ...
            for index_name in sorted(required_indexes - db_indexes):
                drift.append(Drift("missing_index", f"{table_name}.{index_name}", "required active-lifecycle index is absent from database"))

...
def test_whatsapp_sidecar_candidate_smoke_script_defaults_to_no_live_send() -> None:
    ...
    assert "assert_account_metadata" in script
    assert "session_state" in script
    assert "missing hardened account metadata" in script
...
def test_native_whatsapp_candidate_smoke_runbook_keeps_nginx_and_writes_safe() -> None:
    ...
    assert "pairing-code" in runbook
    assert "session_state=partial" in runbook
    assert "qr_status=expired" in runbook

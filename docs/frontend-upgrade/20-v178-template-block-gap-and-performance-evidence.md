...
- Email: verify visual parity for queue, thread, composer, draft/send/audit state. Existing and newly uploaded external ticket attachments are selectable from the Email composer and are bound to outbound draft/send records, SMTP MIME dispatch, and timeline payload readback. Provider status, retry/failure evidence, `runtime.manage` gated dead-message requeue, outbound mailbox thread/message identity, and inbound provider-event sync evidence are now visible in the Email timeline. SMTP dispatch writes `Message-ID`, `In-Reply-To`, and `References` from the stored mailbox identity. Inbound sync writes `TicketInboundEmailMessage`, merges mailbox references onto the ticket thread, updates ticket customer-message state, and writes TicketEvent/AdminAuditLog evidence; remaining Email backend work is continuous mailbox polling/IMAP daemon and provider delivery receipt ingestion/UI.
...
The next PR should continue template-block parity with remaining visible Operations/Engineering handoff screens, broader visual parity polish, continuous Email mailbox polling/receipt ingestion, or Persona runtime analytics.

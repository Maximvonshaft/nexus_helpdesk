...
test('email workbench route uses unified routeAccess RBAC semantics', () => {
  ...
  assert.match(emailRoute, /const emailInternalNoteAccess = \{ allOf: \[CAPABILITIES\.noteWriteInternal\] \}/)
...
test('email workbench closes draft save, outbound send, and timeline refresh loops', () => {
  ...
  assert.match(emailRoute, /api\.addTicketInternalNote\(activeCase\.id, \{ body: internalNote\.trim\(\) \}\)/)
  ...
  assert.match(emailRoute, /Internal note/)
  assert.match(emailRoute, /只写内部 timeline\/audit，不发送给客户/)

...
def test_hybrid_merge_preserves_order_query_current_status() -> None:
    ...
    assert merged.latest_event.description == "Speedaf status code 4"
    assert merged.fact_evidence_present is True
    assert merged.source_authority == SOURCE_AUTHORITY_PRIMARY
    assert merged.evidence_state == EVIDENCE_CONTRADICTORY
    assert merged.used_sources[0]["authority"] == SOURCE_AUTHORITY_PRIMARY
    assert merged.used_sources[1]["authority"] == SOURCE_AUTHORITY_ENRICHMENT
    assert merged.contradictions[0]["resolution"] == "primary_current_status_preserved"
...
def test_hybrid_merge_reports_failed_history_and_preserves_primary() -> None:
    ...
    merged = merge_speedaf_hybrid_tracking_fact(primary=_primary_fact(), history=failed_history)

    assert merged.source == HYBRID_TRACKING_SOURCE
    assert merged.status == "4"
    assert merged.status_label == "Speedaf status code 4"
    assert merged.fact_evidence_present is True
    assert merged.used_sources[1]["evidence_state"] == EVIDENCE_TIMEOUT
...
def test_hybrid_merge_ignores_history_when_primary_is_not_trusted() -> None:
    ...
    assert len(merged.used_sources) == 1
    assert merged.used_sources[0]["authority"] == SOURCE_AUTHORITY_PRIMARY


def test_hybrid_lookup_does_not_query_or_promote_history_when_primary_has_no_evidence(monkeypatch) -> None:
    primary = TrackingFactResult(
        ok=False,
    ...
    monkeypatch.setattr(
        "app.services.speedaf.tracking_fact_source.lookup_speedaf_track_history_fact",
        lambda **_kwargs: history_calls.append(True),
    )

    fake_track_client = SimpleNamespace(config=SimpleNamespace(configured=True))
    result = lookup_speedaf_hybrid_tracking_fact(
        tracking_number="MK000179196R",
        track_client=fake_track_client,
    )

    assert result.fact_evidence_present is False
    assert result.status is None
    assert result.source_authority == SOURCE_AUTHORITY_PRIMARY

...
def _detail_out(db: Session, row) -> PersonaProfileDetailOut:
    versions = [
        PersonaProfileVersionOut.model_validate(item)
        for item in persona_service.list_versions(db, row.id)
    ]
    return PersonaProfileDetailOut.model_validate(row).model_copy(
...
def list_persona_profiles(
    ...
    return PersonaProfileListOut(
        profiles=[_profile_out(row) for row in rows],
        total=total,
    )
...
def resolve_persona_preview(
    ...
    return PersonaResolvePreviewOut(
        profile=_profile_out(row) if row else None,
        match_rank=score,
    )
...
def get_persona_runtime_evidence(
    ...
    request_id = f"persona-evidence-{current_user.id}-{time.time_ns()}"
    session_id = f"persona-evidence:{current_user.id}"
    ...
    matched_profile_key = (
        persona_context.get("profile_key")
        if isinstance(persona_context, dict)
        else None
    )
    match_rank = (
        persona_context.get("match_rank")
        if isinstance(persona_context, dict)
        else None
    )
    ...
    content_json = (
        persona_context.get("content_json")
        if isinstance(persona_context, dict)
        else {}
    )
    return PersonaRuntimeEvidenceOut(
        ...
        persona_context=(
            persona_context if isinstance(persona_context, dict) else None
        ),
        runtime_context=runtime_context,
        evidence={
            "runtime_contract": "AgentDeployment->AgentRelease->build_agent_context",
            "context_version": runtime_context.get("context_version"),
            "release_digest": runtime_context.get("agent_release_digest"),
            "release_error": runtime_context.get("agent_release_error"),
            "identity_ready": bool(identity_context),
            "brand_name": (
                identity_context.get("brand_name")
                if isinstance(identity_context, dict)
                else None
            ),
            "assistant_name": (
                identity_context.get("assistant_name")
                if isinstance(identity_context, dict)
                else None
            ),
            "guardrail_count": (
                len(content_json.get("guardrails") or [])
                if isinstance(content_json, dict)
                and isinstance(content_json.get("guardrails"), list)
                else 0
            ),
        },
    )
...
def list_persona_reviews(
    ...
    ensure_can_read_ai_configs(current_user, db)
    rows, total = persona_service.list_reviews(
        db,
    ...
    return PersonaProfileReviewListOut(
        reviews=[_review_out(row) for row in rows],
        total=total,
    )
...
def publish_approved_persona_review(
    ...
    review = persona_service.get_review_or_404(db, review_id)
    with managed_session(db):
        version_row = persona_service.publish_approved_review(
            db,
            review,
            current_user,
            notes=payload.notes,
...
def publish_persona_profile(
    ...
    del profile_id, payload
    ensure_can_manage_runtime(current_user, db)
    raise HTTPException(
        status_code=409,
        detail="persona_publish_requires_approved_review",
    )
...
def rollback_persona_profile(
    ...
    row = persona_service.get_profile_or_404(db, profile_id)
    with managed_session(db):
        version_row = persona_service.rollback_profile(
            db,
            row,

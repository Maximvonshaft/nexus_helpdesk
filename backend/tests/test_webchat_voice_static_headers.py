...
def test_live_voice_disabled_returns_404_without_exposing_upstream(monkeypatch):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="false", LIVE_VOICE_UPSTREAM_WS_URL="ws://runtime.example.test/live/ws", LIVE_VOICE_UPSTREAM_HEALTH_URL="http://runtime.example.test/live/health", LIVE_VOICE_UPSTREAM_TOKEN="unit-secret-token")
...
def test_live_voice_enabled_health_route_is_same_origin_proxy_scope(monkeypatch):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="true", WEBCHAT_VOICE_PROVIDER="mock", WEBCHAT_VOICE_CONNECT_SRC="", LIVE_VOICE_UPSTREAM_WS_URL="ws://127.0.0.1:1/live/ws", LIVE_VOICE_UPSTREAM_HEALTH_URL="http://127.0.0.1:1/live/health", LIVE_VOICE_UPSTREAM_TOKEN="unit-secret-token")
    response = client.get("/webchat/live/health")
...
def test_voice_enabled_non_voice_path_keeps_microphone_denied(monkeypatch):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="true", WEBCHAT_VOICE_CONNECT_SRC="wss://voice.example.test")
...
def test_voice_disabled_ignores_configured_unreadable_livekit_secret_file(monkeypatch, tmp_path):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="false", WEBCHAT_VOICE_PROVIDER="mock", **{LIVEKIT_KEY_FILE_ENV: str(tmp_path), LIVEKIT_SECRET_FILE_ENV: str(tmp_path)})
    response = client.get("/healthz")
    assert response.status_code == 200
    assert _permissions(response) == "camera=(), microphone=(), geolocation=()"
    assert "connect-src 'self'" in _csp(response)
...
def test_mock_provider_ignores_configured_unreadable_livekit_secret_file(monkeypatch, tmp_path):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="true", WEBCHAT_VOICE_PROVIDER="mock", WEBCHAT_VOICE_CONNECT_SRC="wss://voice.example.test", **{LIVEKIT_KEY_FILE_ENV: str(tmp_path), LIVEKIT_SECRET_FILE_ENV: str(tmp_path)})
    response = client.get("/healthz")
    assert response.status_code == 200
    assert _permissions(response) == "camera=(), microphone=(), geolocation=()"
...
def test_livekit_secret_read_failure_fails_closed_for_non_voice_route(monkeypatch, tmp_path):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="true", WEBCHAT_VOICE_PROVIDER="livekit", WEBCHAT_VOICE_CONNECT_SRC="wss://voice.example.test", **{LIVEKIT_URL_ENV: "wss://voice.example.test", LIVEKIT_KEY_ENV: "unit_key", LIVEKIT_SECRET_FILE_ENV: str(tmp_path)})
    response = client.get("/healthz")
    assert response.status_code == 200
    assert _permissions(response) == "camera=(), microphone=(), geolocation=()"
    assert "wss://voice.example.test" not in _csp(response)
...
def test_voice_enabled_voice_path_allows_microphone_and_configured_wss(monkeypatch):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="true", WEBCHAT_VOICE_CONNECT_SRC="wss://voice.example.test https://voice.example.test")
    response = client.get("/webchat/voice/session_demo")
    assert response.status_code == 200
    assert _permissions(response) == "camera=(), microphone=(self), geolocation=()"
    policy = _csp(response)
    _assert_connect_sources(policy, "'self'", "https://cloudflareinsights.com", "https://static.cloudflareinsights.com", "wss://voice.example.test", "https://voice.example.test")
...
def test_voice_enabled_custom_prefix_controls_voice_header_scope(monkeypatch):
    client = _client(monkeypatch, WEBCHAT_VOICE_ENABLED="true", WEBCHAT_VOICE_ALLOWED_PATH_PREFIXES="/voice/webchat", WEBCHAT_VOICE_CONNECT_SRC="wss://voice.example.test")
...
def test_voice_enabled_webchat_and_webchat_voice_paths_allow_microphone(monkeypatch):
    ...
    _assert_connect_sources(_csp(webchat_response), "'self'", "https://cloudflareinsights.com", "https://static.cloudflareinsights.com", "wss://voice.example.test", "https://voice.example.test")
    _assert_connect_sources(_csp(webchat_voice_response), "'self'", "https://cloudflareinsights.com", "https://static.cloudflareinsights.com", "wss://voice.example.test", "https://voice.example.test")

...
const knowledgeStudio = read('src/routes/knowledge-studio.tsx')
...
test('knowledge studio exposes template production workflow on real API contracts', () => {
  assert.match(knowledgeStudio, /Knowledge Studio/)
  assert.match(knowledgeStudio, /<GuidedWorkflow steps=/)
  assert.match(knowledgeStudio, /data-testid="knowledge-draft-chunk-preview"/)
  assert.match(knowledgeStudio, /data-testid="knowledge-golden-question"/)
  assert.match(knowledgeStudio, /data-testid="knowledge-release-evidence"/)
  assert.match(knowledgeStudio, /测试已发布知识/)
  assert.match(knowledgeStudio, /测试运行时上下文/)
  assert.match(knowledgeStudio, /发布并索引/)
  assert.match(knowledgeStudio, /回滚并重新发布/)
  assert.doesNotMatch(knowledgeStudio, /\bfetch\s*\(/)
})

...
def test_support_hours_json_uses_full_fast_lane_pipeline():
    ...
    try:
        ...
        assert "server_support_hours_policy" in (messages[1].metadata_json or "")
    finally:
        db.close()


def test_support_hours_stream_matches_json_policy_and_persists(monkeypatch):

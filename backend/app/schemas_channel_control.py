...
class ChannelOnboardingTaskCreate(BaseModel):
    ...
    @field_validator("provider", "target_slot", "desired_display_name", "desired_channel_account_binding", mode="before")
    @classmethod
...
class ChannelOnboardingTaskUpdate(BaseModel):
    ...
    @field_validator("target_slot", "desired_display_name", "desired_channel_account_binding", mode="before")
    @classmethod
    def strip_strings(cls, value):
        if isinstance(value, str):
            value = value.strip()
...
class ChannelOnboardingTaskCompleteRequest(BaseModel):
    ...
    @field_validator("desired_channel_account_binding", mode="before")
    @classmethod
    def strip_strings(cls, value):
        if isinstance(value, str):
            value = value.strip()

...
CATALOG_PATH = (
    ROOT / "backend" / "app" / "config" / "business_scenarios.v1.json"
)
VALIDATOR_PATH = (
    ROOT / "backend" / "scripts" / "validate_business_scenario_catalog.py"
)
...
def _scenario(payload: dict, key: str) -> dict:
    return next(
        item for item in payload["scenarios"] if item["scenario_key"] == key
    )
...
def test_production_catalog_is_active_bounded_and_complete() -> None:
    ...
    assert all(
        item.scope_mode == "inherit_resolved_scope" for item in catalog.scenarios
    )
...
def test_exact_alias_resolution_is_deterministic_and_no_fuzzy_match_exists() -> None:
    catalog = _load()
    assert (
        resolve_business_scenario(catalog, issue_type="parcel_status").scenario_key
        == "tracking_status_inquiry"
    )
    assert (
        resolve_business_scenario(
            catalog,
            scenario_key="cod_payment_anomaly",
        ).scenario_key
        == "cod_payment_anomaly"
    )
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_identity_conflict",
    ):
        resolve_business_scenario(
            catalog,
...
def test_non_authoritative_evidence_cannot_be_required_fact(
    fact_class: str,
) -> None:
    payload = _payload()
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_fact_authority_forbidden",
    ):
        _parse(payload)
...
def test_duplicate_json_key_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_catalog_duplicate_json_key",
    ):
...
def test_alias_conflict_and_repeated_key_fail_closed() -> None:
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_alias_conflict",
    ):
        _parse(payload)

    payload = _payload()
    payload["scenarios"][0]["issue_type_aliases"][0] = payload["scenarios"][0][
        "scenario_key"
    ]
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_alias_repeats_key",
    ):
        _parse(payload)


def test_review_due_is_governance_debt_not_runtime_expiry(tmp_path: Path) -> None:
    payload = _payload()
    for row in payload["scenarios"]:
        row["lifecycle"]["review_due"] = "2026-07-12T00:00:00Z"
    path = tmp_path / "review-overdue.json"
    path.write_text(json.dumps(payload), encoding="utf-8")

    evaluation_at = datetime(2026, 7, 13, tzinfo=timezone.utc)
    catalog = load_business_scenario_catalog(path, at=evaluation_at)
    assert all(item.is_active(evaluation_at) for item in catalog.scenarios)
    assert all(
        item.review_is_overdue(evaluation_at) for item in catalog.scenarios
    )


def test_explicit_expiry_fails_active_loading(tmp_path: Path) -> None:
    payload = _payload()
    for row in payload["scenarios"]:
        row["lifecycle"]["expires_at"] = "2026-07-12T00:00:00Z"
    path = tmp_path / "expired.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_catalog_contains_inactive_definition",
    ):
        load_business_scenario_catalog(
            path,
...
def test_semantic_rules_and_high_risk_escalation_are_mandatory() -> None:
    payload = _payload()
    payload["scenarios"][0]["completion_rules"].remove(
        "required_outcomes_completed"
    )
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_completion_rules_incomplete",
    ):
        _parse(payload)
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_high_risk_escalation_policy_missing",
    ):
        _parse(payload)
...
def test_required_action_must_be_allowed_and_allowed_action_cannot_be_blocked() -> None:
    payload = _payload()
    payload["scenarios"][0]["required_action_classes"].append(
        "return_process"
    )
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_required_action_not_allowed",
    ):
        _parse(payload)

    payload = _payload()
    payload["scenarios"][0]["blocked_action_classes"].append(
        "tracking_lookup"
    )
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_action_conflict",
    ):
        _parse(payload)
...
def test_required_notification_has_no_silent_bypass() -> None:
    payload = _payload()
    payload["scenarios"][0]["allowed_no_notification_reasons"] = [
        "no_contact_method"
    ]
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_required_notification_has_bypass",
    ):
        _parse(payload)
...
def test_required_capabilities_use_authoritative_permission_vocabulary() -> None:
    ...
    for item in catalog.scenarios:
        assert set(item.required_capabilities).issubset(
            AUTHORITATIVE_CAPABILITIES
        )
    address = resolve_business_scenario(
        catalog,
        scenario_key="address_contact_correction",
    )
    work_order = resolve_business_scenario(
        catalog,
        scenario_key="delivery_followup_work_order",
    )
    assert "tool:speedaf.order.update_address:write" in address.required_capabilities
    assert (
        "tool:speedaf.work_order.create:write" in work_order.required_capabilities
    )


def test_unknown_capability_fails_closed() -> None:
    payload = _payload()
    payload["scenarios"][0]["required_capabilities"].append(
        "speedaf.address_update.write"
    )
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_capabilities_invalid",
    ):
        _parse(payload)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "invented_cancel_rule",
            "scenario_cancellation_semantics_invalid",
        ),
    ],
)
def test_reason_code_vocabularies_fail_closed(
    field: str,
    value,
    reason: str,
) -> None:
...
def test_conditional_notification_cannot_require_unconditional_action_or_outcome() -> None:
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_conditional_notification_outcome_conflict",
    ):
        _parse(payload)
    ...
    with pytest.raises(
        BusinessScenarioCatalogError,
        match="scenario_conditional_notification_action_conflict",
    ):
        _parse(payload)
...
def test_escalate_and_reclassify_scenarios_cannot_be_falsely_closed() -> None:
    catalog = _load()
    legal = resolve_business_scenario(
        catalog,
        scenario_key="legal_privacy_high_risk",
    )
    intake = resolve_business_scenario(
        catalog,
        scenario_key="missing_information_intake",
    )
...
def test_validator_cli_returns_bounded_reason_for_invalid_catalog(
    tmp_path: Path,
) -> None:

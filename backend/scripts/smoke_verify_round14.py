...
db_path = Path(tempfile.gettempdir()) / "helpdesk_round14_smoke.db"
if db_path.exists():
    db_path.unlink()
...
os.environ["SECRET_KEY"] = "round14-secret-000000000000000000000000"
os.environ["OPENCLAW_SYNC_ENABLED"] = "true"
os.environ["OPENCLAW_EVENT_DRIVER_ENABLED"] = "true"
os.environ["OPENCLAW_CLI_FALLBACK_ENABLED"] = "true"
...
from backend.app.enums import ConversationState, JobStatus, SourceChannel, TicketPriority, TicketSource, UserRole, MessageStatus  # noqa: E402
from backend.app.main import app  # noqa: E402
from backend.app.models import ChannelAccount, Market, Team, User, OpenClawAttachmentReference, TicketAttachment, TicketOutboundMessage  # noqa: E402
from backend.app.schemas import CustomerInput, TicketCreate, OutboundSendRequest  # noqa: E402
...
from backend.app.services.ticket_service import create_ticket, send_outbound_message  # noqa: E402
...
# verify outbound dispatch path is really active
msg = send_outbound_message(db, ticket.id, OutboundSendRequest(channel=SourceChannel.whatsapp, body="Resolved. Here is the update."), admin)
assert msg.status == MessageStatus.pending
processed_msgs = dispatch_pending_messages(db, worker_id="smoke-outbound")
db.commit()
out = db.query(TicketOutboundMessage).filter(TicketOutboundMessage.id == msg.id).first()
assert out.status == MessageStatus.sent, out.status
db.refresh(ticket)
assert ticket.conversation_state == ConversationState.waiting_customer, ticket.conversation_state
...
assert ticket_body["conversation_state"] == "waiting_customer", ticket_body

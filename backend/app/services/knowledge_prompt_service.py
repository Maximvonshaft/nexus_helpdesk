...
def build_knowledge_prompt_block(knowledge_context: dict[str, Any] | None) -> str:
    ...
    lines = [
        "Knowledge context (sanitized KB authoritative context, not live parcel tracking evidence):",
        "- Customer-visible wording must be generated by the AI/provider.",
        "- Locked facts are authoritative; preserve their facts while writing a natural reply.",
        "- Never treat knowledge documents as live parcel tracking evidence.",
    ]
    for index, fact in enumerate(locked_facts[:MAX_LOCKED_FACTS], start=1):
        source = fact.get("source") if isinstance(fact.get("source"), dict) else {}
        answer = _clip(fact.get("answer") or fact.get("direct_answer"), MAX_DIRECT_ANSWER_CHARS)
        if not answer:
            continue
        lines.extend(
            [
                f"[LOCKED FACT {index}] id={_clean(fact.get('id'))} item_key={_clean(source.get('item_key'))} title={_clean(source.get('title'))}",
                f"answer={answer}",
            ]
        )
    for index, hit in enumerate(ordered, start=1):
        ...
        if direct_answer and not locked_facts:
            lines.append(f"authoritative_fact={direct_answer}")
...
def summarize_rag_trace(runtime_context: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "locked_facts": [
            {
                "id": fact.get("id"),
                "source": (fact.get("source") or {}) if isinstance(fact.get("source"), dict) else {},
            }
            for fact in (knowledge.get("locked_facts") or [])[:MAX_LOCKED_FACTS]

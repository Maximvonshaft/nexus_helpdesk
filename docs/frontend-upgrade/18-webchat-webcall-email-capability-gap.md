...
| Email operator workbench | Yes: `/api/lite/cases?source_channel=email`, `/api/tickets/{ticket_id}/summary`, `/api/tickets/{ticket_id}/timeline`, `/api/tickets/{ticket_id}/outbound/channels/capabilities`, `/api/tickets/{ticket_id}/outbound/draft`, `/api/tickets/{ticket_id}/outbound/send`. | PR #279 adds direct `/email` workbench and exposes it through AppShell navigation plus CommandPalette. It is a ticket-backed Email queue over the backend `source_channel=email` contract, not an independent inbound Email backend. | `/email` uses unified `routeAccess` and requires `ticket.read` plus at least one of `outbound.draft.save` or `outbound.send`. The queue is filtered server-side by `source_channel=email` and no longer falls back to generic tickets. Draft save uses the shared frontend API client, writes `TicketOutboundMessage(status=draft, provider_status=draft_saved)` and `TicketEvent(outbound_draft_saved)`. Send writes queued outbound message and `TicketEvent(outbound_queued)`. Timeline returns both `outbound_message` and `ticket_event`. | Missing true inbound Email ingestion/sync, mailbox thread identity, delivery receipt UI, and attachment send support. Category/sub-category Email heuristics are intentionally not treated as queue truth. |
...
- `webapp/tests/email-workbench-contract.test.mjs` locks the `/email` workbench `routeAccess` RBAC semantics, AppShell/CommandPalette entrypoints, backend `source_channel=email` queue filtering, API-client draft save, outbound send call, and timeline invalidation loop.
...
- Email queue filtering is backend-owned: `/email` calls `/api/lite/cases?source_channel=email`, does not use frontend regex/token guessing, and does not show generic ticket fallback rows.

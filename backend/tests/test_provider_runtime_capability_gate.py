...
def configure_static_runtime(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("PRIVATE_AI_RUNTIME_CHAT_MODE", "direct")
...
def test_verified_adapter_uses_one_generation_model_for_direct_and_rag(monkeypatch, tmp_path: Path) -> None:
    configure_static_runtime(monkeypatch, tmp_path)

    adapter = CapabilityVerifiedPrivateAIRuntimeAdapter(capability_probe=ready_probe_result)
...
def test_conflicting_legacy_model_configuration_fails_closed(monkeypatch, tmp_path: Path, name, value) -> None:
    configure_static_runtime(monkeypatch, tmp_path)
    monkeypatch.setenv(name, value)

    adapter = CapabilityVerifiedPrivateAIRuntimeAdapter(capability_probe=ready_probe_result)

    assert adapter._config_error() == "private_ai_runtime_legacy_model_configuration_invalid"


@pytest.mark.asyncio
async def test_capability_mismatch_blocks_underlying_generation(monkeypatch, tmp_path: Path) -> None:
    configure_static_runtime(monkeypatch, tmp_path)
    calls = []
    ...
    assert result.raw_payload_safe_summary["runtime_capability"]["status"] == "not_ready"
...
@pytest.mark.asyncio
async def test_ready_capability_calls_generation_and_attaches_safe_summary(monkeypatch, tmp_path: Path) -> None:
    configure_static_runtime(monkeypatch, tmp_path)
    calls = []
...
@pytest.mark.asyncio
async def test_static_configuration_failure_skips_capability_probe(monkeypatch, tmp_path: Path) -> None:
    configure_static_runtime(monkeypatch, tmp_path)
    monkeypatch.setenv("PRIVATE_AI_RUNTIME_DIRECT_MODEL", "qwen2.5:3b")
    probe_calls = []
    adapter = CapabilityVerifiedPrivateAIRuntimeAdapter(
        capability_probe=lambda: probe_calls.append(True) or ready_probe_result()
    )
    ...
    assert probe_calls == []
    assert result.ok is False
    assert result.error_code == "private_ai_runtime_legacy_model_configuration_invalid"
    assert result.fallback_allowed is False


def test_provider_registry_resolves_only_verified_adapter(monkeypatch, tmp_path: Path) -> None:
    configure_static_runtime(monkeypatch, tmp_path)
    ...
    try:
        ...
        assert not type(adapter) is PrivateAIRuntimeAdapter
    finally:
        ProviderRegistry._factories.clear()

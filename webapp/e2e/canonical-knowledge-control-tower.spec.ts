...
function knowledgeItem(id: number, title: string) {
  return {
    id, item_key: `support.customer.${id}`, title, summary: '', status: 'active', source_type: 'text', knowledge_kind: 'business_fact',
    channel: null, audience_scope: 'customer', language: null, priority: 100,
    fact_question: `${title}怎么处理？`, fact_answer: `${title}的事实和处理规则`, fact_aliases_json: [], fact_status: 'approved',
    answer_mode: 'guided_answer', draft_body: `${title}的事实和处理规则`, published_body: `${title}的事实和处理规则`, published_version: 1,
  }
}
...
test('Knowledge uses operator language and guards an unsaved draft', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/lite/knowledge-studio') return json(route, { kpis: [{ key: 'active', label: '已上线', value: 2, hint: '', tone: 'success' }] })
    if (url.pathname === '/api/knowledge-items' && route.request().method() === 'GET') return json(route, { items: [first, second], total: 2 })
...
test('Knowledge retrieval test explains whether the current service can use the result', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/knowledge-items/retrieve-test') return json(route, { grounding_would_apply: true, hits: [{ item_id: 1, chunk_index: 0, title: '末派失败', text: '先确认运单号和联系方式。', direct_answer: '先确认运单号和联系方式。', score: 0.91 }] })
    return json(route, { detail: `Unhandled test API ${url.pathname}` }, 404)
  })
  await page.goto('/knowledge')
...
test('Control Tower maps old management hrefs to canonical routes', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/lite/control-tower') {
      return json(route, {
        generated_at: '2026-07-14T12:00:00Z', role: 'admin', user_id: 8, capabilities: authUser.capabilities,
        kpis: [{ key: 'unassigned', label: '未分配', value: 12, hint: '需要分配责任人', tone: 'warning' }, { key: 'sla', label: 'SLA 风险', value: 3, hint: '即将或已经超时', tone: 'danger' }],
        manager_actions: [
        ...
        team_workload: [{ team_id: 2, team_name: '客服一组', active_tickets: 20, unassigned: 4, sla_risk: 2, overdue: 1 }], channel_health: [], bulletin_impact: [],
        governance_lanes: [{ key: 'runtime', area: '运行服务', value: 1, risk: 'warning', next: '检查降级状态', href: '/runtime', capability: 'runtime.manage', enabled: true }],
        template_blocks: [], facts: {},
      })
    }
    return json(route, { detail: `Unhandled test API ${url.pathname}` }, 404)
  })

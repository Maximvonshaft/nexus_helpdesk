...
def test_webchat_action_submit_is_idempotent_for_existing_legacy_card():
    ...
    init = client.post(
        "/api/webchat/init",
        json={
            # This test proves action idempotency, not Tenant provisioning. Use
            # the one explicitly supported non-production shadow scope rather
            # than inventing an unregistered Tenant identity.
            "tenant_key": "default",
            "channel_key": "website",
    ...
    sent = client.post(
        ...
        json={
            "body": "Hello, I need help tracking my parcel",
            "client_message_id": "action-idempotency-msg-1",
        },
    )

...
This document covers the mock UI phase of the WebChat Voice foundation work.
...
- Static guard tests for the mock UI contract
...
5. It does not load any realtime media SDK.
...
Later, after this mock UI phase is validated, the incoming voice panel can be embedded directly into `/webchat`.
...
- Calls the WebChat voice runtime config endpoint
- Calls the WebChat init endpoint when needed
- Calls the voice session API
- Opens the dedicated voice page
- Is loaded by the demo page as an optional extension
- Registers the admin mock console route

These guardrails keep the PR within the mock UI state-machine scope.

## Next phase

The real media phase can start only after the infrastructure decisions are made:

- Self-hosted or managed RTC provider
- Voice domain
- HTTPS/WSS strategy
- TURN strategy
...
The real media phase should replace the mock provider token with a real media provider token but still avoid realtime transcription and AI voice handling.

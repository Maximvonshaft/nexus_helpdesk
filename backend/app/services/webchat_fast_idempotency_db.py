...
def canonical_request_payload(
    *,
    ...
    # context. recent_context is still accepted by this function for call-site
    # compatibility and for AI generation, but it is deliberately excluded from
    # the request hash so weak-network retry, stale demo cache, multi-tab state,
    # and stream fallback do not turn the same client_message_id into a false
    # conflict.
    _ = recent_context
    return {

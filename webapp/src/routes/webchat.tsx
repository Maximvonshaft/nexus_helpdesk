...
function parseSafetyNotice(err: Error): SafetyNotice | null {
  const raw = err.message || ''
  const lower = raw.toLowerCase()
  if (lower.includes('blocked by safety gate') || lower.includes('safety_blocked') || lower.includes('internal/sensitive')) {
    return {
      kind: 'block',
      title: '回复已被安全门阻断',
      reasons: ['检测到内部系统信息、token、OpenClaw、MCP、traceback 或其他敏感内容。'],
      nextSteps: ['删除内部信息后重新发送。', '不要把系统调试信息、工具调用、密钥、数据库字段暴露给客户。'],
    }
  }
  if (lower.includes('requires human review') || lower.includes('review')) {
    return {
      kind: 'review',
      title: '该回复需要人工复核',
      reasons: ['回复可能涉及物流事实承诺、赔付、丢件、清关、预计送达等敏感事实。'],
      nextSteps: ['核对 tracking / POD / internal note。', '勾选“我已核对系统证据”。', '勾选“我确认继续发送”。', '再次点击发送。'],
    }
  }
  return null
}

function SafetyNoticeCard({ notice }: { notice: SafetyNotice | null }) {
  if (!notice) return null
  return (
    <Card className={notice.kind === 'block' ? 'danger' : 'soft'}>
      <CardHeader title={notice.title} subtitle={notice.kind === 'block' ? '该消息不会发送给访客。' : '复核前不会发送给访客。'} />
      <CardBody>
        <div className="stack">
          <div>
            <div className="section-title">原因</div>
            <ul>{notice.reasons.map((item) => <li key={item}>{item}</li>)}</ul>
          </div>
          <div>
            <div className="section-title">处理建议</div>
            <ol>{notice.nextSteps.map((item) => <li key={item}>{item}</li>)}</ol>
          </div>
        </div>
      </CardBody>
    </Card>
...
function WebchatInboxPage() {
  ...
  const replyMutation = useMutation({
    ...
    onError: (err: Error) => {
      const notice = parseSafetyNotice(err)
      setSafetyNotice(notice)
      setToast({ message: notice?.title || err.message || '发送失败，已被安全门拦截或需要复核', tone: 'danger' })
    },
  })

  const snippet = '<script src="https://YOUR_DOMAIN/static/webchat/widget.js" data-api-base="https://YOUR_DOMAIN" data-tenant="default" data-channel="website" data-title="Speedaf Support" data-subtitle="Usually replies instantly" data-assistant-name="Speedy" async></script>'
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<div className="button-row"><Button variant="secondary" onClick={() => client.invalidateQueries({ queryKey: ['webchatConversations'] })}>刷新</Button></div>}
      />
      ...
      <div className="page-grid workspace">
          ...
          <CardBody>
            ...
            <div className="list">
              {(conversations.data ?? []).map((item) => (
                <button key={item.conversation_id} className={`queue-card ${selectedTicketId === item.ticket_id ? 'selected' : ''}`} onClick={() => { setSelectedTicketId(item.ticket_id); setSafetyNotice(null) }}>
                  <div className="queue-card-top"><div className="badges"><Badge tone={statusTone(item.status)}>{sanitizeDisplayText(item.status)}</Badge><Badge tone="success">Webchat</Badge></div></div>
        ...
        <div className="stack">
                  ...
                  <div className="timeline">
                    {(thread.data?.messages ?? []).map((msg) => (
                      <div key={msg.id} className="message" data-role={msg.direction === 'visitor' ? 'user' : 'agent'}>
                        <div className="message-head"><strong>{msg.direction === 'visitor' ? '访客' : '客服'}</strong><span>{formatDateTime(msg.created_at)}</span></div>
          ...
          <Card>
            ...
            <CardBody>
              <div className="stack">
                <Field label="回复内容"><Textarea value={reply} onChange={(event) => { setReply(event.target.value); setSafetyNotice(null) }} placeholder="例如：We have received your request and will check it shortly." /></Field>

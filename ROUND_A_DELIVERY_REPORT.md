# NexusDesk × ExternalChannel Round A 交付报告

## 1. 本轮目标

Round A focuses on proving P0 functional chains with safe, repeatable smoke tests and deterministic ExternalChannel mock fixtures. It does not introduce SaaS tenant migration, production channel sending, or large product refactors.
...
| `scripts/smoke/smoke_e2e_external_channel_inbound_event.sh` | 脚本 | ExternalChannel inbound event fixture coverage | inbound event |
...
| `backend/scripts/mock_external_channel_server.py` | 代码 | deterministic ExternalChannel mock server | ExternalChannel mock |
| `backend/tests/fixtures/external_channel/*.json` | 测试夹具 | deterministic fixtures for route, transcript, events, attachments | ExternalChannel mock |
...
| `docs/external_channel-mock-testing.md` | 文档 | mock server contract | ExternalChannel mock |
...
`.github/workflows/round-a-smoke.yml` runs backend compile, pytest, Alembic, frontend typecheck/build, dry-run smoke, and mock-only smoke subset without real ExternalChannel secrets.

## 11. 剩余风险

- Real ExternalChannel Gateway/MCP live validation is still required.
...
3. Validate same-route reply against real ExternalChannel staging Gateway/MCP.

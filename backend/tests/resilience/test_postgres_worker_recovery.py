...
def _cleanup(Session, prefix: str) -> None:  # noqa: ANN001
    with Session() as db:
        db.query(BackgroundJob).filter(BackgroundJob.dedupe_key.like(f"{prefix}%")).delete(synchronize_session=False)
        db.commit()
...
def test_concurrent_postgres_claims_never_duplicate_a_job(session_factory) -> None:  # noqa: ANN001
    ...
    try:
        ...
        with Session() as db:
            rows = db.query(BackgroundJob).filter(BackgroundJob.id.in_(claimed)).all()
    ...
    try:
        with Session() as db:
            row = BackgroundJob(
                ...
                attempt_count=0,
                max_attempts=3,
                locked_at=utc_now() - timedelta(seconds=int(background_jobs.settings.job_lock_seconds) + 5),

...
from app import models, models_osr, operator_models, tool_models, webchat_models  # noqa: E402,F401
...
from app.services.nexus_osr.auto_ticket_service import (  # noqa: E402
...
def make_customer(db_session) -> Customer:
    row = Customer(
        name="Tool Exec Visitor",
        external_ref="tool-exec-visitor",
    )
    db_session.add(row)
    db_session.flush()
    return row
...
def case_context_with_tracking_and_contact(
    ...
    return ctx.with_contact_method(
        channel="whatsapp",
        value="+382 67123456",
        source="webchat_form",
    )
...
def _valid_work_order_arguments() -> dict[str, str]:
    return {
        "tracking_number": "CH1234567890",
        "work_order_type": "WT0103-05",
        "description": "Customer requested delivery follow-up",
    }
        ...
        tool_calls=[
            {
                ...
                "idempotency_key": "disabled-ticket",
                "requires_confirmation": True,
            }
        ],
        case_context=ctx,
...
def test_missing_tracking_and_contact_blocks_before_execution(db_session):
    ...
    assert set(result.summary["missing_requirements"]) == {
        "tracking_number",
        "contact_method",
    }
    assert db_session.query(Ticket).count() == 0
        ...
        tool_calls=[
            {
                ...
                "idempotency_key": "needs-confirm",
                "requires_confirmation": True,
                "arguments": {
                    "summary": "Human confirmation gated event",
                },
            }
        ],
...
def test_ticket_create_requires_customer_confirmation_then_executes_idempotently(
    db_session,
):
    add_policy(
        db_session,
...
def test_handoff_request_create_calls_handoff_service_and_suspends_ai(
    db_session,
):
    add_policy(
        db_session,
...
def test_timeline_event_create_writes_internal_safe_event(db_session):
        ...
        tool_calls=[
            {
                ...
                "arguments": {
                    "summary": (
                        "Customer user@example.com called from +382 67123456 "
                        "about CH1234567890"
                    ),
                },
            }
    ...
    log = (
        db_session.query(ToolCallLog)
        .filter(ToolCallLog.tool_name == "timeline.event.create")
        .one()
    )
...
def test_speedaf_work_order_default_blocked_even_when_policy_allows(
    db_session,
):
    add_policy(
        db_session,
    ...
    result = execute_controlled_tool_calls(
        ...
        tool_calls=[
            {
                "tool_name": "speedaf.workOrder.create",
                "idempotency_key": "speedaf-high-risk",
                "arguments": _valid_work_order_arguments(),
            }
        ],
        case_context=ctx,
        channel="webchat",
...
def test_speedaf_work_order_requires_explicit_test_allow_but_still_needs_handler(
    db_session,
):
    add_policy(
        db_session,
    ...
    result = execute_controlled_tool_calls(
        ...
        tool_calls=[
            {
                "tool_name": "speedaf.workOrder.create",
                "idempotency_key": "speedaf-allowed-test",
                "arguments": _valid_work_order_arguments(),
            }
        ],
        case_context=ctx,
        ...
        options=GovernedToolExecutionOptions(
            ...
            allowed_high_risk_write_tools=frozenset(
                {"speedaf.workOrder.create"}
            ),
        ),
    )[0]
        ...
        tool_calls=[
            {
                "tool_name": "timeline.event.create",
                "arguments": {
                    "summary": "This Tool must remain unavailable",
                },
            }
        ],
...
def test_server_idempotency_is_scoped_to_writes_and_ignores_model_keys():
    ...
    assert (
        core._idempotency_key_for_action(
            read_action,
...
def test_executor_rejects_raw_additional_properties_before_argument_bounding(
    ...
    log = db_session.query(ToolCallLog).one()
    combined = f"{log.input_summary} {log.output_summary} {log.error_message}"

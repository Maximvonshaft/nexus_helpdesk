...
class OperationsDispatchOutboxRecord(Base):
    """Durable state for one internal operations dispatch.

    Raw provider destinations and customer message bodies are deliberately not
    represented. The future provider adapter receives only the stable dispatch
    key and resolves provider-specific destination data through a separately
    governed boundary.
    """
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_operations_dispatch_outbox_attempt_count_bounded",
        ),
        CheckConstraint(

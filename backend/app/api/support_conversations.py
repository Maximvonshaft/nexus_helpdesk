...
def _runtime_latency_summary(db: Session, *, since, current_user) -> dict[str, Any]:
    ...
    for turn in turns:
        ...
        if isinstance(load_value, (int, float)) and not isinstance(load_value, bool) and load_value >= 1000:
            cold_load_count += 1
        if isinstance(prompt_value, (int, float)) and not isinstance(prompt_value, bool) and prompt_value >= 1500:
...
def _conversation_out(
    *,
    ...
    item: dict[str, Any] = {
        ...
        "display_name": mask_support_display_name(raw_display_name, fallback=ticket.ticket_no),
        ...
        "latest_message": safe_support_message_preview(raw_latest_message, limit=160),
        ...
        "can_accept": handoff_status == "requested",
        "can_release": handoff_status == "accepted" and active_agent_id == current_user.id,
        "can_resume_ai": handoff_status in {"requested", "accepted"},
        "can_reply": bool(
            ticket.id
            and not _ai_blocks_manual_reply(conversation)
            and (
                handoff_status == "none"
                or (handoff_status == "accepted" and active_agent_id == current_user.id)
            )
        ),
    }
...
def list_support_conversations(
    ...
    if q and q.strip():
        like = f"%{q.strip()}%"
        query = query.filter((Ticket.ticket_no.ilike(like)) | (Ticket.title.ilike(like)))

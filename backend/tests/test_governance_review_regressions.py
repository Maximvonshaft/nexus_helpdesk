...
def test_role_description_distinguishes_omitted_from_explicit_null(db_session):
    ...
    db_session.add(tenant)
    db_session.flush()
    actor = User(tenant_id=tenant.id, username="description-actor", display_name="Description Actor", password_hash="test", role=UserRole.admin, is_active=True)
    template = RoleTemplate(tenant_id=tenant.id, role_key="description-semantics", display_name="Description semantics", description="old description", base_role=UserRole.agent.value, risk_level="standard", draft_capabilities_json=["ticket.read"])
    db_session.add_all([actor, template])
    db_session.commit()
    governance_service.update_role_template(db_session, row=template, actor=actor, display_name="Renamed")
    assert template.description == "old description"
    governance_service.update_role_template(db_session, row=template, actor=actor, description=None)
...
def test_governance_access_mutations_lock_scope_inside_transaction():
    ...
    for endpoint in (governance_api.publish_role_template, governance_api.apply_role_template):
        source = inspect.getsource(endpoint)
        assert source.index("with managed_session(db):") < source.index("_lock_governance_scope")
        assert source.index("_lock_governance_scope") < source.index("_ensure_governance_access_survives")

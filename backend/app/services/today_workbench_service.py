...
from .tenant_query_authority import actor_tenant_query_scope
    ...
    for marker in EMAIL_MARKERS:
        ...
        predicates.extend(
            [
                ...
                Ticket.sub_category.ilike(like),
                Ticket.preferred_reply_channel.ilike(like),
            ]
        )
...
def _dead_runtime_count(db: Session, user: User) -> int:
    scope = actor_tenant_query_scope(db, user)
    jobs = int(
    ...
    return [
        {
            ...
            "overdue": bool(
                ...
                or ((_sla_due_at(ticket) or now) < now)
            ),
            "href": "/workspace",
        }
        for ticket in rows
    ...
    return [
        {
            "key": "loading",
            "state": "loading",
            "user_copy": "正在加载今日任务；超过 8 秒显示重试入口。",
            "required": "skeleton + retry + request id",
            "status": "implemented",
        },
        {
            "key": "empty",
            "state": "empty",
            "user_copy": "当前没有待处理任务，提供下一步入口而不是空表。",
            "required": "empty state + next best action",
            "status": "implemented",
        },
        {
            "key": "error",
            "state": "error",
            "user_copy": "展示失败原因、重试动作和诊断编号。",
            "required": "error summary + retry",
            "status": "implemented",
        },
        {
            "key": "permission",
            "state": "permission denied",
            "user_copy": "只展示当前 capability 可执行的入口。",
            "required": "capability filtered command",
            "status": "implemented",
        },
        {
            "key": "dirty",
            "state": "unsaved changes",
            "user_copy": "编辑类工作台保留离开确认。",
            "required": "dirty form guard",
            "status": "implemented",
        },
    ]
...
def build_today_workbench(db: Session, current_user: User) -> dict[str, Any]:
    ...
    my_tickets = _count(
        _active_tickets(visible).filter(Ticket.assignee_id == current_user.id)
    )
    unassigned = _count(
    ...
    email_cases = _count(
        _active_tickets(visible).filter(_email_case_filter())
    )
    ...
    handoff_count = (
        _active_handoff_count(db, current_user)
        if CAP_WEBCHAT_HANDOFF_ACCEPT in capabilities
        else 0
    )
    runtime_recovery = (
        _dead_runtime_count(db, current_user)
    ...
    tasks = [
        _task(
            ...
            "workspace",
            "/workspace",
            enabled=CAP_WEBCHAT_HANDOFF_ACCEPT in capabilities,
        ),
        _task(
            "my-tickets",
            "我的处理中工单",
            my_tickets,
            _tone(my_tickets, danger=12),
            "/api/lite/cases?assignee_id=me",
            "按 SLA 剩余时间排序处理",
            "workspace",
            "/workspace",
            enabled=CAP_TICKET_READ in capabilities,
        ),
        _task(
            "sla-risk",
            "30 分钟内 SLA 风险",
        ...
        _task(
            ...
            "客户待回复",
            ready_to_reply,
            _tone(ready_to_reply, danger=8),
            "/api/lite/cases?status=pending_human",
            "避免客户二次催问",
            "workspace",
            "/workspace",
            enabled=CAP_TICKET_READ in capabilities,
        ...
        _task(
            ...
            "等待中的 Email",
            email_cases,
            _tone(email_cases, danger=8),
            "/api/lite/cases?channel=email",
            "进入渠道与案例工作台处理邮件",
            "channels",
            "/channels",
            enabled=bool(
                {CAP_OUTBOUND_DRAFT_SAVE, CAP_OUTBOUND_SEND} & capabilities
            ),
        ),
    ]
    if CAP_TICKET_ASSIGN in capabilities:
        tasks.append(
            _task(
                ...
                "/api/lite/cases?assignee_id=null",
                "按语言、市场和负载分配",
                "workspace",
                "/workspace",
            )
    ...
    if CAP_RUNTIME_MANAGE in capabilities:
        tasks.append(
            _task(
                ...
                "运行异常待恢复",
                runtime_recovery,
                _tone(runtime_recovery, danger=5),
    ...
    return {
        ...
        "metrics": [
            ...
            _metric(
                ...
                "已超时",
                overdue,
                "first response 或 resolution 已违约",
                _tone(overdue, danger=1),
            ),
            _metric(
                "ready_to_reply",
                "客户待回复",
        ...
        "command_center": [
            ...
            _command(
                ...
                "/workspace",
                "在统一队列处理来电案例",
                enabled=CAP_WEBCALL_VOICE_QUEUE_VIEW in capabilities,
                capability=CAP_WEBCALL_VOICE_QUEUE_VIEW,
            ),
            _command(
                "cmd-runtime",
                "运行恢复 / dead 重排",
                ...
                "/runtime",
                "打开运行与审计页面",
                enabled=CAP_RUNTIME_MANAGE in capabilities,

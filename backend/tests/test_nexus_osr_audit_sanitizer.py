...
from app.services.nexus_osr.audit_sanitizer import AuditSanitizerLimits, safe_audit_label, sanitize_audit_payload
...
def test_sanitizer_redacts_hostile_nested_payload_and_preserves_safe_metadata():
    ...
    payload = {
        ...
        "summary": {"status": "out_for_delivery", "note": f"Contact {raw_email} at {raw_phone} for {raw_tracking} from {raw_address}"},
...
def test_sensitive_keys_key_names_and_failure_fallback_are_safe():
    ...
    assert "comparison must not run" not in _serialized(fallback)

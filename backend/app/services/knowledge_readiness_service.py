...
READINESS_SCHEMA_VERSION = "nexus_knowledge_readiness_v1"
EXPECTED_EMBEDDING_DIMENSION = 384
EXPECTED_VECTOR_TYPE = f"vector({EXPECTED_EMBEDDING_DIMENSION})"
MAX_DISTINCT_COVERAGE = 1000
...
def _is_current(item: Any, now: datetime) -> bool:
    for start in (getattr(item, "valid_from", None), getattr(item, "starts_at", None)):
        value = _as_utc(start)
        if value is not None and now < value:
            return False
    for end in (getattr(item, "valid_until", None), getattr(item, "ends_at", None)):
        value = _as_utc(end)
        if value is not None and now >= value:
            return False
    return True
...
def _is_customer_visible(item: Any) -> bool:
    return (
        ...
        and _scope(getattr(item, "audience_scope", None), "customer").lower() == "customer"
    )
...
def _is_stale(item: Any, *, now: datetime, freshness_cutoff: datetime) -> bool:
    ...
    if not _is_current(item, now):
        return True
    return published_at is None or published_at < freshness_cutoff
...
def collect_knowledge_snapshot(
    ...
    *,
    ...
    for item in iterator:
        ...
        eligible = customer_published and current and (
            _scope(getattr(item, "knowledge_kind", None), "document").lower() not in STRUCTURED_KINDS or approved
        )
        ...
        if customer_published and _is_stale(item, now=now, freshness_cutoff=freshness_cutoff):
            snapshot.stale_items += 1
        if customer_published and not _has_owner(item):
            snapshot.owner_missing_items += 1
        if customer_published and getattr(item, "review_due_at", None) is None:
            snapshot.review_date_missing_items += 1
        if eligible and _is_indexed(item):
    ...
    for chunk in chunk_iterator:
        ...
        if getattr(chunk, "embedding_status", None) == "failed":
            snapshot.failed_embedding_chunks += 1
        if getattr(chunk, "embedding_status", None) == "embedded":
            if int(getattr(chunk, "embedding_dim", 0) or 0) == EXPECTED_EMBEDDING_DIMENSION and getattr(chunk, "embedding", None):
                snapshot.embedded_chunks += 1
            else:
                snapshot.invalid_dimension_chunks += 1
        elif getattr(chunk, "embedding_dim", None) not in (None, EXPECTED_EMBEDDING_DIMENSION):
            snapshot.invalid_dimension_chunks += 1
    return snapshot


def provider_readiness(settings: Any) -> dict[str, Any]:
    runtime_version = _scope(getattr(settings, "knowledge_runtime_version", None), "v1").lower()
    ...
    dim_ok = int(getattr(settings, "knowledge_embedding_dim", 0) or 0) == EXPECTED_EMBEDDING_DIMENSION
    ...
    return {
        ...
        "dimension": EXPECTED_EMBEDDING_DIMENSION if dim_ok else 0,
    }
...
def retrieval_readiness(
    ...
    *,
    ...
    query = _scope(
        getattr(item, "fact_question", None)
...
def knowledge_gap_state(db: Any, snapshot: KnowledgeReadinessSnapshot, retrieval_gate: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        task_count = int(
            ...
            .filter(
                ...
                OperatorTask.status.notin_(TERMINAL_GAP_STATUSES),
            )
            .scalar()
            or 0
        )
    ...
    return {
        ...
        "reason_codes": reasons,
        "open_gap_count": min(total, 1_000_000),
...
def assess_knowledge_readiness(
    ...
    *,
    ...
    for gate in gates.values():
        for reason in gate.get("reason_codes") or []:
            ...
            if len(reasons) >= 20:
                break
    counts = {
        "active_items": _bounded_count(snapshot.active_items),
        "approved_items": _bounded_count(snapshot.approved_items),
        "published_items": _bounded_count(snapshot.published_items),
        "customer_published_items": _bounded_count(snapshot.customer_published_items),
        "eligible_items": _bounded_count(snapshot.eligible_items),
        "stale_items": _bounded_count(snapshot.stale_items),
        "owner_missing_items": _bounded_count(snapshot.owner_missing_items),
        "review_date_missing_items": _bounded_count(snapshot.review_date_missing_items),
        "indexed_items": _bounded_count(snapshot.indexed_items),
        "total_chunks": _bounded_count(snapshot.total_chunks),
        "embedded_chunks": _bounded_count(snapshot.embedded_chunks),
        "failed_embedding_chunks": _bounded_count(snapshot.failed_embedding_chunks),
        "invalid_dimension_chunks": _bounded_count(snapshot.invalid_dimension_chunks),
        "open_knowledge_gaps": _bounded_count(gap_gate.get("open_gap_count", 0)),
    }
    coverage = {
    ...
    freshness = {
        ...
        "stale_items": _bounded_count(snapshot.stale_items),
        "owner_coverage_percent": round(100.0 * max(0, snapshot.published_items - snapshot.owner_missing_items) / snapshot.published_items, 2) if snapshot.published_items else 0.0,
        "review_date_coverage_percent": round(100.0 * max(0, snapshot.published_items - snapshot.review_date_missing_items) / snapshot.published_items, 2) if snapshot.published_items else 0.0,
    }
    metrics = (
...
def unavailable_report(*, evaluated_at: datetime | None = None) -> KnowledgeReadinessReport:
    ...
    return KnowledgeReadinessReport(
        ...
        coverage={
            ...
            "country_count": 0,
            "channel_count": 0,
            "tenant_count_overflow": False,
            "country_count_overflow": False,
            "channel_count_overflow": False,
            "expected_tenant_covered": None,
            "expected_country_covered": None,
            "expected_channel_covered": None,
        },
        freshness={
        ...
        gates={"runtime": gate},
        metrics=(
            {"name": "nexus_knowledge_readiness", "value": 0, "labels": {"status": "unavailable"}},
        ),
    )

...
TRACKING_ID_RE = re.compile(
    r"(?<![A-Z0-9])[A-Z0-9][A-Z0-9-]{7,34}[A-Z0-9](?![A-Z0-9])",
    re.IGNORECASE,
)
...
class KnowledgeVectorContractError(ValueError):
    pass
...
def validate_embedding_dimension(value: Any) -> int:
    if isinstance(value, bool):
        raise KnowledgeVectorContractError("knowledge_vector_dimension_mismatch")
    try:
        dimension = int(value)
    except (TypeError, ValueError) as exc:
        raise KnowledgeVectorContractError("knowledge_vector_dimension_mismatch") from exc
    if dimension != KNOWLEDGE_VECTOR_DIMENSION:
        raise KnowledgeVectorContractError("knowledge_vector_dimension_mismatch")
    return dimension


def validate_knowledge_vector(value: Any, *, dimension: int = KNOWLEDGE_VECTOR_DIMENSION) -> list[float]:
    validate_embedding_dimension(dimension)
    if not isinstance(value, (list, tuple)):
        raise KnowledgeVectorContractError("knowledge_embedding_vector_invalid")
...
def validate_embedding_batch(vectors: Any, *, expected_count: int) -> list[list[float]]:
    if not isinstance(vectors, (list, tuple)) or len(vectors) != expected_count:
        raise KnowledgeVectorContractError("knowledge_embedding_cardinality_mismatch")
    return [validate_knowledge_vector(vector) for vector in vectors]
...
def is_live_tracking_intent(value: str | None) -> bool:
    ...
    has_identifier = any(
        ...
        for match in TRACKING_ID_RE.finditer(original)
    )
    ...
    return bool(live_signal and not policy_only)
...
def retrieve_knowledge(
    ...
    *,
    ...
    requested_audience = _scope_value(audience_scope, "customer")
    filters = {
        "tenant_id": tenant_id,
        "brand_id": brand,
    ...
    if requested_audience != "customer":
        return _blocked_result(
            reason="knowledge_audience_not_customer_visible",
            started=started,
    ...
    if is_live_tracking_intent(query):
        return _blocked_result(
            reason="live_tracking_requires_truth_source",
            started=started,
            normalized=normalized,
            terms=terms,
            filters=filters,
    ...
    if settings.knowledge_embeddings_enabled:
        try:
            ...
            query_vectors = provider.embed_texts([normalized])
            query_embedding = validate_embedding_batch(query_vectors, expected_count=1)[0]
    ...
    direct_facts = [
        _fact_from_hit(hit)
        for hit in hits
        if hit.direct_answer and hit.metadata.get("fact_status") == "approved"
    ]
    ...
    methods = sorted(
        {
            method
            for hit in hits
            for method in hit.retrieval_method.split("+")
            if method
        }
    )
    if vector_degraded:
    ...
    *,
    ...
    query = (
        ...
        .filter(
            ...
            KnowledgeChunk.shareability == "customer_visible",
            KnowledgeItem.shareability == "customer_visible",
            KnowledgeChunk.audience_scope == audience_scope,
            ...
            or_(
                ...
                (
                    ...
                    & (KnowledgeChunk.fact_status == "approved")
                ),
            ),
        )
    )
    ...
    if market_id is not None:
        query = query.filter(
            or_(KnowledgeChunk.market_id.is_(None), KnowledgeChunk.market_id == market_id)
        )
    if channel:
        requested = channel.strip()
        query = query.filter(
            or_(KnowledgeChunk.channel.is_(None), KnowledgeChunk.channel == requested),
            or_(KnowledgeItem.channel.is_(None), KnowledgeItem.channel == requested),
        )
    if language:
        lang = language.strip().lower()
        query = query.filter(
            or_(
                KnowledgeChunk.language.is_(None),
                KnowledgeChunk.language == lang,
                KnowledgeChunk.language == "mixed",
                KnowledgeChunk.language.like(f"{lang}-%"),
            )
        )
...
def _postgres_candidate_rows(
    ...
    *,
    ...
    for chunk, item in _structured_exact_rows(
        db,
        terms=terms,
        normalized_query=normalized_query,
        tenant_id=tenant_id,
    ...
    search_ids = _postgres_fts_ids(
        db,
        query_text=normalized_query or " ".join(terms),
        tenant_id=tenant_id,
        brand_id=brand_id,
        country_scope=country_scope,
        channel_scope=channel_scope,
...
def _vector_rows(
    ...
    *,
    ...
    validated_query = validate_knowledge_vector(query_embedding)
    if _is_postgres(db):
        ids_and_scores = _postgres_vector_ids(
            db,
            query_embedding=validated_query,
            tenant_id=tenant_id,
            brand_id=brand_id,
            country_scope=country_scope,
            channel_scope=channel_scope,
        ...
        rows_by_id = {
            chunk.id: (chunk, item)
            for chunk, item in _rows_by_chunk_ids(
                db,
                [chunk_id for chunk_id, _score in ids_and_scores],
            )
        }
        now = _as_utc(utc_now()) or datetime.now(timezone.utc)
        result: list[tuple[KnowledgeChunk, KnowledgeItem, float]] = []
        for chunk_id, score in ids_and_scores:
            pair = rows_by_id.get(chunk_id)
            if pair is None:
                continue
            chunk, item = pair
            if not _eligible(
                ...
                continue
            result.append((chunk, item, score))
        return result
    ...
    for chunk, item in fallback_rows:
        if (
            ...
            or chunk.embedding_dim != KNOWLEDGE_VECTOR_DIMENSION
        ):
            continue
        try:
            stored = validate_knowledge_vector(chunk.embedding)
        except KnowledgeVectorContractError:
            continue
        result.append((chunk, item, cosine_similarity(validated_query, stored)))
    return result
...
def _postgres_vector_ids(
    ...
    *,
    ...
    params["query_vector"] = vector_literal(validate_knowledge_vector(query_embedding))
    rows = db.execute(text(sql), params).mappings().all()
    return [
        (int(row["chunk_id"]), max(0.0, 1.0 - float(row["distance"])))
        for row in rows
    ]
...
def _postgres_candidate_sql(
    *,
    ...
    filters = [
        ...
        "ki.published_version > 0",
        "ki.published_at IS NOT NULL",
        ...
        "kc.shareability = 'customer_visible'",
        "ki.shareability = 'customer_visible'",
        "kc.audience_scope = :audience_scope",
        "ki.audience_scope = :audience_scope",
        ...
        "(ki.starts_at IS NULL OR ki.starts_at <= now())",
        "(ki.ends_at IS NULL OR ki.ends_at >= now())",
        "(ki.valid_from IS NULL OR ki.valid_from <= now())",
        "(ki.valid_until IS NULL OR ki.valid_until >= now())",
        "(ki.knowledge_kind IS NULL OR ki.knowledge_kind NOT IN ('faq', 'business_fact') OR (ki.fact_status = 'approved' AND kc.fact_status = 'approved'))",
    ...
    if channel:
        filters.extend(
            [
                "(kc.channel IS NULL OR kc.channel = :channel)",
                "(ki.channel IS NULL OR ki.channel = :channel)",
            ]
        )
        params["channel"] = channel.strip()
    if language:
        filters.append(
            "(kc.language IS NULL OR kc.language = :language OR "
            "kc.language = 'mixed' OR kc.language LIKE :language_prefix)"
        )
        params["language"] = language.strip().lower()
        params["language_prefix"] = f"{language.strip().lower()}-%"
    where = " AND ".join(filters)
    if vector:
        params["vector_dim"] = KNOWLEDGE_VECTOR_DIMENSION
        sql = f"""
            ...
            WHERE {where}
              ...
              AND kc.embedding_dim = :vector_dim
...
def _exclude_probe_rows(query):
    ...
    return query.filter(
        ...
        or_(
            KnowledgeItem.citation_metadata_json.is_(None),
            ~KnowledgeItem.citation_metadata_json.cast(Text).ilike("%probe_category%"),
        ),
        or_(
            KnowledgeChunk.metadata_json.is_(None),
            ~KnowledgeChunk.metadata_json.cast(Text).ilike("%probe_category%"),
        ),
        or_(
            KnowledgeChunk.metadata_json.is_(None),
            ~KnowledgeChunk.metadata_json.cast(Text).ilike("%probe_seed%"),
        ),
    )
...
def _eligible(
    ...
    *,
    ...
    if str(item.status or "").lower() != "active":
        return False
    if str(chunk.status or "").lower() != "active":
        return False
    ...
    if int(chunk.published_version or 0) != int(item.published_version or 0):
        return False
    if _scope_value(item.tenant_id, "default") != tenant_id:
        return False
    if _scope_value(chunk.tenant_id, "default") != tenant_id:
        return False
    if _scope_value(item.brand_id, "default") != brand_id:
        return False
    if _scope_value(chunk.brand_id, "default") != brand_id:
        return False

    requested_country = _country_scope(country_scope)
    for value in (item.country_scope, chunk.country_scope):
        if _country_scope(value) not in {requested_country, GLOBAL_COUNTRY_SCOPE}:
            return False

    requested_channel = _scope_value(channel_scope, GLOBAL_CHANNEL_SCOPE).lower()
    for value in (item.channel_scope, chunk.channel_scope):
        if _scope_value(value, GLOBAL_CHANNEL_SCOPE).lower() not in {
            requested_channel,
            GLOBAL_CHANNEL_SCOPE,
        }:
            return False
    if channel:
    ...
    requested_audience = _scope_value(audience_scope, "customer").lower()
    for value in (item.audience_scope, chunk.audience_scope):
        if _scope_value(value, "customer").lower() != requested_audience:
            return False
    for value in (item.visibility, chunk.visibility):
        if _scope_value(value, "internal").lower() != CUSTOMER_VISIBILITY:
            return False
    for value in (item.shareability, chunk.shareability):
        if _scope_value(value, "internal").lower() != "customer_visible":
            return False
    ...
    kind = str(item.knowledge_kind or "document").lower()
    if kind in STRUCTURED_KINDS:
        if str(item.fact_status or "draft").lower() != "approved":
            return False
        if str(chunk.fact_status or "draft").lower() != "approved":
            return False
    return True
...
def _score_row(
    ...
    *,
    ...
    structured = (
        (item.knowledge_kind or "document") in STRUCTURED_KINDS
        and item.fact_status == "approved"
        and chunk.fact_status == "approved"
    )
    direct = (
        structured
        and item.answer_mode == "direct_answer"
        and bool((item.fact_answer or "").strip())
    )
    structured_fact_answer = (
        (item.fact_answer or "").strip()
        if structured
        and (item.answer_mode or "guided_answer") in {"direct_answer", "guided_answer"}
        else None
    )
    ...
    metadata.update(
        {
            ...
            "country_scope": (
                chunk.country_scope or item.country_scope or GLOBAL_COUNTRY_SCOPE
            ).upper(),
            "channel_scope": (
                chunk.channel_scope or item.channel_scope or GLOBAL_CHANNEL_SCOPE
            ),
            ...
            "review_due_at": (
                chunk.review_due_at.isoformat()
                if chunk.review_due_at
                else (item.review_due_at.isoformat() if item.review_due_at else None)
            ),
...
def _merge_hits(
    left: KnowledgeRuntimeHit | None,
    right: KnowledgeRuntimeHit,
) -> KnowledgeRuntimeHit:
    if left is None:
        return right
    ...
    methods = "+".join(
        sorted(
            set(left.retrieval_method.split("+"))
            | set(right.retrieval_method.split("+"))
        )
    )
    ...
    return KnowledgeRuntimeHit(
        **{
            **left.__dict__,
            ...
            "score_breakdown": breakdown,
            "matched_terms": sorted(set(left.matched_terms + right.matched_terms)),
        }
    )
...
def _apply_answerability_policy(
    ...
    for hit in hits:
        ...
        if (
            ...
            and hit.score >= MIN_HIGH_RISK_SCORE
        ):
            gated.append(hit)
    return gated
def _blocked_result(

...
class PersonaProfile(Base):
    ...
    draft_content_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    published_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    published_content_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class KnowledgeItem(Base):
    ...
    starts_at: Mapped[datetime | None] = mapped_column(UTCDateTime, nullable=True, index=True)
    ends_at: Mapped[datetime | None] = mapped_column(UTCDateTime, nullable=True, index=True)
    source_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
    file_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    file_storage_key: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    mime_type: Mapped[str | None] = mapped_column(String(120), nullable=True)
    file_size: Mapped[int | None] = mapped_column(Integer, nullable=True)
    draft_body: Mapped[str | None] = mapped_column(Text, nullable=True)
    draft_normalized_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    published_body: Mapped[str | None] = mapped_column(Text, nullable=True)
    published_normalized_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    published_version: Mapped[int] = mapped_column(Integer, default=0)
    published_at: Mapped[datetime | None] = mapped_column(UTCDateTime, nullable=True, index=True)
    created_by: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True, index=True)
    updated_by: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True, index=True)
...
class ChannelOnboardingTask(Base):
    ...
    market_id: Mapped[int | None] = mapped_column(ForeignKey("markets.id"), nullable=True, index=True)
    target_slot: Mapped[str | None] = mapped_column(String(120), nullable=True, index=True)
    desired_display_name: Mapped[str | None] = mapped_column(String(160), nullable=True)
    desired_channel_account_binding: Mapped[str | None] = mapped_column(String(160), nullable=True)
    openclaw_account_id: Mapped[str | None] = mapped_column(String(160), nullable=True, index=True)

...
DEFAULT_CATALOG_PATH = (
    Path(__file__).resolve().parents[2] / "config" / "business_scenarios.v1.json"
)
...
TRIGGERS = frozenset(
    {"customer_message", "operator_input", "operational_signal", "system_failure"}
)
FACTS = frozenset(
    {
        "parcel_identity",
...
FORBIDDEN_FACTS = frozenset(
    {"customer_claim", "prior_ai_output", "ai_recommendation"}
)
RISKS = frozenset({"low", "medium", "high", "critical"})
NOTIFICATION_POLICIES = frozenset(
    {"required", "required_if_contactable", "optional", "prohibited"}
)
...
TERMINAL_BEHAVIORS = frozenset(
    {"closeable", "reclassify_only", "escalate_only"}
)
OUTCOMES = frozenset(
    {
        "accepted",
...
COMPLETION_RULES = frozenset(
    {
        ...
        "required_customer_inputs_present",
        "required_actions_completed",
        "required_outcomes_completed",
...
REOPEN_CONDITIONS = frozenset(
    {
        "customer_recontacts_within_observation",
        "new_authoritative_contradiction",
        "required_action_failed",
        "delivery_failure_after_intervention",
        "customer_disputes_resolution",
        "dispatch_repair_required",
...
METRICS = frozenset(
    {
        ...
        "business_result_confirmation_rate",
        "false_closure_rate",
...
AUTHORITATIVE_CAPABILITIES = frozenset(
    str(value).strip().lower() for value in ALL_CAPABILITIES
)

CATALOG_FIELDS = frozenset(
    {"schema", "catalog_version", "owner", "approved_at", "scope_mode", "scenarios"}
)
...
class ScenarioLifecycle:
    ...
    def is_active(self, at: datetime) -> bool:
        """Return runtime eligibility; governance review debt is not expiry."""
    ...
    def review_is_overdue(self, at: datetime) -> bool:
        """Return governance debt without disabling customer operations."""

        return at >= self.review_due
...
class BusinessScenarioDefinition:
    ...
    def review_is_overdue(self, at: datetime | None = None) -> bool:
        return self.lifecycle.review_is_overdue(_utc(at))
    ...
    def safe_summary(self) -> dict[str, Any]:
        return {
            ...
            "review_due": self.lifecycle.review_due.isoformat(),
            "expires_at": (
                self.lifecycle.expires_at.isoformat()
                if self.lifecycle.expires_at is not None
                else None
            ),
        }
...
def load_business_scenario_catalog(
    ...
    *,
    ...
    if require_all_active and any(
        not item.is_active(at) for item in catalog.scenarios
    ):
        raise BusinessScenarioCatalogError(
            "scenario_catalog_contains_inactive_definition"
        )
    return catalog
...
def parse_business_scenario_catalog(
    ...
    *,
    ...
    approved_at = _timestamp(
        root["approved_at"],
        "scenario_catalog_approved_at_invalid",
    )
    ...
    return BusinessScenarioCatalog(
        ...
        catalog_version=_key(
            root["catalog_version"],
            "scenario_catalog_version_invalid",
        ),
...
def evaluate_scenario_readiness(
    ...
    *,
    ...
    missing_facts = tuple(
        item for item in scenario.required_fact_classes if item not in facts
    )
    missing_inputs = tuple(
        item for item in scenario.required_customer_inputs if item not in inputs
    )
    missing_actions = tuple(
        item for item in scenario.required_action_classes if item not in actions
    )
    missing_outcomes = tuple(
        item for item in scenario.required_outcome_levels if item not in outcomes
    )
...
def _parse_scenario(payload: Any) -> BusinessScenarioDefinition:
    ...
    raw_facts = _sequence(
        row["required_fact_classes"],
        "scenario_fact_classes_invalid",
    )
    normalized_facts = tuple(
        str(item or "").strip().lower() for item in raw_facts
    )
    ...
    if notification == "required" and no_notification:
        raise BusinessScenarioCatalogError(
            "scenario_required_notification_has_bypass"
        )
    if notification == "required_if_contactable" and not no_notification:
        raise BusinessScenarioCatalogError(
            "scenario_conditional_notification_reasons_missing"
        )
    if notification in {"optional", "prohibited"} and no_notification:
        raise BusinessScenarioCatalogError(
            "scenario_notification_waiver_not_applicable"
        )
    ...
    risk = _choice(row["risk_level"], RISKS, "scenario_risk_level_invalid")
...
def _validate_scenario_semantics(
    *,
    ...
    requirements = (
        ...
        (terminal_behavior == "reclassify_only", "scenario_reclassified"),
        (
    ...
    if risk_level in {"high", "critical"} and escalation_policy_key is None:
        raise BusinessScenarioCatalogError(
            "scenario_high_risk_escalation_policy_missing"
        )
    if (
        terminal_behavior == "reclassify_only"
        and "scenario_reclassify" not in required_actions
    ):
        raise BusinessScenarioCatalogError(
            "scenario_reclassification_action_missing"
        )
    ...
    if notification_policy == "required":
        if not customer_notified_required:
            raise BusinessScenarioCatalogError(
                "scenario_required_notification_outcome_missing"
            )
        if not notify_action_required:
            raise BusinessScenarioCatalogError(
                "scenario_required_notification_action_missing"
            )
    else:
        if customer_notified_required:
            raise BusinessScenarioCatalogError(
                "scenario_conditional_notification_outcome_conflict"
            )
        if notify_action_required:
            raise BusinessScenarioCatalogError(
                "scenario_conditional_notification_action_conflict"
            )
...
def _parse_lifecycle(payload: Any) -> ScenarioLifecycle:
    ...
    if approved > effective:
        raise BusinessScenarioCatalogError(
            "scenario_lifecycle_approval_after_effective"
        )
    if review <= effective:
...
def _notification_satisfied(
    ...
    if policy == "optional":
        return value in {
            ...
            "sent",
            "delivered",
        } or _valid_notification_waiver(value, allowed_reasons)
...
def _reject_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise BusinessScenarioCatalogError(
                "scenario_catalog_duplicate_json_key"
            )
        result[key] = value
    return result

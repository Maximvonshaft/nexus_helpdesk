...
test('email mailbox daemon status and enqueue use unified API client', () => {
  assert.match(types, /export interface EmailMailboxSyncStatus/)
  assert.match(types, /export interface EmailMailboxSyncEnqueueResult/)
  assert.match(apiClient, /emailMailboxSyncStatus: \(\) => request<EmailMailboxSyncStatus>\('\/api\/email\/mailbox-sync\/status'\)/)
  assert.match(apiClient, /enqueueEmailMailboxSync: \(payload\?: \{ account_id\?: number \| null \}\)/)
  assert.match(apiClient, /'\/api\/email\/mailbox-sync\/enqueue'/)
  assert.match(emailRoute, /const emailMailboxSyncAccess = \{ allOf: \[CAPABILITIES\.runtimeManage\] \}/)
  assert.match(emailRoute, /data-testid="email-mailbox-daemon"/)
  assert.match(emailRoute, /api\.emailMailboxSyncStatus/)
  assert.match(emailRoute, /api\.enqueueEmailMailboxSync\(\{\}\)/)
  assert.match(emailRoute, /Mailbox Polling \/ IMAP Daemon/)
  assert.match(emailRoute, /client\.invalidateQueries\(\{ queryKey: \['emailMailboxSyncStatus'\] \}\)/)
  assert.match(emailRoute, /daemon_enabled/)
  assert.match(emailRoute, /pending_jobs/)
  assert.doesNotMatch(emailRoute, /\bfetch\s*\(/)
})

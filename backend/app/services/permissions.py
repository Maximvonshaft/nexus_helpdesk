...
def ensure_can_manage_channels(user, db: Session | None = None):
...
def ensure_can_manage_ai_configs(user, db: Session | None = None):
    ensure_capability(user, CAP_AI_CONFIG_MANAGE, db, message="Not authorized to manage AI configs")


def ensure_can_manage_runtime(user, db: Session | None = None):
    ensure_capability(user, CAP_RUNTIME_MANAGE, db, message="Not authorized to access runtime operations")
...
def ensure_can_manage_capabilities(user, db: Session | None = None):
    ensure_can_manage_users(user, db)

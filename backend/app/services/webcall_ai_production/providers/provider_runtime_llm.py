...
async def _route_request(db: Session, request: ProviderRequest) -> ProviderResult:
    ...
    if provider_alias not in _ALLOWED_PROVIDER_ALIASES:
        return ProviderResult.unavailable("router", "provider_runtime_provider_not_allowed", 0, fallback_allowed=False)
...
def _build_request(*, text: str, language: str | None) -> ProviderRequest:
    ...
    metadata = {
        ...
        "safety_policy": {"knowledge_scope": "voice_transcript_only_without_tracking_evidence", "tracking_truth_boundary": "Parcel live status requires tracking_fact_evidence_present=true and trusted tracking_fact_summary."},
        "source": "webcall_ai_production",
        "language": lang,
    }
    return ProviderRequest(request_id=f"webcall-ai-llm-{uuid.uuid4().hex}", tenant_id=_env("WEBCALL_AI_PROVIDER_RUNTIME_TENANT_ID", _DEFAULT_TENANT), tenant_key=_env("WEBCALL_AI_PROVIDER_RUNTIME_TENANT_KEY", _env("WEBCALL_AI_PROVIDER_RUNTIME_TENANT_ID", _DEFAULT_TENANT)), channel_key=_env("WEBCALL_AI_PROVIDER_RUNTIME_CHANNEL_KEY", _DEFAULT_CHANNEL), session_id=_env("WEBCALL_AI_PROVIDER_RUNTIME_SESSION_ID", "webcall-ai-production"), scenario=_env("WEBCALL_AI_PROVIDER_RUNTIME_SCENARIO", _DEFAULT_SCENARIO), body=body, recent_context=[{"role": "user", "content": body, "language": lang}], tracking_fact_summary=None, tracking_fact_evidence_present=False, output_contract=_env("WEBCALL_AI_PROVIDER_RUNTIME_OUTPUT_CONTRACT", _DEFAULT_CONTRACT), timeout_ms=_int_env("WEBCALL_AI_PROVIDER_RUNTIME_TIMEOUT_MS", 10000, minimum=500, maximum=30000), metadata=metadata)
...
def _to_llm_result(result: ProviderResult) -> LLMResult:
    ...
    return LLMResult(response_text=reply, intent=intent, handoff_required=handoff_required, handoff_reason=handoff_reason, provider_name=f"provider_runtime:{result.provider}")

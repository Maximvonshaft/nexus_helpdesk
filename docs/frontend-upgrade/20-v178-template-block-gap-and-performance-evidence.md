...
- AI Persona Builder: `/persona-builder` now consumes `/api/lite/persona-builder` for real PersonaProfile/PersonaProfileVersion state, including profile readiness, resolve-preview simulation, release lifecycle, publish/rollback linkage and runtime persona-context evidence. Dedicated submit-review/approval/release-window commands and a standalone runtime evidence query endpoint remain missing and are marked as not implemented.

## Next PR

The next PR should continue template-block parity with Knowledge Studio conflict/golden-test commands, Persona approval/release-window commands, or remaining Operations governance write endpoints as the next likely cut after this Persona Builder read-model lands.

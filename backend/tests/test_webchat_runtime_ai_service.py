...
async def test_runtime_routes_request_through_generic_agent(monkeypatch) -> None:
    settings = runtime_service.get_webchat_runtime_settings()
    monkeypatch.setattr(
        runtime_service,
        "get_webchat_runtime_settings",
        lambda: replace(settings, enabled=True),
    )
    captured = {}
...
async def test_disabled_runtime_still_produces_visible_terminal_reply(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        runtime_service,
        "get_webchat_runtime_settings",
        lambda: replace(settings, enabled=False),
    )

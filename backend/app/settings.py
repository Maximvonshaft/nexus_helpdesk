...
class Settings:
    ...
    HTTP_PROCESS_ROLES = {"unspecified", "web"}
    ...
    def __init__(self) -> None:
        ...
        self.process_role = (
            os.getenv("NEXUS_PROCESS_ROLE", "unspecified").strip().lower()
            or "unspecified"
        )
        default_app_version = (
            "unknown" if self.app_env == "production" else "development"
        )
        self.app_version = (
            os.getenv("APP_VERSION", "").strip() or default_app_version
        )
        self.expected_migration_head = (
            os.getenv("EXPECTED_MIGRATION_HEAD", "").strip() or None
        )
        ...
        self.database_echo = (
            os.getenv("DATABASE_ECHO", "false").strip().lower() == "true"
        )
        default_tenant_authority_mode = (
            "enforce" if self.app_env == "production" else "shadow"
        )
        ...
        self.allow_dev_auth_requested = self._is_truthy(
            self.allow_dev_auth_raw
        )
        self.allow_dev_auth = (
            self.allow_dev_auth_requested
            and self.app_env in {"development", "test", "local"}
        )

        self.auto_init_db = (
            os.getenv("AUTO_INIT_DB", "false").strip().lower() == "true"
        ...
        self.storage_backend = (
            os.getenv("STORAGE_BACKEND", "local").strip().lower()
            or "local"
        )
        default_upload_root = self.backend_root / "uploads"
        self.upload_root = Path(
            os.getenv("UPLOAD_ROOT", str(default_upload_root))
        ).resolve()
        ...
        self.s3_presign_expiry_seconds = int(
            os.getenv("S3_PRESIGN_EXPIRY_SECONDS", "900")
        )
        ...
        self.allowed_upload_extensions = {
            ext.lower()
            for ext in self._parse_csv(
                os.getenv(
                    "ALLOWED_UPLOAD_EXTENSIONS",
                    ".jpg,.jpeg,.png,.webp,.pdf,.txt,.md,.markdown,.csv,"
                    ".html,.htm,.docx,.xlsx",
                )
            )
        }
        ...
        self.outbound_email_test_send_max_age_hours = int(
            os.getenv("OUTBOUND_EMAIL_TEST_SEND_MAX_AGE_HOURS", "24")
        )
        self.outbox_batch_size = int(os.getenv("OUTBOX_BATCH_SIZE", "50"))
        self.outbox_lock_seconds = int(
            os.getenv("OUTBOX_LOCK_SECONDS", "300")
        )
        self.outbox_max_retries = int(
            os.getenv("OUTBOX_MAX_RETRIES", "3")
        )
        self.allow_legacy_originless_outbound = _env_bool(
            "ALLOW_LEGACY_ORIGINLESS_OUTBOUND",
            False,
        )
        self.email_mailbox_sync_enabled = _env_bool(
            "EMAIL_MAILBOX_SYNC_ENABLED",
            True,
        )
        self.email_mailbox_sync_interval_seconds = int(
            os.getenv("EMAIL_MAILBOX_SYNC_INTERVAL_SECONDS", "60")
        )
        self.email_mailbox_sync_batch_size = int(
            os.getenv("EMAIL_MAILBOX_SYNC_BATCH_SIZE", "20")
        )
        ...
        self.worker_poll_seconds = float(
            os.getenv("WORKER_POLL_SECONDS", "2")
        )
        self.webchat_ai_worker_poll_seconds = float(
            os.getenv("WEBCHAT_AI_WORKER_POLL_SECONDS", "0.35")
        )
        ...
        self.require_prometheus_client_in_production = (
            os.getenv(
                "REQUIRE_PROMETHEUS_CLIENT_IN_PRODUCTION",
                "false",
            )
            .strip()
            .lower()
        ...
        self.runtime_contract_signing_secret = os.getenv(
            "RUNTIME_CONTRACT_SIGNING_SECRET",
            "",
        ).strip()

        self.login_max_failures = int(
            os.getenv("LOGIN_MAX_FAILURES", "5")
        )
        self.login_lock_minutes = int(
            os.getenv("LOGIN_LOCK_MINUTES", "15")
        )
        self.trusted_proxy_ips = self._parse_csv(
            os.getenv("TRUSTED_PROXY_IPS", "")
        )

        self.allow_legacy_integration_api_key = (
            os.getenv("ALLOW_LEGACY_INTEGRATION_API_KEY", "false")
            .strip()
            .lower()
            == "true"
        )
        self.integration_api_key = os.getenv("INTEGRATION_API_KEY")
        self.integration_default_rate_limit_per_minute = int(
            os.getenv("INTEGRATION_DEFAULT_RATE_LIMIT_PER_MINUTE", "60")
        )
        self.integration_require_idempotency_key = (
            os.getenv("INTEGRATION_REQUIRE_IDEMPOTENCY_KEY", "true")
            .strip()
            .lower()
            == "true"
        )

        self.webchat_allowed_origins = self._parse_csv(
            os.getenv("WEBCHAT_ALLOWED_ORIGINS", "")
        )
        self.webchat_allow_no_origin = (
            os.getenv("WEBCHAT_ALLOW_NO_ORIGIN", "false")
            .strip()
            .lower()
            == "true"
        )
        self.webchat_allow_legacy_token_transport = (
            os.getenv("WEBCHAT_ALLOW_LEGACY_TOKEN_TRANSPORT", "false")
            .strip()
            .lower()
            == "true"
        )
        self.webchat_rate_limit_backend = (
            os.getenv(
                "WEBCHAT_RATE_LIMIT_BACKEND",
                "database" if self.app_env == "production" else "memory",
            ).strip().lower()
            or "database"
        )
        self.webchat_rate_limit_window_seconds = int(
            os.getenv("WEBCHAT_RATE_LIMIT_WINDOW_SECONDS", "60")
        )
        self.webchat_rate_limit_max_requests = int(
            os.getenv("WEBCHAT_RATE_LIMIT_MAX_REQUESTS", "20")
        )
        self.admin_action_rate_limit_enabled = (
            os.getenv("ADMIN_ACTION_RATE_LIMIT_ENABLED", "true")
            .strip()
            .lower()
            == "true"
        )
        self.admin_action_rate_limit_window_seconds = int(
            os.getenv("ADMIN_ACTION_RATE_LIMIT_WINDOW_SECONDS", "60")
        )
        self.admin_action_rate_limit_single_max = int(
            os.getenv("ADMIN_ACTION_RATE_LIMIT_SINGLE_MAX", "20")
        )
        self.admin_action_rate_limit_batch_max = int(
            os.getenv("ADMIN_ACTION_RATE_LIMIT_BATCH_MAX", "3")
        )
        self.admin_action_rate_limit_consume_once_max = int(
            os.getenv("ADMIN_ACTION_RATE_LIMIT_CONSUME_ONCE_MAX", "5")
        )
        self.webchat_ai_enabled = _env_bool("WEBCHAT_AI_ENABLED", True)
        self.provider_runtime_enabled = _env_bool(
            "PROVIDER_RUNTIME_ENABLED",
            False,
        )
        self.private_ai_runtime_enabled = _env_bool(
            "PRIVATE_AI_RUNTIME_ENABLED",
            False,
        )
        self.webchat_ai_auto_reply_mode = (
            os.getenv("WEBCHAT_AI_AUTO_REPLY_MODE", "runtime")
            .strip()
            .lower()
            or "runtime"
        )
        self.webchat_ai_turn_debounce_seconds = float(
            os.getenv("WEBCHAT_AI_TURN_DEBOUNCE_SECONDS", "0.15")
        )
        self.webchat_ai_reconciler_enabled = _env_bool(
            "WEBCHAT_AI_RECONCILER_ENABLED",
            True,
        )
        try:
        ...
        self.webchat_knowledge_reply_mode = (
            os.getenv("WEBCHAT_KNOWLEDGE_REPLY_MODE", "ai_grounded")
            .strip()
            .lower()
            or "ai_grounded"
        )
        self.webchat_knowledge_no_evidence_fallback_enabled = _env_bool(
            "WEBCHAT_KNOWLEDGE_NO_EVIDENCE_FALLBACK_ENABLED",
            True,
        )
        self.knowledge_embeddings_enabled = _env_bool(
        ...
        self.knowledge_embedding_model = os.getenv(
            "KNOWLEDGE_EMBEDDING_MODEL",
            "nexus-deterministic-hash-v1",
        ).strip()
        self.knowledge_embedding_dim = int(
            os.getenv("KNOWLEDGE_EMBEDDING_DIM", "1024")
        )
        self.knowledge_embedding_batch_size = int(
            os.getenv("KNOWLEDGE_EMBEDDING_BATCH_SIZE", "32")
        )
        self.knowledge_embedding_timeout_seconds = int(
            os.getenv("KNOWLEDGE_EMBEDDING_TIMEOUT_SECONDS", "20")
        )
        self.knowledge_vector_fallback_allowed = _env_bool(
            "KNOWLEDGE_VECTOR_FALLBACK_ALLOWED",
            True,
        )
        self.knowledge_embedding_base_url = os.getenv(
            "KNOWLEDGE_EMBEDDING_BASE_URL",
            "https://api.openai.com/v1",
        ).strip().rstrip("/")
        self.knowledge_embedding_api_key = (
            os.getenv("KNOWLEDGE_EMBEDDING_API_KEY", "").strip() or None
        )
        self.knowledge_embedding_api_key_file = (
            os.getenv("KNOWLEDGE_EMBEDDING_API_KEY_FILE", "").strip()
            or None
        )
        self.webchat_tracking_fact_lookup_enabled = (
            os.getenv("WEBCHAT_TRACKING_FACT_LOOKUP_ENABLED", "false")
            .strip()
            .lower()
            == "true"
        )
        self.webchat_tracking_fact_source = (
            os.getenv("WEBCHAT_TRACKING_FACT_SOURCE", "speedaf_api")
            .strip()
            .lower()
            or "speedaf_api"
        )
        self.webchat_tracking_fact_timeout_seconds = int(
            os.getenv("WEBCHAT_TRACKING_FACT_TIMEOUT_SECONDS", "8")
        )
        self.webchat_tracking_fact_redaction_enabled = (
            os.getenv("WEBCHAT_TRACKING_FACT_REDACTION_ENABLED", "true")
            .strip()
            .lower()
            == "true"
        )
        self.webchat_tracking_fact_card_enabled = (
            os.getenv("WEBCHAT_TRACKING_FACT_CARD_ENABLED", "false")
            .strip()
            .lower()
            == "true"
        )
        self.webchat_ai_session_ttl_hours = int(
            os.getenv("WEBCHAT_AI_SESSION_TTL_HOURS", "24")
        )
        self.webchat_ai_session_max_messages = int(
            os.getenv("WEBCHAT_AI_SESSION_MAX_MESSAGES", "40")
        )
        ...
        self.webchat_ws_public_enabled = _env_bool(
            ...
            self.webchat_ws_enabled,
        )
        self.webchat_ws_admin_enabled = _env_bool(
            "WEBCHAT_WS_ADMIN_ENABLED",
            self.webchat_ws_enabled,
        )
        self.webchat_ws_broker = (
            os.getenv("WEBCHAT_WS_BROKER", "database").strip().lower()
            or "database"
        )
        self.webchat_ws_replay_poll_ms = int(
            os.getenv("WEBCHAT_WS_REPLAY_POLL_MS", "500")
        )
        self.webchat_ws_fallback_poll_ms = int(
            os.getenv("WEBCHAT_WS_FALLBACK_POLL_MS", "4000")
        )
        self.webchat_ws_heartbeat_ms = int(
            os.getenv("WEBCHAT_WS_HEARTBEAT_MS", "25000")
        )
        self.webchat_ws_hello_timeout_ms = int(
            os.getenv("WEBCHAT_WS_HELLO_TIMEOUT_MS", "5000")
        )
        self.webchat_ws_max_connections = int(
            os.getenv("WEBCHAT_WS_MAX_CONNECTIONS", "1000")
        )
        self.webchat_ws_max_connections_per_user = int(
            os.getenv("WEBCHAT_WS_MAX_CONNECTIONS_PER_USER", "10")
        )

        self.request_id_header = os.getenv(
            "REQUEST_ID_HEADER",
            "X-Request-Id",
        )
        self.log_json = (
            os.getenv("LOG_JSON", "true").strip().lower() == "true"
        )
        self.metrics_enabled = (
            os.getenv("METRICS_ENABLED", "false").strip().lower()
            == "true"
        )
    ...
    def is_http_process(self) -> bool:
        return self.process_role in self.HTTP_PROCESS_ROLES

    @property
    def is_ai_capable_process(self) -> bool:
        return self.process_role in self.AI_CAPABLE_PROCESS_ROLES

    def _normalize(self) -> None:
        self.upload_root.mkdir(parents=True, exist_ok=True)
        if self.process_role not in self.VALID_PROCESS_ROLES:
            raise RuntimeError("NEXUS_PROCESS_ROLE is not supported")
        if self.webchat_rate_limit_backend not in {"memory", "database"}:
            raise RuntimeError(
                "WEBCHAT_RATE_LIMIT_BACKEND must be memory or database"
            )
        if not 1 <= self.admin_action_rate_limit_window_seconds <= 3600:
            raise RuntimeError(
                "ADMIN_ACTION_RATE_LIMIT_WINDOW_SECONDS must be between 1 "
                "and 3600"
            )
        if not 1 <= self.admin_action_rate_limit_single_max <= 1000:
            raise RuntimeError(
                "ADMIN_ACTION_RATE_LIMIT_SINGLE_MAX must be between 1 and 1000"
            )
        if not 1 <= self.admin_action_rate_limit_batch_max <= 1000:
            raise RuntimeError(
                "ADMIN_ACTION_RATE_LIMIT_BATCH_MAX must be between 1 and 1000"
            )
        if not 1 <= self.admin_action_rate_limit_consume_once_max <= 1000:
            raise RuntimeError(
                "ADMIN_ACTION_RATE_LIMIT_CONSUME_ONCE_MAX must be between 1 "
                "and 1000"
            )
        if self.webchat_ai_auto_reply_mode not in {"off", "runtime"}:
            raise RuntimeError(
                "WEBCHAT_AI_AUTO_REPLY_MODE must be off or runtime"
            )
        if not 0.1 <= self.worker_poll_seconds <= 60:
            raise RuntimeError(
                "WORKER_POLL_SECONDS must be between 0.1 and 60"
            )
        if not 0.05 <= self.webchat_ai_worker_poll_seconds <= 10:
            raise RuntimeError(
                "WEBCHAT_AI_WORKER_POLL_SECONDS must be between 0.05 and 10"
            )
        if not 0.01 <= self.webchat_ai_worker_busy_poll_seconds <= 5:
            raise RuntimeError(
                "WEBCHAT_AI_WORKER_BUSY_POLL_SECONDS must be between 0.01 "
                "and 5"
            )
        if not 0 <= self.webchat_ai_turn_debounce_seconds <= 10:
            raise RuntimeError(
                "WEBCHAT_AI_TURN_DEBOUNCE_SECONDS must be between 0 and 10"
            )
        if self.webchat_knowledge_reply_mode not in {
            "ai_grounded",
            "deterministic_direct_answer",
        }:
            raise RuntimeError(
                "WEBCHAT_KNOWLEDGE_REPLY_MODE must be ai_grounded or "
                "deterministic_direct_answer"
            )
        if not 8 <= self.knowledge_embedding_dim <= 4096:
            raise RuntimeError(
                "KNOWLEDGE_EMBEDDING_DIM must be between 8 and 4096"
            )
        if not 1 <= self.knowledge_embedding_batch_size <= 512:
            raise RuntimeError(
                "KNOWLEDGE_EMBEDDING_BATCH_SIZE must be between 1 and 512"
            )
        if not 1 <= self.knowledge_embedding_timeout_seconds <= 120:
            raise RuntimeError(
                "KNOWLEDGE_EMBEDDING_TIMEOUT_SECONDS must be between 1 and 120"
            )
        if self.knowledge_embedding_provider not in {
            "deterministic_hash",
            "hash",
            "test",
            "openai_compatible",
            ...
            raise RuntimeError(
                "KNOWLEDGE_EMBEDDING_PROVIDER must be deterministic_hash, "
                "hash, test, or openai_compatible"
            )
        if self.webchat_tracking_fact_source not in {
            "speedaf_api",
            "speedaf_track_query",
            "speedaf_hybrid",
        }:
            raise RuntimeError(
                "WEBCHAT_TRACKING_FACT_SOURCE must be speedaf_api, "
                "speedaf_track_query, or speedaf_hybrid"
            )
        if not 1 <= self.webchat_tracking_fact_timeout_seconds <= 30:
            raise RuntimeError(
                "WEBCHAT_TRACKING_FACT_TIMEOUT_SECONDS must be between 1 "
                "and 30"
            )
        if not 1 <= self.webchat_ai_session_ttl_hours <= 168:
            raise RuntimeError(
                "WEBCHAT_AI_SESSION_TTL_HOURS must be between 1 and 168"
            )
        if not 4 <= self.webchat_ai_session_max_messages <= 200:
            raise RuntimeError(
                "WEBCHAT_AI_SESSION_MAX_MESSAGES must be between 4 and 200"
            )
        if not 1 <= self.webchat_ai_session_summary_messages <= 20:
            raise RuntimeError(
                "WEBCHAT_AI_SESSION_SUMMARY_MESSAGES must be between 1 and 20"
            )
        if self.webchat_ws_broker not in {"database", "memory"}:
            raise RuntimeError("WEBCHAT_WS_BROKER must be database or memory")
        if not 100 <= self.webchat_ws_replay_poll_ms <= 10000:
            raise RuntimeError(
                "WEBCHAT_WS_REPLAY_POLL_MS must be between 100 and 10000"
            )
        if not 1000 <= self.webchat_ws_fallback_poll_ms <= 60000:
            raise RuntimeError(
                "WEBCHAT_WS_FALLBACK_POLL_MS must be between 1000 and 60000"
            )
        if not 5000 <= self.webchat_ws_heartbeat_ms <= 120000:
            raise RuntimeError(
                "WEBCHAT_WS_HEARTBEAT_MS must be between 5000 and 120000"
            )
        if not 1000 <= self.webchat_ws_hello_timeout_ms <= 30000:
            raise RuntimeError(
                "WEBCHAT_WS_HELLO_TIMEOUT_MS must be between 1000 and 30000"
            )
        if not 1 <= self.webchat_ws_max_connections <= 100000:
            raise RuntimeError(
                "WEBCHAT_WS_MAX_CONNECTIONS must be between 1 and 100000"
            )
        if not 1 <= self.webchat_ws_max_connections_per_user <= 1000:
            raise RuntimeError(
                "WEBCHAT_WS_MAX_CONNECTIONS_PER_USER must be between 1 and 1000"
            )
        if not 1 <= self.outbound_email_test_send_max_age_hours <= 168:
            raise RuntimeError(
        ...
        if not 1 <= self.email_mailbox_sync_batch_size <= 100:
            raise RuntimeError(
                "EMAIL_MAILBOX_SYNC_BATCH_SIZE must be between 1 and 100"
            )
        if (
            self.webchat_tracking_fact_lookup_enabled
            and not self.webchat_tracking_fact_redaction_enabled
        ):
            raise RuntimeError(
                "WEBCHAT_TRACKING_FACT_REDACTION_ENABLED must be true when "
                "tracking lookup is enabled"
            )
    ...
    def _validate_production(self) -> None:
        if self.tenant_runtime_authority_mode != "enforce":
            raise RuntimeError(
                "TENANT_RUNTIME_AUTHORITY_MODE=enforce is required in production"
            )
        if not self.is_postgres:
            raise RuntimeError("Production requires a PostgreSQL DATABASE_URL")
        if self.auto_init_db or self.seed_demo_data:
            raise RuntimeError(
                "AUTO_INIT_DB and SEED_DEMO_DATA must be disabled in production"
            )
        ...
        if self.allow_legacy_integration_api_key:
            raise RuntimeError(
                "ALLOW_LEGACY_INTEGRATION_API_KEY must be disabled in production"
            )
        ...
        if self.is_http_process:
            ...
            if (
                normalized_secret in weak_secrets
                or self.jwt_secret_key.startswith("dev-only-")
            ):
                raise RuntimeError(
                    "SECRET_KEY must be a non-placeholder production secret"
                )
            localhost_origins = {
                "http://localhost",
                "http://127.0.0.1",
            ...
            if not self.frontend_dist_available:
                raise RuntimeError(
                    "frontend_dist/index.html must exist in the production "
                    "Web process"
                )

        ai_execution_requested = self.is_ai_capable_process and (
            self.webchat_ai_enabled
            or self.provider_runtime_enabled
            or self.private_ai_runtime_enabled
            or self.webchat_ai_auto_reply_mode != "off"
        )
        if ai_execution_requested and not self.knowledge_embeddings_enabled:
            raise RuntimeError(
                "KNOWLEDGE_EMBEDDINGS_ENABLED=true is required when a "
                "production AI execution path is enabled"
            )
        if self.is_ai_capable_process and self.knowledge_embeddings_enabled:
            if self.knowledge_embedding_provider in {
                "deterministic_hash",
                "hash",
                "test",
            }:
                raise RuntimeError(
                    "Production Knowledge Runtime requires a real embedding "
                    "provider when embeddings are enabled"
                )
            if (
                self.knowledge_embedding_provider == "openai_compatible"
                and not (
                    self.knowledge_embedding_api_key
                    or self.knowledge_embedding_api_key_file
                )
            ):
                raise RuntimeError(
                    "KNOWLEDGE_EMBEDDING_API_KEY_FILE or "
                    "KNOWLEDGE_EMBEDDING_API_KEY is required for enabled "

...
PR-0/PR-1 does not make WebCall AI functional yet. It only adds the guarded architecture, schema, config, and tests. No real AI voice, STT, TTS, LiveKit AI worker, or Speedaf write automation is introduced here.
...
WebCall remains the voice channel. LiveKit remains the real-time media room. In a later PR, a backend AI worker will join the LiveKit room as an AI participant. NexusDesk remains the control plane and system of record for state, facts, action decisions, audit, evidence, and human handoff.
...
2. PR-2: webcall-ai-worker skeleton and AI session claim lifecycle only.
...
This foundation does not implement real STT. It does not implement real TTS. It also does not implement a real LiveKit AI participant join, real OpenClaw/LLM voice calls, frontend WebCall UI changes, AI handoff UI, or any Speedaf write action execution from AI.

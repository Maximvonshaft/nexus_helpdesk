...
test('webcall non-livekit accept path cleans up acquired microphone track', () => {
  const nonLivekitBranch = /if \(accepted\.provider !== 'livekit'\) \{[\s\S]*?return accepted\n\s*\}/.exec(agentWebCallPanel)?.[0] || ''
  assert.match(nonLivekitBranch, /await cleanupRoom\(\)/)
})

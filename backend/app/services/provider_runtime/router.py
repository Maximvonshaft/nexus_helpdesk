...
class ProviderRuntimeRouter:
    ...
    async def route(self, request: ProviderRequest) -> ProviderResult:
        ...
        for provider_name in providers:
            health_decision = ProviderRuntimeHealth.should_skip(
                provider_name
            )
            if health_decision.skip:
                self._write_audit(
                    ...
                    _traffic_summary(
                        ...
                        {
                            "provider_health":
                            health_decision.safe_summary()
                        },
                    ),
                    health_decision.reason
                    or "provider_health_skip",
                )
                continue
            ...
            if not result.ok:
                error_code = _bounded_provider_error_code(
                    result.error_code
                )
            ...
            health_event = ProviderRuntimeHealth.record_success(
                provider_name
            )
            if health_event:
                safe_summary["provider_health"] = health_event
...
def _effective_configuration(
    ...
    if any(
        provider not in _APPROVED_PROVIDERS
    ...
    return {
        ...
        "canary_percent": effective_canary_percent(
            raw["canary_percent"]
        ),
    }

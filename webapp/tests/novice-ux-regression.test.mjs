...
const qaTraining = read('src/routes/qa-training.tsx')
...
test('qa training loop exposes guided scorecard and knowledge gap workflow', () => {
  assert.match(qaTraining, /<GuidedWorkflow steps=/)
  assert.match(qaTraining, /QA Queue \/ Scorecard/)
  assert.match(qaTraining, /Scorecard \/ Coaching Feedback/)
  assert.match(qaTraining, /Training \/ Knowledge Gap Closed Loop/)
  assert.match(qaTraining, /保存评分并生成培训任务/)
  assert.match(qaTraining, /data-testid="qa-training-queue"/)
  assert.match(qaTraining, /data-testid="qa-scorecard"/)
  assert.match(qaTraining, /data-testid="qa-training-tasks"/)
  assert.match(qaTraining, /<ErrorSummary/)
  assert.match(qaTraining, /<EmptyState/)
  assert.match(qaTraining, /<TechnicalDetails title="QA 后端契约"/)
  assert.doesNotMatch(qaTraining, /\bfetch\s*\(/)
})

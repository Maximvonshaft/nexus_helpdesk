...
def _traffic_routing_rules(db: Session) -> dict[str, Any]:
    try:
        ...
        return {"status": "unavailable", "reason_code": "provider_runtime_routing_rules_unavailable", "items": [], "truncated": False}
    truncated = len(rows) > 100
    output: list[dict[str, Any]] = []
    invalid_rule_found = False
    for row in rows[:100]:
        selection = safe_traffic_configuration(default_canary_percent=row["canary_percent"], default_kill_switch=row["kill_switch"])
    ...
    return {"status": "misconfigured" if invalid_rule_found else "ready", "reason_code": "provider_runtime_routing_rule_invalid" if invalid_rule_found else None, "items": output, "truncated": truncated}
...
def provider_runtime_audit_recent(request_id: str | None = None, limit: int = 20, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ensure_can_manage_runtime(current_user, db)
    ...
    for row in rows:
        ...
        if isinstance(safe_summary, str):
            try:
                ...
                safe_summary = {}
        items.append({"id": row["id"], "tenant_id": row["tenant_id"], "provider": row["provider"], "request_id": row["request_id"], "channel_key": row["channel_key"], "session_id": row["session_id"], "operation": row["operation"], "status": row["status"], "safe_summary": safe_summary if isinstance(safe_summary, dict) else {}, "error_code": row["error_code"], "elapsed_ms": row["elapsed_ms"], "created_at": row["created_at"].isoformat() if hasattr(row["created_at"], "isoformat") else row["created_at"]})
    return {"items": items, "total": len(items)}

@router.patch("/routing/webchat-runtime")
def update_webchat_runtime_routing(payload: WebchatRuntimeRoutingUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ensure_can_manage_runtime(current_user, db)
    try:
        payload.validate_allowed()
    except ValueError:
        error_code = "primary_provider_not_allowed" if payload.primary_provider not in _ALLOWED_PRIMARY_PROVIDERS else "fallback_provider_not_allowed"
        raise HTTPException(status_code=400, detail={"error_code": error_code}) from None
    existing = db.execute(text("""
        SELECT id FROM provider_routing_rules
        WHERE tenant_id = :tenant_id AND channel_key = :channel_key AND scenario = :scenario
        LIMIT 1
    """), {"tenant_id": payload.tenant_id, "channel_key": payload.channel_key, "scenario": _WEBCHAT_RUNTIME_SCENARIO}).mappings().first()
    params = {"id": existing["id"] if existing else str(uuid.uuid4()), "tenant_id": payload.tenant_id, "channel_key": payload.channel_key, "scenario": _WEBCHAT_RUNTIME_SCENARIO, "primary_provider": payload.primary_provider, "fallback_providers": json.dumps(payload.fallback_providers, separators=(",", ":")), "output_contract": _WEBCHAT_RUNTIME_OUTPUT_CONTRACT, "timeout_ms": payload.timeout_ms, "canary_percent": payload.canary_percent, "kill_switch": payload.kill_switch, "enabled": payload.enabled}
    if existing:
        db.execute(text("""
            UPDATE provider_routing_rules SET primary_provider=:primary_provider, fallback_providers=:fallback_providers,
            output_contract=:output_contract, timeout_ms=:timeout_ms, canary_percent=:canary_percent,
            kill_switch=:kill_switch, enabled=:enabled, updated_at=CURRENT_TIMESTAMP WHERE id=:id
        """), params)
    else:
        db.execute(text("""
            INSERT INTO provider_routing_rules (id,tenant_id,channel_key,scenario,primary_provider,fallback_providers,output_contract,timeout_ms,canary_percent,kill_switch,enabled,created_at,updated_at)
            VALUES (:id,:tenant_id,:channel_key,:scenario,:primary_provider,:fallback_providers,:output_contract,:timeout_ms,:canary_percent,:kill_switch,:enabled,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
        """), params)
    db.commit()
    return {"ok": True, "routing_rule": {**params, "fallback_providers": payload.fallback_providers, "traffic_selection": safe_traffic_configuration(default_canary_percent=payload.canary_percent, default_kill_switch=payload.kill_switch)}}

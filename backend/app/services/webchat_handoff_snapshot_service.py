...
def _snapshot_issue_key(snapshot: dict[str, Any]) -> str:
    ...
    intent = _clip(snapshot.get("intent"), 80) or _clip(snapshot.get("handoff_reason"), 80) or "handoff"
...
def enqueue_webchat_handoff_snapshot_job(db: Session, *, snapshot: dict[str, Any]) -> BackgroundJob:
    ...
    return enqueue_background_job(db, queue_name="webchat_handoff_snapshot", job_type=WEBCHAT_HANDOFF_SNAPSHOT_JOB, payload={"snapshot": snapshot}, dedupe_key=dedupe_key)


def _existing_ticket(db: Session, snapshot: dict[str, Any], source_dedupe_key: str) -> Ticket | None:
    ticket = db.execute(select(Ticket).where(Ticket.source_dedupe_key == source_dedupe_key, Ticket.status.in_(ACTIVE_TICKET_STATUSES)).limit(1)).scalar_one_or_none()
    if ticket is not None:
        return ticket
    tracking = _clip(snapshot.get("tracking_number"), 120)
    if tracking:
        return db.execute(select(Ticket).where(Ticket.tracking_number == tracking, Ticket.source_channel == SourceChannel.web_chat, Ticket.status.in_(ACTIVE_TICKET_STATUSES)).limit(1)).scalar_one_or_none()
    return None
...
def _find_or_create_customer(db: Session, *, snapshot: dict[str, Any], public_id: str) -> Customer:
    ...
    external_ref = _clip(visitor.get("external_ref") or visitor.get("visitor_ref") or snapshot.get("session_id") or public_id, 120)
...
def process_webchat_handoff_snapshot_job(db: Session, *, snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"status": "done", "ticket_id": ticket.id, "ticket_no": ticket.ticket_no, "public_conversation_id": conversation.public_id if conversation is not None else None}

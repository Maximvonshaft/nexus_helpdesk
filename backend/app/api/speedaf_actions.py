...
class SpeedafWorkOrderRequest(BaseModel):
    ...
    description: str = Field(
        min_length=1,
        max_length=WORK_ORDER_INPUT_DESCRIPTION_MAX_LENGTH,
    )
...
def _require_provider_processing(ticket_id: int) -> None:
    try:
        ensure_ticket_processing_allowed_fresh(
            ticket_id=ticket_id,
...
def _address_dedupe_key(
    *,
    ...
    return (
        f"speedaf-update-address:ticket:{ticket_id}:"
        f"waybill:{_hash_short(waybill_code)}:phone:{_hash_short(whatsapp_phone)}"
    )


def _reserve_address_update(
    db: Session,
    *,
    dedupe_key: str,
    ticket_id: int,
...
@router.post(
    "/{ticket_id}/speedaf/waybills/query",
    response_model=SpeedafWaybillLookupResponse,
)
def query_speedaf_waybills(
    ticket_id: int,
    payload: SpeedafWaybillLookupRequest,
    request: Request,
    ...
    enforce_admin_action_rate_limit(
        ...
        action_key=WAYBILL_LOOKUP_ACTION_KEY,
    ...
    candidates = [
        SpeedafWaybillCandidateResponse(
...
def create_speedaf_work_order(
    ticket_id: int,
    payload: SpeedafWorkOrderRequest,
    request: Request,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    _require_feature(
        ...
        "speedaf_work_order_create_disabled",
    )
    ensure_can_create_speedaf_work_order(current_user, db)
    enforce_admin_action_rate_limit(
        db,
        actor_id=current_user.id,
        action_key=WORK_ORDER_ACTION_KEY,
    ...
    if not is_auto_work_order_type_allowed(work_order_type):
        raise HTTPException(
            ...
            detail="speedaf_work_order_type_not_allowed",
        )
    job = enqueue_speedaf_work_order_create_job(
        ...
        description=_clean(
            payload.description,
            limit=WORK_ORDER_SPEEDAF_DESCRIPTION_MAX_LENGTH,
        ),
    ...
    _append_event(
        ...
        payload={
            ...
            **safe_waybill_payload(payload.waybillCode),
            **safe_caller_payload(payload.callerID),
        },
    )
    db.commit()
    return SpeedafActionResponse(
        ok=True,
        status="queued",
...
def submit_speedaf_address_update(
    ticket_id: int,
    payload: SpeedafAddressUpdateRequest,
    request: Request,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    _require_feature(
        ...
        "speedaf_update_address_disabled",
    )
    ensure_can_update_speedaf_address(current_user, db)
    enforce_admin_action_rate_limit(
        db,
        actor_id=current_user.id,
        action_key=ADDRESS_UPDATE_ACTION_KEY,
        max_requests=get_settings().admin_action_rate_limit_batch_max,
        request_id=_request_id(request),
    )
    _load_visible_ticket(db, ticket_id=ticket_id, user=current_user)
    ...
    dedupe_key = _address_dedupe_key(
        ticket_id=ticket_id,
        waybill_code=waybill,
        whatsapp_phone=phone,
    )
    request_id = _request_id(request)
    _reserve_address_update(
        db,
        dedupe_key=dedupe_key,
        ticket_id=ticket_id,
        waybill_code=waybill,
    ...
    job = enqueue_speedaf_address_update_job(
        db,
        ticket_id=ticket_id,
        waybill_code=waybill,
        caller_id=caller,
    ...
    _append_event(
        ...
        new_value="queued",
        note=(
            "Speedaf address update confirmation request queued. Final address "
            "change remains pending Speedaf/customer confirmation."
        ),
        payload={
            "job_id": job.id,
            "dedupe_key": dedupe_key,
            **safe_waybill_payload(waybill),
    ...
    return SpeedafActionResponse(
        ...
        status="queued",
        message=(
            "Address update confirmation request queued. Final address change "
            "remains pending Speedaf/customer confirmation."
        ),
        jobId=job.id,

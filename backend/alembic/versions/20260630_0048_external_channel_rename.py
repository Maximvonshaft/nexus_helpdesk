...
def _column_type_map(inspector: sa.Inspector, table_name: str) -> dict[str, object]:
    try:
...
def _postgres_json_cast(bind: sa.engine.Connection, column_type: object) -> str | None:
    if bind.dialect.name != "postgresql":
        return None
    class_name = column_type.__class__.__name__.lower()
    try:
        rendered = column_type.compile(dialect=bind.dialect).lower()  # type: ignore[attr-defined]
    except Exception:
        rendered = str(column_type).lower()
    if class_name == "jsonb" or rendered == "jsonb":
        return "jsonb"
    if class_name == "json" or rendered == "json":
        return "json"
    return None
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conrelid = to_regclass('{source_table}')
          AND conname = '{source}'
    ) THEN
        ALTER TABLE "{source_table}" RENAME CONSTRAINT "{source}" TO "{target}";
    END IF;
END $$;
"""
            )
...
def _replace_column_values(inspector: sa.Inspector, *, reverse: bool = False) -> None:
    ...
    for table_name, columns in VALUE_COLUMNS.items():
        if table_name not in existing_tables:
            continue
        column_types = _column_type_map(inspector, table_name)
        for column in columns:
            if column not in column_types:
                continue
            json_cast = _postgres_json_cast(bind, column_types[column])
            for old_value, new_value in replacements:
                ...
                if json_cast:
                    bind.execute(
                        sa.text(
                            f'UPDATE "{table_name}" '
                            f'SET "{column}" = CAST(replace("{column}"::text, :old_value, :new_value) AS {json_cast}) '
                            f'WHERE "{column}"::text LIKE :pattern'
                        ),
                        params,
                    )
                else:
...
def downgrade() -> None:
    # Historical migrations in this branch are normalized to external_channel
    # names. Reintroducing retired vendor names here breaks older downgrade
    # scripts and violates the cleanup contract, so this compatibility rename
    # intentionally remains forward-only.
    return None

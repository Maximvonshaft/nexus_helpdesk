...
def codex_direct_smoke(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    # Readiness-only smoke. It never returns or reads token content; it checks
    # the Codex CLI binary, the presence of HOME/.codex/auth.json, and a safe
    # `codex login status` result.
    return CodexDirectAdapter().smoke_check()

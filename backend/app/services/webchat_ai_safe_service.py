...
def _request_handoff_without_public_reply(
    ...
    *,
    ...
    if suppress_stale_reply_if_needed(db, conversation=conversation, turn=turn, reason="newer_message_before_handoff_request"):
        return {"status": "superseded", "reason": "newer_message_before_handoff_request", "reply_source": "suppressed"}
    ...
    LOGGER.info(
        "webchat_ai_reply_suppressed_handoff_requested",
        extra={"event_payload": {"conversation_id": conversation.id, "ticket_id": ticket.id, "visitor_message_id": visitor_message.id, "ai_turn_id": turn.id if turn else None, "reason": reason}},
    )
    return {
...
def _complete_turn_if_present(db: Session, *, conversation: WebchatConversation, turn: WebchatAITurn | None, result: dict[str, Any]) -> None:
    if turn is None:
        return
    if turn.status in AI_TURN_TERMINAL_STATUSES or result.get("status") == "handoff_requested":
        return
...
def process_webchat_ai_reply_job(db: Session, *, conversation_id: int, ticket_id: int, visitor_message_id: int) -> dict[str, Any]:
    ...
    if mode == "safe_ack":
        return _request_handoff_without_public_reply(
            ...
            turn=turn,
            reason="webchat_safe_ack_suppressed",
    ...
    if mode == "safe_ai" and _has_high_risk_intent(visitor_message.body):
        return _request_handoff_without_public_reply(
            ...
            visitor_message=visitor_message,
            turn=turn,
            reason="webchat_safe_ai_high_risk_suppressed",
            recommended_agent_action="Review the high-risk customer request before any customer-visible reply.",
        )

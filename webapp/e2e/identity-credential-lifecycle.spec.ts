...
test('administrator forces rotation and revokes sessions from the one credential panel', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: '密码与登录' }).click()
  await expect(page.getByRole('heading', { level: 2, name: '密码与登录' })).toBeVisible()
  const row = page.getByRole('row', { name: /Agent One/ })

  await row.getByRole('button', { name: '要求改密' }).click()
  const forceDialog = page.getByRole('dialog', { name: '要求修改密码' })
  await forceDialog.getByRole('button', { name: '确认要求修改' }).click()
  await expect.poll(() => commands).toContain('require-password-change')

  await row.getByRole('button', { name: '退出设备' }).click()
  const revokeDialog = page.getByRole('dialog', { name: '退出所有设备' })
  await revokeDialog.getByRole('button', { name: '确认退出所有设备' }).click()

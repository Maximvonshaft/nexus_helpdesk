...
def test_status_flow_start_complete_and_terminal_immutable(db_session):
    ...
    completed = complete_onboarding_task(
        task.id,
        ChannelOnboardingTaskCompleteRequest(desired_channel_account_binding="wa-final"),
        db_session,
        admin,
    )
...
def test_fail_then_complete_task(db_session):
    ...
    completed = complete_onboarding_task(
        task.id,
        ChannelOnboardingTaskCompleteRequest(desired_channel_account_binding="recovered"),
        db_session,
        admin,
    )
    assert completed.status == "completed"

...
def _contains_forbidden(text: str) -> bool:
    ...
    return any(_normalize_visible(term) in normalized or term.lower() in compact for term in FORBIDDEN)
...
async def _probe_one(client: httpx.AsyncClient, base_url: str, seq: int) -> ProbeResult:
    ...
    try:
        async with client.stream(
            ...
            if response.status_code != 200 or "text/event-stream" not in response.headers.get("content-type", ""):
                ...
                return ProbeResult(fallback.status_code < 500 and bool(reply), elapsed, elapsed, leaks, False, True, data.get("error_code") if isinstance(data, dict) else None)
            stream_buffer = ""
        ...
        return ProbeResult(False, 0.0, (time.perf_counter() - started) * 1000, 0, False, False, type(exc).__name__)
    ...
    return ProbeResult(ok=bool(visible_text) and error_code is None, first_chunk_ms=first_chunk_ms or total_ms, total_ms=total_ms, raw_leak_count=raw_leak_count, replayed=replayed, fallback=False, error_code=error_code)
...
async def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout, limits=limits) as client:
        async def guarded(seq: int) -> ProbeResult:
            async with sem:
                return await _probe_one(client, base_url, seq)

...
function modeHelp(mode: SearchMode) {
  if (mode === 'waybill') return '按运单号、包裹号或 tracking number 查询关联工单。'
...
function CustomerSearchPage() {
  ...
  const grouped = useMemo(() => {
    const active = rows.filter((item) => !['resolved', 'closed', 'canceled', 'cancelled'].includes(String(item.status)))
    const closed = rows.filter((item) => ['resolved', 'closed', 'canceled', 'cancelled'].includes(String(item.status)))
  ...
  return (
    <AppShell>
      <PageHeader
        eyebrow="客户 / 运单查询"
        title="客户、运单与 CallerID 快查"
        description="给一线客服的一级查询入口。先查对象，再进入工单、WebChat、Email 或 WebCall 继续处理。当前版本复用后端工单搜索能力，不新造后端接口。"
      />
      <Card className="soft">
        ...
        <CardBody>
          <div className="workspace-toolbar customer-search-toolbar">
            ...
            <Field label="关键词" hint="至少输入 2 个字符；支持运单、客户、电话、工单标题或工单号。">
              <Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例如：CH020000006856 / +4179... / 客户姓名 / Ticket" />
            </Field>
          </div>
        </CardBody>
      </Card>

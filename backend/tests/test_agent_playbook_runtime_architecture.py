...
async def test_agent_loop_executes_tool_then_returns_final_reply(monkeypatch) -> None:
    ...
    monkeypatch.setattr(agent_runtime, "_runtime_policy", lambda *_args, **_kwargs: {"max_tool_rounds": 3, "allow_high_risk_writes": False, "allowed_tools": [], "provider_timeout_ms": 15000, "enabled": True})
    monkeypatch.setattr(agent_runtime, "prompt_playbook_catalog", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(agent_runtime.ProviderRuntimeRouter, "route", route)
    monkeypatch.setattr(agent_runtime, "_authoritative_provider_audit_exists", lambda *_args, **_kwargs: True)
...
async def test_agent_loop_fails_closed_when_tool_transaction_does_not_commit(monkeypatch) -> None:
    ...
    def execute(_db, *, calls, context, allow_high_risk_writes=False):
        ...
        return [ToolObservation(tool_name="speedaf.order.query", ok=True, status="executed", result={})]
...
def test_public_agent_access_policy_does_not_derive_grants_from_visible_tools(monkeypatch) -> None:
    monkeypatch.setenv("WEBCHAT_AGENT_ALLOWED_TOOLS", "knowledge.search,ticket.create")
...
def test_canonical_agent_runtime_has_no_static_skill_chain() -> None:
    ...
    runtime = (ROOT / "backend/app/services/agent_runtime/runtime.py").read_text(encoding="utf-8")
...
def test_canonical_executor_separates_execution_arguments_from_audit_projection() -> None:
    source = (ROOT / "backend/app/services/nexus_osr/tool_execution_service_core.py").read_text(encoding="utf-8")
    constructor = source.split("def runtime_tool_actions_from_tool_calls", 1)[1].split("def execute_controlled_tool_calls", 1)[0]
...
def test_provider_capability_contract_has_only_agent_turn() -> None:
    schemas = (ROOT / "backend/app/services/provider_runtime/schemas.py").read_text(encoding="utf-8")
    adapter = (ROOT / "backend/app/services/provider_runtime/adapters/private_ai_runtime.py").read_text(encoding="utf-8")
...
def test_canonical_executor_does_not_trust_model_idempotency_keys() -> None:
    source = (ROOT / "backend/app/services/nexus_osr/tool_execution_service_core.py").read_text(encoding="utf-8")
    function = source.split("def _idempotency_key_for_action", 1)[1].split("def _safe_tool_arguments", 1)[0]
...
def test_generic_context_has_no_legacy_domain_compatibility_surface() -> None:
    source = (ROOT / "backend/app/services/ai_runtime_context.py").read_text(encoding="utf-8")
    signature = source.split("def build_agent_context", 1)[1].split(") -> dict[str, Any]:", 1)[0]
...
def test_provider_accepts_only_the_agent_turn_model_contract() -> None:
    source = (ROOT / "backend/app/services/provider_runtime/output_contracts.py").read_text(encoding="utf-8")

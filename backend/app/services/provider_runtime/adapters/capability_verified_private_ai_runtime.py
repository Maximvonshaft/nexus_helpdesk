...
class CapabilityVerifiedPrivateAIRuntimeAdapter(PrivateAIRuntimeAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self.generation_model = (
            os.getenv("PRIVATE_AI_RUNTIME_GENERATION_MODEL", _DEFAULT_GENERATION_MODEL).strip()
        ...
        self.capabilities_path = (
            os.getenv("PRIVATE_AI_RUNTIME_CAPABILITIES_PATH", _DEFAULT_CAPABILITIES_PATH).strip()
    ...
    def _config_error(self) -> str | None:
        ...
        if parent_error:
            return parent_error
        for name in ("PRIVATE_AI_RUNTIME_DIRECT_MODEL", "PRIVATE_AI_RUNTIME_RAG_MODEL"):
    ...
    def _run_capability_probe(self) -> CapabilityProbeResult:
        if self._capability_probe_override is not None:
            ...
            if not isinstance(result, CapabilityProbeResult):
                return CapabilityProbeResult.not_ready("capability_payload_malformed")
            return result
        try:
    ...
    async def generate(self, db, request: ProviderRequest) -> ProviderResult:
        ...
        if not probe_result.ready:
            error_code = probe_result.reason_codes[0] if probe_result.reason_codes else "capability_runtime_not_ready"
            return self._blocked_result(
...
def _capability_timeout_from_env() -> float:
    raw = (os.getenv("PRIVATE_AI_RUNTIME_CAPABILITY_TIMEOUT_SECONDS") or "").strip()
    if not raw:

...
Scope: read-only audit of the WhatsApp customer channel, retired vendor runtime gateway, and Speedy Console on `178.105.160.174`. No server files, containers, routes, databases, or secrets were changed.

## Executive Decision

The WhatsApp customer channel can be extracted into NexusDesk, but the extraction should not embed retired vendor runtime runtime or Speedy Console as a dependency.
...
| Speedy support API | nginx proxies `/api/support/conversations*` and `/api/support/knowledge/*` to local retired vendor runtime gateway `127.0.0.1:18789`. |
| retired vendor runtime gateway | Running as `retired_vendor_runtime gateway --port 18789`; direct support endpoints require auth. |
| Legacy bridge | retired vendor runtime bridge is still listening on `172.19.0.1:18792`. |
...
flowchart TD
  ...
  support_api --> retired_vendor_runtime_gateway["retired vendor runtime gateway :18789"]
  ...
  candidate --> db
  legacy_worker["deploy-worker-outbound-1"] --> legacy_bridge["retired vendor runtime bridge :18792"]
  legacy_bridge --> retired_vendor_runtime_gateway
```
...
| Speedy Console conversation filtering UX | Rewrite | Keep the product idea, but back it with Nexus DB read models instead of retired vendor runtime `/api/support/*`. |
...
| retired vendor runtime gateway as support API dependency | Drop | It should not be required for Nexus WhatsApp customer channel operation. |
| retired vendor runtime bridge dispatch | Drop | It is already retired in source and still exists only as production drift. |
| retired vendor runtime env names and route names for runtime control | Drop | They preserve the wrong mental model for de-retired vendor runtime runtime. |
...
3. Wire candidate sidecar path without public cutover.
   ...
   - Keep retired vendor runtime bridge disabled in the candidate path.
...
retired vendor runtime and Speedy Console should only be used as historical evidence for what the customer-channel workflow looked like. They should not remain in the runtime dependency chain.

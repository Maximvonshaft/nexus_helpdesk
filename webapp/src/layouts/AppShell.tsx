...
function routeRequirementForPath(pathname: string): AccessRequirement | undefined {
  const entries = Object.entries(routeAccess).sort((a, b) => b[0].length - a[0].length)
...
export function AppShell({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    if (location.pathname !== '/login' && !session.isLoading && !session.isFetching && !session.data) navigate({ to: '/login', replace: true })
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><div className="brand-kicker">客服协同中心</div><h1>客服工作台</h1><div className="subtle">按真实客服工作流组织：先处理渠道，再跟进工单，再查询客户与运单。</div></div>
        <nav className="nav" data-testid="operator-primary-navigation" aria-label="客服运营后台主导航">
          {navGroups.map((group) => {
            const groupItems = group.items.map((to) => availableNav.find((item) => item.to === to)).filter(Boolean) as NavItem[]
            if (!groupItems.length) return null
            return <div className="nav-group" key={group.label}><div className="nav-group-label">{group.label}</div>{groupItems.map((item) => {
              ...
              return <Link key={item.to} to={item.to} data-active={active ? 'true' : 'false'} aria-current={active ? 'page' : undefined}><span>{item.label}</span><small>{item.hint}</small>{showRuntimeAttention ? <Badge tone="danger">需处理 {runtimeAttentionCount}</Badge> : null}</Link>
            })}</div>
          })}
        </nav>
        <div className="card soft sidebar-card"><div className="section-title">当前账号</div><div className="section-subtitle">{userLabel}</div><div className="sidebar-helper">{roleWorkspaceHint(session.data)}</div><div className="button-row" style={{ marginTop: 12 }}><Button variant="secondary" onClick={() => setCommandOpen(true)}>快捷操作</Button><Button variant="ghost" onClick={() => { logout(); navigate({ to: '/login', replace: true }) }}>退出登录</Button></div></div>
      </aside>
      <main role="main">
        <div className="topbar"><div><div className="section-title">客服运营台</div><div className="section-subtitle">先处理 WebChat / WebCall / Email，再跟进工单；客户或运单查询请走工单与查询入口。</div></div><div className="button-row topbar-status"><Badge tone={!canSeeOps ? 'default' : runtimeNeedsAttention ? 'danger' : runtime.data?.warnings?.length ? 'warning' : 'success'}>{!canSeeOps ? '客服模式' : runtimeNeedsAttention ? '运行需处理' : runtime.data?.warnings?.length ? '需要关注' : '运行正常'}</Badge><Badge>{autoRefresh.enabled ? '自动刷新已开启' : '自动刷新已暂停'}</Badge><Button variant="secondary" onClick={() => autoRefresh.setEnabled(!autoRefresh.enabled)}>{autoRefresh.enabled ? '暂停刷新' : '恢复刷新'}</Button><Button variant="secondary" onClick={() => setCommandOpen(true)}>快捷键 ⌘/Ctrl + K</Button></div></div>
        <div className="content">{routeDenied ? <NoAccessCard title="当前账号无权访问此页面" description="该入口需要额外 capability；左侧菜单已自动隐藏无权限入口，直接 URL 访问会在这里被拦截。" action="请联系主管或管理员开通对应权限，或返回今日工作台继续处理可见任务。" /> : (children ?? <Outlet />)}</div>
      </main>

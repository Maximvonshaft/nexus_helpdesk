...
def load_webchat_voice_runtime_config() -> WebchatVoiceRuntimeConfig:
    ...
    config = WebchatVoiceRuntimeConfig(
        ...
        provider=provider,
        routing_mode=(os.getenv("WEBCHAT_VOICE_ROUTING_MODE", "human_first").strip().lower() or "human_first"),
...
def validate_webchat_voice_runtime_config(config: WebchatVoiceRuntimeConfig) -> None:
    ...
    if config.enabled and not config.allowed_path_prefixes:
        raise RuntimeError("WEBCHAT_VOICE_ALLOWED_PATH_PREFIXES must be set when voice is enabled")
    ...
    if config.live_ai_voice_enabled and not config.livekit_agent_name:
        raise RuntimeError("LIVEKIT_AGENT_NAME must be set when WEBCHAT_LIVE_AI_VOICE_ENABLED=true")
    if config.live_ai_voice_enabled and not config.livekit_agent_shared_secret:
        raise RuntimeError(
            "LIVEKIT_AGENT_SHARED_SECRET must be set when WEBCHAT_LIVE_AI_VOICE_ENABLED=true"
        )

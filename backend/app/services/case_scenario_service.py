...
def _identity_changed(ticket: Ticket) -> bool:
    ...
    return any(state.attrs[field].history.has_changes() for field in SCENARIO_IDENTITY_FIELDS)


@event.listens_for(Ticket, "after_insert")
def _assign_scenario_after_ticket_insert(mapper, connection, target: Ticket) -> None:  # noqa: ANN001
    del mapper
    _insert_automatic_assignment(connection, target, source="ticket_insert")


@event.listens_for(Ticket, "before_update")
def _guard_scenario_identity_update(mapper, connection, target: Ticket) -> None:  # noqa: ANN001
    del mapper
    if target.id is None or not _identity_changed(target):
        return
    catalog = load_runtime_scenario_catalog()
    scenario = resolve_candidate_scenario(target, catalog)
    current = _connection_current_assignment(connection, int(target.id))
    if (
        current is not None
        and scenario is not None
        and scenario.scenario_key != current["scenario_key"]
    ):
        raise _http_conflict(
            "case_scenario_reclassification_command_required",
            ticket_id=target.id,
            current_scenario_key=current["scenario_key"],
            requested_scenario_key=scenario.scenario_key,
        )


@event.listens_for(Ticket, "after_update")
def _assign_scenario_after_ticket_update(mapper, connection, target: Ticket) -> None:  # noqa: ANN001
    del mapper

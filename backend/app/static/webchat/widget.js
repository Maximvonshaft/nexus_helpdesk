(function () {
  ...
  function showTyping() { if (state.typingEl) return; var wrapper = document.createElement('div'); wrapper.className = 'nd-webchat-typing'; wrapper.setAttribute('aria-label', aiTypingText()); for (var i = 0; i < 3; i += 1) { var dot = document.createElement('span'); dot.className = 'nd-webchat-typing-dot'; wrapper.appendChild(dot); } state.typingEl = wrapper; messagesEl.appendChild(wrapper); if (state.open || state.userNearBottom) messagesEl.scrollTop = messagesEl.scrollHeight; }
  function hideTyping() { if (!state.typingEl) return; state.typingEl.remove(); state.typingEl = null; }
  function applyAiState(data) { state.aiPending = Boolean(data && data.ai_pending); state.aiStatus = data && data.ai_status ? data.ai_status : null; if (state.aiPending) { showTyping(); setStatus(aiTypingText()); state.fastPollUntil = Date.now() + 20000; } else { hideTyping(); if (state.open) setStatus('Online'); } }

...
def persisted_traffic_configuration_errors(*, canary_percent: Any, kill_switch: Any) -> list[str]:
    errors: list[str] = []
    try:
...
def safe_traffic_configuration(*, default_canary_percent: int = 0, default_kill_switch: bool = False) -> dict[str, Any]:
    errors = persisted_traffic_configuration_errors(canary_percent=default_canary_percent, kill_switch=default_kill_switch)
    try:
...
def stable_canary_bucket(request: ProviderRequest) -> int:
    identity = "\x1f".join((str(request.tenant_id or ""), str(request.tenant_key or ""), str(request.channel_key or ""), str(request.session_id or ""), str(request.scenario or "")))
    digest = hashlib.sha256(identity.encode("utf-8", errors="replace")).digest()
    return int.from_bytes(digest[:8], "big", signed=False) % 100

def select_provider_traffic(request: ProviderRequest, *, canary_percent: int, kill_switch: bool, configured_mode_value: str | None = None) -> ProviderTrafficSelection:
    ...
    if normalized_kill_switch:
        ...
        try:
            ...
            mode = "invalid"
            _append_unique(errors, _TRAFFIC_MODE_INVALID)
        return ProviderTrafficSelection(mode, ProviderTrafficPath.KILL_SWITCH, percent, None, False, False, "kill_switch_active", tuple(errors))
    percent = _validated_canary_percent(canary_percent)
    mode = configured_traffic_mode(configured_mode_value)
    bucket = stable_canary_bucket(request)
    if mode == "control":
        return ProviderTrafficSelection(mode, ProviderTrafficPath.CONTROL, percent, bucket, False, False, "control_mode_configured")
    if mode == "shadow":
        return ProviderTrafficSelection(mode, ProviderTrafficPath.SHADOW_ONLY, percent, bucket, True, False, "shadow_mode_configured")
    if percent == 0:
        return ProviderTrafficSelection(mode, ProviderTrafficPath.CONTROL, percent, bucket, False, False, "canary_percent_zero")
    if bucket < percent:
        return ProviderTrafficSelection(mode, ProviderTrafficPath.CANARY_AUTHORITATIVE, percent, bucket, True, True, "bucket_selected")
    return ProviderTrafficSelection(mode, ProviderTrafficPath.CONTROL, percent, bucket, False, False, "bucket_not_selected")

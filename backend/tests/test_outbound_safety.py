...
def test_public_webchat_ai_low_risk_general_reply_allows():
    decision = evaluate_outbound_safety(DummyTicket(), 'Thanks. Our team will check your request shortly.', 'webchat_ai_public')
    assert decision.level == 'allow'
    assert decision.allowed is True


def test_public_webchat_ai_logistics_claim_without_evidence_requires_review():
    decision = evaluate_outbound_safety(DummyTicket(), 'Your parcel will arrive today.', 'webchat_ai_public')
    assert decision.level == 'review'
    assert decision.requires_human_review is True

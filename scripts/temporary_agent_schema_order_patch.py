...
def patch_core() -> None:
    ...
    if old in text:
        ...
        raise RuntimeError("raw schema validation projection: expected old or new form")
...
def patch_architecture() -> None:
    ...
    old = "5. Validate Tool registration, input schema, availability, permission, confirmation, and write-risk policy."
    new = "5. Validate raw model Tool arguments against registration and JSON Schema, then validate availability, permission, confirmation, and write-risk policy before execution-time bounding."
    if old in text:
        text = replace_once(text, old, new, label="architecture validation order")
    elif new not in text:
        raise RuntimeError("architecture validation order: expected old or new form")

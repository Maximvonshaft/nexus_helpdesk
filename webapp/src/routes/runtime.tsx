...
const OPENCLAW_INTEGRATION_UI_ENABLED = import.meta.env.VITE_OPENCLAW_INTEGRATION_ENABLED === 'true'
...
function RuntimePage() {
  ...
  const [runtime, readiness, signoff, jobs, queue, connectivity] = useQueries({
    queries: [
      { queryKey: ['runtimeHealth'], queryFn: api.runtimeHealth, enabled: permitted && OPENCLAW_INTEGRATION_UI_ENABLED },
      ...
      { queryKey: ['openclawConnectivity'], queryFn: api.openclawConnectivityCheck, enabled: permitted && OPENCLAW_INTEGRATION_UI_ENABLED },
    ],
  })

  async function refreshRuntimeViews() {
    await Promise.all([
      OPENCLAW_INTEGRATION_UI_ENABLED ? client.invalidateQueries({ queryKey: ['runtimeHealth'] }) : Promise.resolve(),
      ...
      OPENCLAW_INTEGRATION_UI_ENABLED ? client.invalidateQueries({ queryKey: ['openclawConnectivity'] }) : Promise.resolve(),
    ])
  }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<div style={{ display: 'flex', gap: 8 }}>{OPENCLAW_INTEGRATION_UI_ENABLED ? <><Button onClick={() => checkConnectivity.mutate()} disabled={checkConnectivity.isPending || !permitted}>{checkConnectivity.isPending ? '检查中…' : '检查 OpenClaw 联调'}</Button><Button onClick={() => consumeOnce.mutate()} disabled={consumeOnce.isPending || !permitted}>{consumeOnce.isPending ? '执行中…' : '执行一次消息同步'}</Button></> : null}</div>}
      />
      ...
      <div className="page-grid split-grid">{OPENCLAW_INTEGRATION_UI_ENABLED ? <Card><CardHeader title="消息同步状态" /><CardBody><DataTable columns={['项目', '值']} rows={[['同步游标', sanitizeDisplayText(runtime.data?.sync_cursor)],['当前状态', sanitizeDisplayText(runtime.data?.sync_daemon_status)],['最近心跳', formatDateTime(runtime.data?.sync_daemon_last_seen_at)],['待补同步', String(runtime.data?.stale_link_count ?? '—')],['待执行同步任务', String(runtime.data?.pending_sync_jobs ?? '—')],['失败同步任务', String(runtime.data?.dead_sync_jobs ?? '—')],['待处理附件任务', String(runtime.data?.pending_attachment_jobs ?? '—')]]} /></CardBody></Card> : null}<Card><CardHeader title="上线检查清单" /><CardBody><DataTable columns={['检查项', '状态']} rows={Object.entries(signoff.data?.checks ?? {}).map(([key, value]) => [signoffLabel(key), value ? '通过' : '未通过'])} /></CardBody></Card>{OPENCLAW_INTEGRATION_UI_ENABLED ? <Card><CardHeader title="OpenClaw 联调检查" subtitle="先确认本地或远端 Gateway 桥接是否真的通，再做消息和附件联调。" /><CardBody><DataTable columns={['项目', '值']} rows={[['部署模式', sanitizeDisplayText(connectivity.data?.deployment_mode)],['消息方式', sanitizeDisplayText(connectivity.data?.transport)],['MCP 命令', sanitizeDisplayText(connectivity.data?.command)],['Gateway 地址', sanitizeDisplayText(connectivity.data?.url)],['桥接已启动', connectivity.data?.bridge_started ? '是' : '否'],['会话列表工具可用', connectivity.data?.conversations_tool_ok ? '是' : '否'],['可见会话数', String(connectivity.data?.conversations_seen ?? '—')],['示例会话键', sanitizeDisplayText(connectivity.data?.sample_session_key)],['Token 鉴权已配置', connectivity.data?.token_auth_configured ? '是' : '否'],['Password 鉴权已配置', connectivity.data?.password_auth_configured ? '是' : '否']]} /></CardBody></Card> : null}<Card><CardHeader title="系统配置状态" /><CardBody><DataTable columns={['项目', '值']} rows={[['环境', sanitizeDisplayText(readiness.data?.app_env)],['数据库', sanitizeDisplayText(readiness.data?.database_url_scheme)],['附件存储', sanitizeDisplayText(readiness.data?.storage_backend)],['OpenClaw 可选集成', readiness.data?.openclaw_integration_enabled ? '已启用' : '未启用'],['监控开关', String(readiness.data?.metrics_enabled ?? false)],['同步开关', String(readiness.data?.openclaw_sync_enabled ?? false)]]} /></CardBody></Card><Card><CardHeader title="最近后台任务" /><CardBody><DataTable columns={['任务类型', '状态', '尝试次数', '更新时间', '恢复动作']} rows={jobRows(jobs.data ?? [], (job) => confirmAndRecover('requeue_job', `确认重排 dead 任务 #${job.id}（${job.job_type}）？`, job), recoveryPending)} /></CardBody></Card></div></>}

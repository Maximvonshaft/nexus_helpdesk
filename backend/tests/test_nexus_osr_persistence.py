...
os.environ.setdefault(
    "DATABASE_URL",
    "sqlite:////tmp/nexus_osr_persistence_tests.db",
)
...
from app import models, models_osr, webchat_models  # noqa: F401,E402
...
def test_case_context_persists_and_loads_short_lived_state(db_session):
    ...
    ctx = (
        ctx.with_inbound_message("Please check CH1234567890")
        .with_contact_method(
            channel="whatsapp",
            value="+382 67123456",
            source="webchat_form",
...
def test_scoped_policy_resolution_prefers_specific_rows(db_session):
    db_session.add(
        HumanHoursPolicyRecord(
            country_code="GLOBAL",
            channel="all",
    ...
    hours = resolve_human_hours_policy(
        db_session,
        country_code="ME",
        channel="webchat",
        queue_key="support",
    ...
    assert hours.timezone_name == "Europe/Podgorica"
    assert hours.weekly_hours["mon"] == [("09:00", "18:00")]
...
def test_whatsapp_routing_rule_resolution(db_session):
    ...
    rule = resolve_whatsapp_routing_rule(
        db_session,
        country_code="ME",
        issue_type="signed_not_received",
    )
...
def test_tool_execution_audit_persists_sanitized_snapshot(db_session):
    ticket, conversation = make_ticket_and_conversation(db_session)
    ctx = CaseContext(
        conversation_id=conversation.id,
        ticket_id=ticket.id,
    ...
    decision = RuntimeDecision(
        ...
        tool_actions=[
            RuntimeToolAction(
                ...
                executed=True,
                result_source_id="tool-log:1",
            )
        ],
        audit_reasons=["executed", "ok"],
    )

    row = audit_runtime_decision(
        db_session,
        decision=decision,
        evaluation=RuntimeDecisionEvaluation(allowed=True),
        case_context=ctx,
        tenant_id="pytest",
        channel="webchat",
        country_code="ME",
    ...
    assert row.decision_json["business_reply_type"] == "tool_action_result"
    assert row.decision_json["tool_actions"][0]["tool_name"] == "knowledge.search"

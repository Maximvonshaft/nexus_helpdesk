...
def apply_support_ticket_scope(
    query: Query,
    current_user: Any,
    db: Session,
) -> Query:
...
def apply_support_conversation_scope(
    ...
    Ticketless conversations are normal queue work. They always require an
    explicit active tenant/country/channel grant, including for administrators.
    """
...
def ensure_support_ticket_visible(
    db: Session,
    current_user: Any,
    ticket_id: int,
) -> None:

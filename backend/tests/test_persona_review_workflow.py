...
def _setup_db(tmp_path):
    ...
    db_session.commit()
    return engine, db_session, admin, agent


def test_persona_review_submit_approve_release_window_publish_contract(tmp_path):
    engine, db_session, admin, _agent = _setup_db(tmp_path)
    ...
    try:
        ...
        profile_response = client.post("/api/persona-profiles", headers=_headers(admin), json=_profile_payload())
        ...
        list_response = client.get("/api/persona-profiles/reviews?status=pending", headers=_headers(admin))
        ...
        detail_response = client.get(f"/api/persona-profiles/{profile['id']}", headers=_headers(admin))
        builder_response = client.get("/api/lite/persona-builder", headers=_headers(admin))
    finally:
...
def test_persona_review_publish_respects_future_release_window(tmp_path):
    ...
    try:
        client = TestClient(app)
        profile_response = client.post("/api/persona-profiles", headers=_headers(admin), json=_profile_payload("persona.review.future"))
        assert profile_response.status_code == 200, profile_response.text
        profile = profile_response.json()
        submit_response = client.post(
            f"/api/persona-profiles/{profile['id']}/submit-review",
            headers=_headers(admin),
            json={"release_window_start": (utc_now() + timedelta(days=1)).isoformat()},
        )
        assert submit_response.status_code == 200, submit_response.text
        review = submit_response.json()
        approve_response = client.post(
...
def test_persona_review_requires_ai_config_manage(tmp_path):
    engine, db_session, admin, agent = _setup_db(tmp_path)
    ...
    try:
        client = TestClient(app)
        profile_response = client.post("/api/persona-profiles", headers=_headers(admin), json=_profile_payload("persona.review.forbidden"))
        assert profile_response.status_code == 200, profile_response.text
        profile = profile_response.json()

...
Scope: turn the current OpenClaw-backed WhatsApp/support-agent capabilities into Nexus-native production capabilities. This is a delivery plan, not a deployment record.
...
OpenClaw remains only a historical reference during migration. It must not be required by the final runtime path.
...
- nginx proxies `/api/support/conversations*` and `/api/support/knowledge/*` to OpenClaw gateway.
- OpenClaw support agent uses one MCP server named `speedaf-support`.
- That MCP server is implemented at `/root/.openclaw/agents/support/workspace/mcp/support_mcp_server.py`.
...
- OpenClaw QR/session material exists under OpenClaw runtime paths and must not be copied into Git or treated as source code.
...
| OpenClaw tool/capability | Nexus target | Status |
| --- | --- | --- |
...
| `support_knowledge_retrieve` | Nexus knowledge/support-intelligence read model | Partial; should be Nexus DB-backed, not OpenClaw memory-backed. |
...
- Candidate path can run without OpenClaw gateway/bridge.
...
- No OpenClaw bridge dispatch happens in candidate path.

### Phase 5: Nexus-Native WhatsApp Conversation Read Model

Goal: replace Speedy Console / WhatsApp Lite dependence on OpenClaw support APIs.
...
- Nexus can list WhatsApp conversations without OpenClaw `/api/support/conversations`.
- Detail view shows inbound and outbound messages from Nexus DB.
- OpenClaw gateway can be unavailable without breaking Nexus WhatsApp workspace.

### Phase 6: Speedaf Read-Only Tools In AI Runtime

Goal: give Nexus AI the same useful lookup capability as OpenClaw support agent, but with Nexus safety.

OpenClaw tools to map:
...
- `support_knowledge_retrieve` maps to Nexus knowledge/support-intelligence, not OpenClaw memory.
...
Goal: support the same action family as OpenClaw while preventing AI from directly executing risky operations.
...
- Nexus no longer needs OpenClaw `/api/support/conversations`.
...
- Admin can bind WhatsApp from Nexus without OpenClaw.
...
- No production path requires OpenClaw gateway, OpenClaw bridge, or Speedy Console APIs.

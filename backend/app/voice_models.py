...
class WebchatVoiceSession(Base):
    ...
    recording_provider_ref: Mapped[Optional[str]] = mapped_column(String(180), nullable=True, index=True)
...
class WebchatVoiceParticipant(Base):
    ...
    parent_leg_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("webchat_voice_participants.id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    termination_reason: Mapped[Optional[str]] = mapped_column(String(160), nullable=True, index=True)
    metadata_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    started_at: Mapped[Optional[datetime]] = mapped_column(UTCDateTime, nullable=True, index=True)
    answered_at: Mapped[Optional[datetime]] = mapped_column(UTCDateTime, nullable=True, index=True)
...
class VoiceRoutingOffer(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "voice_session_id", "agent_id", "sequence", name="uq_voice_offer_session_agent_sequence"
        ),
        CheckConstraint(
            "status IN ('offered', 'accepted', 'declined', 'expired', 'cancelled')",
            name="ck_voice_routing_offer_status",
        ),
        Index("ix_voice_offer_agent_status_expiry", "agent_id", "status", "expires_at"),
    ...
    offered_at: Mapped[datetime] = mapped_column(UTCDateTime, default=utc_now, index=True)
...
class WebchatVoiceSessionAction(Base):
    ...
    __table_args__ = (
        UniqueConstraint("idempotency_key", name="uq_voice_session_action_idempotency_key"),
        CheckConstraint(
        ...
        Index("ix_voice_command_dispatch", "status", "next_attempt_at", "lease_expires_at"),
    )
    ...
    provider_status: Mapped[str] = mapped_column(String(40), default="pending", index=True)
    provider_reason: Mapped[Optional[str]] = mapped_column(String(160), nullable=True, index=True)
    provider_reference: Mapped[Optional[str]] = mapped_column(String(180), nullable=True, index=True)
...
class VoiceChannelConfiguration(Base):
    ...
    __table_args__ = (
        UniqueConstraint("channel_account_id", name="uq_voice_channel_configuration_account"),
        CheckConstraint(
    ...
    channel_account_id: Mapped[int] = mapped_column(
        ForeignKey("channel_accounts.id", ondelete="CASCADE"), nullable=False, index=True
    )
    livekit_project_ref: Mapped[Optional[str]] = mapped_column(String(160), nullable=True)
    ...
    dispatch_rule_id: Mapped[Optional[str]] = mapped_column(String(160), nullable=True, index=True)
    ...
    offer_timeout_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=20)
    wrap_up_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=30)
    overflow_action: Mapped[str] = mapped_column(String(24), nullable=False, default="ai")
    voicemail_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    recording_policy: Mapped[str] = mapped_column(String(32), nullable=False, default="disabled")
    transcription_policy: Mapped[str] = mapped_column(String(32), nullable=False, default="disabled")
...
class TelephonyEventInbox(Base):
    ...
    __table_args__ = (
        UniqueConstraint("provider", "provider_event_id", name="uq_telephony_event_provider_identity"),
        CheckConstraint(
            "status IN ('received', 'processing', 'processed', 'ignored', 'retryable', 'failed', 'dead_letter')",
            name="ck_telephony_event_inbox_status",
        ),
        Index("ix_telephony_event_inbox_status_received", "status", "received_at"),
    ...
    raw_payload_object_key: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    ...
    voice_session_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("webchat_voice_sessions.id", ondelete="SET NULL"), nullable=True, index=True
    )
    attempt_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    last_error_code: Mapped[Optional[str]] = mapped_column(String(120), nullable=True)
    next_attempt_at: Mapped[Optional[datetime]] = mapped_column(UTCDateTime, nullable=True, index=True)
    processing_started_at: Mapped[Optional[datetime]] = mapped_column(UTCDateTime, nullable=True)
    ...
    dead_lettered_at: Mapped[Optional[datetime]] = mapped_column(UTCDateTime, nullable=True)

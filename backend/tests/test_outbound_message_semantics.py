...
def make_team(db_session):
    team = Team(name='Ops', team_type='support')
    db_session.add(team)
    db_session.flush()
    return team


def make_user(db_session, team=None):
    user = User(
        username='ops-user',
...
def make_ticket(db_session, *, channel=SourceChannel.whatsapp, contact='+15550001'):
    ...
    ticket = Ticket(
        ticket_no=f'T-{channel.value}-{contact[-4:]}',
...
def test_non_external_outbound_never_calls_provider_send(db_session, monkeypatch):
    ...
    monkeypatch.setattr('app.services.message_dispatch.dispatch_via_openclaw_bridge', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('provider send must not run for web_chat')))
    monkeypatch.setattr('app.services.message_dispatch.dispatch_via_openclaw_mcp', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('messages_send must not run for web_chat')))
    monkeypatch.setattr('app.services.message_dispatch.dispatch_via_openclaw_cli', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('cli send must not run for web_chat')))
...
def test_inbound_auto_sync_does_not_create_outbound_messages(db_session, monkeypatch):
    ...
    monkeypatch.setattr(openclaw_bridge_service, 'dispatch_via_openclaw_bridge', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('send-message should not be called')))
    monkeypatch.setattr(openclaw_bridge_service, 'dispatch_via_openclaw_mcp', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('messages_send should not be called')))
    monkeypatch.setattr(openclaw_bridge_service, 'dispatch_via_openclaw_cli', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('cli send should not be called')))
    monkeypatch.setattr(openclaw_bridge_service, 'list_openclaw_conversations', lambda **kwargs: {
        'conversations': [
            {'sessionKey': 'sess-inbound-no-outbox', 'route': {'channel': 'whatsapp', 'recipient': '+15550104', 'accountId': 'default'}},
        ]
    })
    monkeypatch.setattr(openclaw_bridge_service, 'read_openclaw_bridge_conversation', lambda session_key, limit=50: (
        {'sessionKey': session_key, 'route': {'channel': 'whatsapp', 'recipient': '+15550104', 'accountId': 'default'}},
        [{'id': 'msg-inbound-1', 'role': 'user', 'author': 'customer', 'text': 'hello inbound'}],
    ))
...
def test_worker_disabled_outbound_still_never_claims_or_sends(monkeypatch):

...
- WebCall: top-level `/webcall` is connected to real queue/session, customer identity, AI turn, handoff, session action and timeline APIs. Follow-up PRs extended the WebChat thread contract with redacted runtime events and added call-note writes through `POST /api/webchat/admin/tickets/{ticket_id}/voice/{voice_session_id}/notes`, backed by `TicketInternalNote`, ticket timeline, WebChat event and admin audit evidence. Remaining work is visual parity polish for transcript/live-console density.

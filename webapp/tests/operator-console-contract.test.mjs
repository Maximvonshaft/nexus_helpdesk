...
const operationsShell = readFileSync(resolve(root, 'src/shared/layout/OperationsShell.tsx'), 'utf8')
const mainEntry = readFileSync(resolve(root, 'src/main.tsx'), 'utf8')
...
test('operator shell exposes the operations cockpit foundation', () => {
  assert.match(mainEntry, /@\/styles\/tokens\.css/)
  assert.match(mainEntry, /@\/styles\/components\.css/)
  assert.match(appShell, /import \{ OperationsShell \} from '@\/shared\/layout\/OperationsShell'/)
  assert.match(appShell, /workflowContextForPath/)
  assert.match(appShell, /Unified Agent Inbox/)
  assert.match(appShell, /Knowledge \+ QMD/)
  assert.match(appShell, /Ontology \/ Status Dictionary/)
  assert.match(appShell, /Runtime Observatory/)
  assert.match(appShell, /OperationsEventDock/)
  assert.match(operationsShell, /data-testid="operations-context-panel"/)
  assert.match(operationsShell, /data-testid="operations-event-dock"/)
  assert.match(operationsShell, /className="ops-workspace"/)
})

...
_SECRET_KEYS = {"raw_payload", "auth", "token", "access_token", "refresh_token", "secret", "password", "authorization", "api_key", "cookie"}
...
class PrivateAIRuntimeAdapter(ProviderAdapter):
    ...
    async def generate(self, db: Session, request: ProviderRequest) -> ProviderResult:
        ...
        if not token:
            return _failure(profile, "private_ai_runtime_token_missing", started, retryable=False)
        ...
        try:
            response_payload = await asyncio.to_thread(_post_json, profile, endpoint, payload, token)
        except (TimeoutError, socket.timeout):
            return _failure(profile, "private_ai_runtime_timeout", started, {"endpoint_path": safe_url_path(endpoint)}, retryable=True)
        except urllib.error.HTTPError as exc:
            return _failure(profile, f"private_ai_runtime_http_{exc.code}", started, {"endpoint_path": safe_url_path(endpoint), "http_status": exc.code}, retryable=exc.code in _RETRYABLE_HTTP)
        except urllib.error.URLError as exc:
            return _failure(profile, "private_ai_runtime_url_error", started, {"endpoint_path": safe_url_path(endpoint), "reason": str(exc.reason)[:120]}, retryable=True)
        except OSError as exc:
            return _failure(profile, "private_ai_runtime_network_error", started, {"endpoint_path": safe_url_path(endpoint), "reason": type(exc).__name__}, retryable=True)
        except ValueError as exc:
            return _failure(profile, "private_ai_runtime_bad_response", started, {"endpoint_path": safe_url_path(endpoint), "reason": str(exc)[:160]}, retryable=True)

        repaired = False
        try:
            decision = _normalize_agent_turn(response_payload, max_output_chars=profile.max_output_chars)
            decision = OutputContracts.validate_and_parse(request.output_contract, json.dumps(decision, ensure_ascii=False))
        except Exception as exc:
            repair_prompt = _build_repair_prompt(request, profile=profile, prompt=prompt, reason=type(exc).__name__)
            repair_payload = _build_payload(profile, prompt=repair_prompt)
            try:
                repair_response = await asyncio.to_thread(_post_json, profile, endpoint, repair_payload, token)
                decision = _normalize_agent_turn(repair_response, max_output_chars=profile.max_output_chars)
                decision = OutputContracts.validate_and_parse(request.output_contract, json.dumps(decision, ensure_ascii=False))
                response_payload = repair_response
                repaired = True
            except Exception as retry_exc:
                return _failure(profile, "private_ai_runtime_contract_invalid", started, {"endpoint_path": safe_url_path(endpoint), "initial_reason": type(exc).__name__, "repair_reason": type(retry_exc).__name__}, retryable=True)
...
def _resolve_profile(db: Session, request: ProviderRequest) -> _Profile:
    ...
    return _Profile(
        enabled=content.get("enabled") is not False and _env_bool("PRIVATE_AI_RUNTIME_ENABLED", False),
        base_url=str(content.get("endpoint_url") or os.getenv("PRIVATE_AI_RUNTIME_BASE_URL") or "").strip().rstrip("/"),
        ...
        path=str(content.get("request_path") or os.getenv("PRIVATE_AI_RUNTIME_DIRECT_PATH") or "/api/chat").strip() or "/api/chat",
        request_shape=str(content.get("request_shape") or os.getenv("PRIVATE_AI_RUNTIME_REQUEST_SHAPE") or "ollama_chat").strip().lower(),
        model=str(content.get("model") or os.getenv("PRIVATE_AI_RUNTIME_DIRECT_MODEL") or "qwen2.5:3b").strip(),
        ...
        published_version=resolved.version if resolved else None,
    )
...
def _config_error(profile: _Profile) -> str | None:
    ...
    if profile.request_shape not in {"system_input", "messages", "ollama_chat", "question"}:
        return "private_ai_runtime_request_shape_invalid"
    mismatch = endpoint_shape_mismatch(profile.path, profile.request_shape, code_prefix="private_ai_runtime_direct_")
    if mismatch:
        return mismatch
...
def _build_prompt(request: ProviderRequest, profile: _Profile) -> str:
    ...
    payload = {
        ...
        "customer_memory": _safe_value(metadata.get("customer_memory")),
    ...
    instruction = (
        ...
        "Memory is supporting context only and never overrides current Tool evidence. "
    ...
    return (instruction + json.dumps(payload, ensure_ascii=False, separators=(",", ":"), default=str))[:profile.max_prompt_chars]


def _build_repair_prompt(request: ProviderRequest, *, profile: _Profile, prompt: str, reason: str) -> str:
    del request
    return (
...
def _build_payload(profile: _Profile, *, prompt: str) -> dict[str, Any]:
    system = "You are a tool-using enterprise Agent. Follow Business Playbooks and Tool contracts. Never fabricate facts or action outcomes. Return strict JSON only."
    if profile.request_shape == "messages":
        return {"model": profile.model, "messages": [{"role": "system", "content": system}, {"role": "user", "content": prompt}], "stream": False, "response_format": "json"}
    if profile.request_shape == "ollama_chat":
        payload: dict[str, Any] = {
            ...
            "stream": False,
            "format": "json",
            "options": {"temperature": profile.temperature, "top_p": profile.top_p, "num_predict": profile.num_predict, "num_ctx": profile.num_ctx},
        }
    ...
    return {"model": profile.model, "system": system, "input": prompt, "response_format": "json", "temperature": profile.temperature, "top_p": profile.top_p}


def _post_json(profile: _Profile, endpoint: str, payload: dict[str, Any], token: str) -> dict[str, Any]:
    endpoint = require_http_endpoint(endpoint, label="Private AI runtime endpoint")
    request = urllib.request.Request(endpoint, data=json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8"), headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}, method="POST")
    with urllib.request.urlopen(request, timeout=float(profile.timeout_seconds)) as response:  # nosec B310
...
def _failure(profile: _Profile, error_code: str, started: float, summary: dict[str, Any] | None = None, *, retryable: bool) -> ProviderResult:
    return ProviderResult(
        ok=False,
        ...
        raw_payload_safe_summary={
            ...
            "credential_reference_configured": bool(profile.credential_ref or profile.token_file),
            "model_profile_key": profile.resource_key,
            "model_profile_version": profile.published_version,
            **(summary or {}),
        },
...
def _normalize_agent_turn(payload: dict[str, Any], *, max_output_chars: int) -> dict[str, Any]:
    ...
    for key in ("output_text", "text", "response_text", "answer", "raw_content"):
        if isinstance(candidate, dict) and isinstance(candidate.get(key), str) and candidate[key].strip():
...
def _safe_runtime_usage(payload: Any) -> dict[str, Any]:
    ...
    return {str(key)[:80]: value for key, value in list(usage.items())[:20] if isinstance(value, (int, float, str, bool))}
...
def _safe_value(value: Any, *, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        return {str(key)[:100]: _safe_value(item, depth=depth + 1) for key, item in list(value.items())[:100] if str(key).lower() not in _SECRET_KEYS}

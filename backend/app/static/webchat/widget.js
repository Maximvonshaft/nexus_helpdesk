(function () {
  ...
  var LOCAL_WELCOME_KEY = '__local_welcome__';
  var MAX_TYPING_MS = 45000;
  var state = {
    conversationId: null,
    visitorToken: null,
    open: false,
  ...
  style.textContent = '\n'
    ...
    + '.nd-webchat-typing-dot{width:6px;height:6px;border-radius:999px;background:#98a2b3;animation:ndTypingBounce 1.1s infinite ease-in-out}\n'
    + '.nd-webchat-typing-dot:nth-child(2){animation-delay:.15s}.nd-webchat-typing-dot:nth-child(3){animation-delay:.3s}\n'
    + '@keyframes ndTypingBounce{0%,80%,100%{transform:translateY(0);opacity:.45}40%{transform:translateY(-4px);opacity:1}}\n'
    + '@media (prefers-reduced-motion:reduce){.nd-webchat-typing-dot{animation:none}}\n'
    ...
    + '@media (max-height:700px){.nd-webchat-panel{height:calc(100dvh - 96px)}.nd-webchat-header{padding:10px 12px}.nd-webchat-header span{display:none}.nd-webchat-messages{padding:10px}.nd-webchat-form{gap:6px;padding:9px 10px}.nd-webchat-send{min-width:64px;padding:0 12px}}\n';
  ...
  function aiTypingText() { return locale.indexOf('zh') === 0 ? assistantName + ' 正在输入…' : assistantName + ' is typing...'; }
  function showTyping() { if (state.typingEl) return; var wrapper = document.createElement('div'); wrapper.className = 'nd-webchat-typing'; wrapper.setAttribute('aria-label', aiTypingText()); for (var i = 0; i < 3; i += 1) { var dot = document.createElement('span'); dot.className = 'nd-webchat-typing-dot'; wrapper.appendChild(dot); } state.typingEl = wrapper; state.typingShownAt = Date.now(); messagesEl.appendChild(wrapper); if (state.open || state.userNearBottom) messagesEl.scrollTop = messagesEl.scrollHeight; }
  function hideTyping() { if (!state.typingEl) return; state.typingEl.remove(); state.typingEl = null; state.typingShownAt = 0; }
  function applyAiState(data) { state.aiPending = Boolean(data && data.ai_pending); state.aiStatus = data && data.ai_status ? data.ai_status : null; state.aiTurnId = data && data.ai_turn_id ? data.ai_turn_id : null; if (state.aiPending) { if (state.typingShownAt && Date.now() - state.typingShownAt > MAX_TYPING_MS) { hideTyping(); setStatus('Still working...'); state.fastPollUntil = Date.now() + 5000; return; } showTyping(); setStatus(aiTypingText()); state.fastPollUntil = Date.now() + 20000; } else { hideTyping(); if (state.open) setStatus('Online'); } }
  ...
  function messageAliases(msg) { var aliases = []; if (msg) { if (msg.id !== undefined && msg.id !== null) aliases.push('server:' + String(msg.id)); if (msg.client_message_id) aliases.push('client:' + String(msg.client_message_id)); } return aliases; }
  function findRenderedAlias(msg) { var aliases = messageAliases(msg); for (var i = 0; i < aliases.length; i += 1) { if (state.rendered[aliases[i]]) return state.rendered[aliases[i]]; } return null; }
  function rememberAliases(msg, el) { var aliases = messageAliases(msg); for (var i = 0; i < aliases.length; i += 1) state.rendered[aliases[i]] = el; }
  function renderWelcomeIfEmpty() { if (state.welcomeRendered || state.rendered[LOCAL_WELCOME_KEY] || state.realMessageCount > 0) return; var w = document.createElement('div'); w.className = 'nd-webchat-msg agent'; w.textContent = welcome; messagesEl.appendChild(w); state.rendered[LOCAL_WELCOME_KEY] = w; state.welcomeRendered = true; }
  function roleClassFor(msg) { return msg.direction === 'visitor' ? 'visitor' : msg.direction === 'action' ? 'action' : msg.direction === 'system' ? 'system' : 'agent'; }
  function appendTextMessage(msg, extraClass) { var existing = findRenderedAlias(msg); if (existing) { existing.textContent = msg.body_text || msg.body || existing.textContent || ''; existing.className = 'nd-webchat-msg ' + roleClassFor(msg) + (extraClass ? ' ' + extraClass : ''); rememberAliases(msg, existing); return existing; } var el = document.createElement('div'); el.className = 'nd-webchat-msg ' + roleClassFor(msg) + (extraClass ? ' ' + extraClass : ''); el.textContent = msg.body_text || msg.body || ''; rememberAliases(msg, el); state.realMessageCount += 1; messagesEl.appendChild(el); return el; }
  function renderUnknownCard(card) { var text = (card && (card.title || card.body)) || 'Unsupported card. Please type your request.'; var el = document.createElement('div'); el.className = 'nd-webchat-msg system'; el.textContent = text; messagesEl.appendChild(el); return el; }
  function appendCardMessage(msg) { var key = 'server:' + String(msg.id); if (state.rendered[key]) return; var payload = msg.payload_json || {}; if (!payload || ['quick_replies', 'handoff'].indexOf(payload.card_type) === -1) { state.rendered[key] = renderUnknownCard(payload); state.realMessageCount += 1; return; } var card = document.createElement('div'); card.className = 'nd-webchat-card'; var titleEl = document.createElement('div'); titleEl.className = 'nd-webchat-card-title'; titleEl.textContent = payload.title || 'Support options'; card.appendChild(titleEl); if (payload.body) { var bodyEl = document.createElement('div'); bodyEl.className = 'nd-webchat-card-body'; bodyEl.textContent = payload.body; card.appendChild(bodyEl); } var actionsEl = document.createElement('div'); actionsEl.className = 'nd-webchat-card-actions'; card.appendChild(actionsEl); (payload.actions || []).forEach(function (action) { var a = document.createElement('button'); a.type = 'button'; a.className = 'nd-webchat-card-action'; a.textContent = action.label || action.id; a.disabled = msg.action_status === 'submitted'; a.addEventListener('click', function () { submitAction(msg, payload, action, a); }); actionsEl.appendChild(a); }); state.rendered[key] = card; state.realMessageCount += 1; messagesEl.appendChild(card); }
  function appendMessages(messages) { var wasNearBottom = state.userNearBottom; var hasAgentMessage = false; if (!messages || !messages.length) renderWelcomeIfEmpty(); (messages || []).forEach(function (msg) { if (msg.id && msg.id > state.lastMessageId) state.lastMessageId = msg.id; if (msg.message_type === 'card') appendCardMessage(msg); else appendTextMessage(msg); if (msg.direction !== 'visitor') hasAgentMessage = true; if (!state.open && msg.direction !== 'visitor') state.unread += 1; }); if (hasAgentMessage) { state.aiPending = false; hideTyping(); } persist(); updateUnread(); if (wasNearBottom || state.open) messagesEl.scrollTop = messagesEl.scrollHeight; }
  ...
  function ensureSession() { if (state.conversationId && state.visitorToken) return Promise.resolve(); return init(); }
  function poll(resetBackoff) { if (!state.conversationId || !state.visitorToken) return Promise.resolve(); var qs = '?limit=50'; if (state.lastMessageId) qs += '&after_id=' + encodeURIComponent(state.lastMessageId); return api('/api/webchat/conversations/' + encodeURIComponent(state.conversationId) + '/messages' + qs, { headers: { 'X-Webchat-Visitor-Token': state.visitorToken } }, 10000).then(function (data) { appendMessages(data.messages || []); applyAiState(data); if (resetBackoff !== false) state.backoffMs = 4000; }).catch(function () { setStatus('Reconnecting...'); state.backoffMs = Math.min(30000, Math.max(4000, state.backoffMs * 1.6)); }); }
  function currentPollDelay() { if (document.visibilityState === 'hidden') return Math.max(state.backoffMs, 15000); if (state.aiPending || Date.now() < state.fastPollUntil) return 1000; return state.backoffMs; }
  function schedulePoll() { if (state.pollTimer) clearTimeout(state.pollTimer); state.pollTimer = setTimeout(function tick() { if (state.open && document.visibilityState !== 'hidden') poll(false).finally(schedulePoll); else schedulePoll(); }, currentPollDelay()); }
  ...
  function submitAction(msg, payload, action, buttonEl) { if (!state.conversationId || !state.visitorToken || buttonEl.disabled) return; buttonEl.disabled = true; setStatus('Sending selection...'); api('/api/webchat/conversations/' + encodeURIComponent(state.conversationId) + '/actions', { method: 'POST', headers: { 'X-Webchat-Visitor-Token': state.visitorToken }, body: JSON.stringify({ message_id: msg.id, card_id: payload.card_id, action_id: action.id, action_type: action.action_type || 'quick_reply', payload: action.payload || {} }) }, 12000).then(function (data) { appendMessages([data.message]); setStatus(data.handoff_triggered ? 'Support requested' : 'Selection sent'); return poll(true); }).catch(function () { buttonEl.disabled = false; setStatus('Failed to send selection'); }); }
  function markFailed(cmid, body) { var optimistic = state.rendered['client:' + cmid]; if (!optimistic) return; optimistic.className = optimistic.className.replace('sending', 'failed'); if (optimistic.querySelector('.nd-webchat-retry')) return; var retry = document.createElement('button'); retry.type = 'button'; retry.className = 'nd-webchat-retry'; retry.textContent = 'Retry'; retry.addEventListener('click', function () { retry.disabled = true; resendMessage(body, cmid); }); optimistic.appendChild(retry); }
  function markSending(cmid) { var optimistic = state.rendered['client:' + cmid]; if (!optimistic) return; optimistic.className = optimistic.className.replace('failed', 'sending'); var retry = optimistic.querySelector('.nd-webchat-retry'); if (retry) retry.remove(); }
  function markSent(cmid) { var optimistic = state.rendered['client:' + cmid]; if (!optimistic) return; optimistic.className = optimistic.className.replace('sending', '').replace('failed', ''); var retry = optimistic.querySelector('.nd-webchat-retry'); if (retry) retry.remove(); }
  function sendMessage(body, cmid, didRetryAfterInit) { state.pendingBodies[cmid] = body; return ensureSession().then(function () { return api('/api/webchat/conversations/' + encodeURIComponent(state.conversationId) + '/messages', { method: 'POST', headers: { 'X-Webchat-Visitor-Token': state.visitorToken }, body: JSON.stringify({ body: body, client_message_id: cmid }) }); }).then(function (data) { inputEl.value = ''; setStatus(data.idempotent ? 'Already sent' : 'Sent'); if (data.message) appendTextMessage(data.message); markSent(cmid); delete state.pendingBodies[cmid]; if (data.message && data.message.id) state.lastMessageId = Math.max(state.lastMessageId, data.message.id); applyAiState(data); return poll(true); }).catch(function (err) { if (err && err.status === 403 && !didRetryAfterInit) { state.conversationId = null; state.visitorToken = null; persist(); return init().then(function () { return sendMessage(body, cmid, true); }); } markFailed(cmid, body); setStatus('Failed to send. Please retry.'); throw new Error('send_failed'); }); }
  ...
  inputEl.addEventListener('keydown', function (event) { if (event.isComposing || state.composing || event.keyCode === 229) return; });
  formEl.addEventListener('submit', function (event) { event.preventDefault(); if (state.composing || event.isComposing) return; var body = inputEl.value.trim(); if (!body || state.busy) return; var cmid = clientMessageId(); state.busy = true; sendEl.disabled = true; setStatus('Sending...'); appendTextMessage({ direction: 'visitor', body: body, body_text: body, client_message_id: cmid }, 'sending'); sendMessage(body, cmid).catch(function () {}).finally(function () { state.busy = false; sendEl.disabled = false; }); });

...
- `webapp/tests/email-workbench-contract.test.mjs` locks the `/email` workbench `routeAccess` RBAC semantics, AppShell/CommandPalette entrypoints, tokenized Email queue filtering, API-client draft save, outbound send call, and timeline invalidation loop.

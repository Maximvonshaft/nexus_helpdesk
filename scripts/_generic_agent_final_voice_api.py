...
# ---------------------------------------------------------------------------
# 5. Voice is Session-first. Live voice never creates a formal Ticket.
# ---------------------------------------------------------------------------
conversation_path = "backend/app/services/conversation_first_service.py"
conversation = read(conversation_path)
if re.search(r"^def ensure_voice_ticket_for_public_conversation\(", conversation, flags=re.MULTILINE):
    conversation = remove_function(conversation, "ensure_voice_ticket_for_public_conversation")
conversation = conversation.replace("from ..enums import SourceChannel, TicketPriority\n", "")
conversation = conversation.replace("from ..voice_models import WebchatVoiceSession\n", "")
conversation = conversation.replace("from .nexus_osr.auto_ticket_service import create_or_reuse_ticket_from_case_context\n", "")
conversation = conversation.replace("from .nexus_osr.case_context import CaseContext\n", "")
write(conversation_path, conversation)

voice_schema_path = "backend/app/voice_schemas.py"
voice_schema = read(voice_schema_path)
voice_schema = voice_schema.replace("    ticket_id: int\n", "    ticket_id: int | None = None\n")
voice_schema = voice_schema.replace("    ticket_event_id: int\n", "    ticket_event_id: int | None = None\n")
write(voice_schema_path, voice_schema)

...
def test_disabled_policy_blocks_and_writes_safe_audit(db_session):
    ...
    result = execute_controlled_tool_calls(
        db_session,
        tool_calls=[{"tool_name": "ticket.create", "idempotency_key": "disabled-ticket"}],
        case_context=ctx,
...
def test_missing_tracking_and_contact_blocks_before_execution(db_session):
    ...
    result = execute_controlled_tool_calls(
        db_session,
        tool_calls=[{"tool_name": "ticket.create", "idempotency_key": "missing-context"}],
        case_context=ctx,
        channel="webchat",
        country_code="ME",
    )[0]
...
def test_ticket_create_creates_real_ticket_and_is_idempotent(db_session):
    add_policy(db_session, "ticket.create", requires_tracking_number=True, requires_contact=True)
    ...
    first = execute_controlled_tool_calls(
        db_session,
        tool_calls=[{"tool_name": "ticket.create", "idempotency_key": "ticket-idem", "arguments": {"issue_type": "tracking"}}],
        case_context=ctx,
        conversation=conversation,
        customer=customer,
        channel="webchat",

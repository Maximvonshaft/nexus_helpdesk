...
test('email workbench closes draft save, outbound send, and timeline refresh loops', () => {
  ...
  assert.match(emailRoute, /request_id=\$\{requestId \|\| '-'\}/)
  assert.match(emailRoute, /message_id=\$\{messageId \|\| '-'\}/)
  assert.match(emailRoute, /function EmailDeliveryEvidence/)
  assert.match(emailRoute, /data-testid="email-delivery-evidence"/)
  assert.match(emailRoute, /provider_status/)
  assert.match(emailRoute, /failure_code/)

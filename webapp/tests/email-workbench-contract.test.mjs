...
test('email workbench route uses explicit ticket plus outbound RBAC semantics', () => {
  assert.match(router, /EmailRoute/)
  assert.match(router, /@\/routes\/email/)
  assert.match(emailRoute, /path: '\/email'/)
  assert.match(emailRoute, /const emailWorkbenchAccess = \{[\s\S]*allOf: \[CAPABILITIES\.ticketRead\][\s\S]*anyOf: \[CAPABILITIES\.outboundDraftSave, CAPABILITIES\.outboundSend\]/)
  ...
  assert.doesNotMatch(emailRoute, /routeAccess\['\/email'\]/)
})
...
test('email workbench closes draft save, outbound send, and timeline refresh loops', () => {
  assert.match(emailRoute, /\/api\/tickets\/\$\{ticketId\}\/outbound\/draft/)

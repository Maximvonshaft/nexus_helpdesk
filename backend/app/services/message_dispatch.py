...
def _build_fact_evidence(ticket: Ticket | None) -> dict[str, Any] | None:
    ...
    A tracking number alone is not enough to bypass review. A human/system update
    on the ticket must also be present so the safety gate can distinguish a real
    operator-checked logistics reply from an unsupported assertion.
    """
    ...
    if decision.requires_human_review:
        ...
        log_event(
            ...
            payload={
                ...
                'fact_evidence_present': fact_evidence is not None,
            },
        )
        return False
    ...
    if target:
        ...
        if status == MessageStatus.failed and settings.openclaw_cli_fallback_enabled:
            LOGGER.warning(
                ...
                extra={'event_payload': {'message_id': message.id, 'ticket_id': message.ticket_id, 'provider_status': provider_status, 'route': route_context}},
            )

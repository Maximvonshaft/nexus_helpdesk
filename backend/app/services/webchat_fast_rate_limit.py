...
def _client_ip_from_forwarded_for(xff: str) -> str | None:
    ...
    Nginx uses proxy_add_x_forwarded_for, so a client-supplied spoofed XFF value
    is preserved on the left and the real peer seen by Nginx is appended on the
    right. Choosing the first public address lets attackers rotate rate-limit
    buckets by spoofing the left-most value. Walking from right to left and
    skipping trusted proxies binds the bucket to the closest untrusted client.
    """

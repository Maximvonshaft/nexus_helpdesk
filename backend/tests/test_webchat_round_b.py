...
from app.webchat_models import WebchatConversation, WebchatMessage  # noqa: F401
    ...
    try:
        if not db.query(User).filter(User.id == 9999).first():
            db.add(
                User(
                    id=9999,
                    username="roundb_admin",
                    display_name="Round B Admin",
                    password_hash="test",
                    role=UserRole.admin,
                    is_active=True,
                )
...
def _create_webchat_message_flow(client: TestClient):
    ...
    assert any(
        ...
        and "Hello, what can you help me with?" in item["body"]
...
def test_public_webchat_init_send_poll_and_background_ai_reply(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: WebchatRuntimeReplyResult(
            ...
            reply=(
                "I can help with shipment questions, delivery updates, "
    ...
    assert any(
        ...
        and item["author_label"] == "AI Assistant"
        for item in messages_after
    )
    assert any(
        "shipment" in item["body"].lower()
        or "support" in item["body"].lower()
        for item in messages_after
